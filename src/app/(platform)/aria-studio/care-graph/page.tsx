...
// ══════════════════════════════════════════════════════════════════════════════
// ARIA STUDIO — CARE KNOWLEDGE GRAPH
//
        ...
        <div className="rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-gradient-to-r from-[var(--cs-aria-gold-bg)] to-white p-5">
          <div className="flex items-start gap-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--cs-navy)]">
              <Network className="h-5 w-5 text-[var(--cs-aria-gold)]" />
            </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-all",
              !typeFilter ? "border-[var(--cs-aria-gold)] bg-[var(--cs-aria-gold-bg)] text-[var(--cs-navy)]" : "border-[var(--cs-border)] bg-white text-[var(--cs-text-secondary)]")}
          >
            All ({nodes.length})
          </button>
          {Object.entries(typeCounts).map(([type, count]) => {
            ...
            return (
              <button
                ...
                className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-all",
                  typeFilter === type ? "ring-1 ring-[var(--cs-aria-gold-soft)]" : "", config.colour)}
              >
          ...
          <div className="lg:col-span-1 space-y-3">
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search nodes..." className="w-full rounded-xl border border-[var(--cs-border)] bg-white pl-10 pr-4 py-2.5 text-sm text-[var(--cs-navy)] placeholder:text-[var(--cs-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--cs-aria-gold)]" />
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn("w-full flex items-center gap-2.5 rounded-lg border px-3 py-2.5 text-left transition-all",
                      isSelected ? "border-[var(--cs-aria-gold)] bg-[var(--cs-aria-gold-bg)] ring-1 ring-[var(--cs-aria-gold-soft)]" : "border-[var(--cs-border)] bg-white hover:border-[var(--cs-aria-gold-soft)]")}
                  >
                    ...
                    <ChevronRight className={cn("h-3.5 w-3.5 shrink-0", isSelected ? "text-[var(--cs-aria-gold)]" : "text-[var(--cs-text-muted)]")} />
                  </button>
                );
              })}
            </div>
                        ...
                        <div key={edge.id} className="flex items-center gap-3 rounded-lg border border-[var(--cs-border-subtle)] p-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            <div className="w-12 h-1.5 bg-[var(--cs-surface)] rounded-full overflow-hidden">
                              <div className="h-full bg-[var(--cs-aria-gold)] rounded-full" style={{ width: `${edge.strength * 100}%` }} />
                            </div>
