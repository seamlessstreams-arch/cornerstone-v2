      ...
      <Card className="mb-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Child reference (pseudonym / ID)</label>
              <Input
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Record date</label>
              <Input
                type="date"
                value={recordDate}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Author role (optional)</label>
              <Input
                value={authorRole}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Actor user ID (your sign-in for audit log)</label>
              <Input
                value={actorUserId}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <Card>
              <CardContent className="p-4">
                <div className="text-xs uppercase text-slate-500 mb-1">Quality score</div>
                <div className="text-3xl font-semibold text-slate-900">{review.qualityScore}<span className="text-base text-slate-500">/100</span></div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="text-xs uppercase text-slate-500 mb-1">Risk level</div>
                <Badge className={cn("border", RISK_COLOUR[review.riskLevel])}>{review.riskLevel}</Badge>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="text-xs uppercase text-slate-500 mb-1">Practice judgement</div>
                  ...
                  <ul className="text-sm space-y-1.5">
                    {review.planLinks.map((p) => (
                      <li key={p.plan} className="flex items-start gap-2">
                        ...
                        <div>
                          <div className={cn(p.detected ? "text-slate-900" : "text-slate-500")}>
                            {PLAN_LABEL[p.plan] ?? p.plan}
                          </div>
                          {p.detected && p.evidenceQuote ? (
                            <div className="text-xs text-slate-500 italic">…{p.evidenceQuote}…</div>
                          ) : null}
                        </div>
                      </li>
                    ))}
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between gap-2 text-base">
                    <span className="flex items-center gap-2">
                      <Sparkles className="h-4 w-4 text-violet-500" /> Aria suggested draft
                    </span>
              ...
              <Card>
                ...
                <CardContent>
                  {editing ? (
                    ...
                    <p className="text-sm text-slate-700">{editedSummary}</p>
                  )}
                </CardContent>
              </Card>
