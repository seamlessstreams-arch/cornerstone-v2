...
function LiversGeneratorForm({
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const res = await api.post<{ data: { parsed: Record<string, unknown> } }>("/cara", {
      ...
      if (!parsed) throw new Error("Cara did not return a valid analysis.");
  ...
  return (
    <div className="space-y-3">
      ...
      <Button onClick={handleGenerate} disabled={loading || !canCreateAnalysis} className="gap-2">
        ...
        Generate L.I.V.E.R.S. Analysis with Cara
      </Button>
      {!canCreateAnalysis && (
...
export default function LiversPage() {
  ...
  return (
    <PageShell
      ...
      actions={<SmartUploadButton variant="inline" label="Upload Intervention Document" uploadContext="Cara Intelligence — LIVERS intervention supporting document upload" />}
    >
      <div className="space-y-6 max-w-4xl">
        ...
        {analyses.length === 0 && sessions.length === 0 && selectedChildId && (
          <div className="rounded-xl border border-dashed border-[var(--cs-border)] p-10 text-center">
            ...
            <p className="text-xs text-[var(--cs-text-muted)] mt-1">
              Add context above and generate a L.I.V.E.R.S. Analysis with Cara to get started.
            </p>
          </div>
        )}
