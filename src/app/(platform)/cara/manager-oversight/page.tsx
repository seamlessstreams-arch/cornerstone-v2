      ...
      <div className="mx-auto max-w-3xl space-y-4 pb-10">
        ...
        {data && (
          <>
                    ...
                    {data.awaiting_review.map((r) => (
                      <details key={r.id} className="rounded-xl border border-[var(--cs-border)] bg-white px-3 py-2.5">
                        ...
                        <div className="mt-3 space-y-2 border-t border-[var(--cs-border)] pt-3">
                          ...
                          {r.ai_suggested_text && <details className="rounded-lg bg-[var(--cs-bg)] px-3 py-2"><summary className="cursor-pointer text-[11px] font-bold text-[var(--cs-text-muted)]">Cara suggestion (preserved)</summary><pre className="mt-1.5 whitespace-pre-wrap font-sans text-xs text-[var(--cs-text-secondary)]">{r.ai_suggested_text}</pre></details>}
                    ...
                    return (
                      <div key={a.key} className={cn("rounded-xl border border-l-4 border-[var(--cs-border)] bg-white px-3 py-2.5", m.border, !open && "opacity-55")}>
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {a.incident_session_id && <Link href="/cara/incident-mode" className="inline-flex items-center gap-1 rounded-lg border border-[var(--cs-border)] bg-[var(--cs-bg)] px-2.5 py-1 text-[11px] font-semibold text-[var(--cs-navy)] hover:bg-white">Open Incident Mode <ArrowRight className="h-3 w-3" /></Link>}
                          {open ? (
                            <>
            ...
            <Card>
              <CardContent className="py-4">
                <p className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-[var(--cs-text-muted)]"><TrendingUp className="h-3.5 w-3.5" /> Patterns Cara has noticed</p>
