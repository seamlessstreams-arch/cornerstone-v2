...
export default function EventCapturePage() {
  ...
  return (
    <PageShell
      ...
      caraContext={{ pageTitle: "Event Capture", sourceType: "general" }}
    >
      {isLoading || !intel ? (
        ...
        <div className="space-y-6">
          ...
          <div className={cn("rounded-2xl border p-4 flex items-center gap-3", intel.ready_to_submit ? "border-[--cs-success-soft] bg-[--cs-success-bg]" : "border-[--cs-warning-soft] bg-[--cs-warning-bg]")}>
            {intel.ready_to_submit ? <CheckCircle2 className="h-5 w-5 text-[--cs-success]" /> : <AlertTriangle className="h-5 w-5 text-[--cs-warning]" />}
            <div>
              <p className={cn("text-sm font-semibold", intel.ready_to_submit ? "text-[--cs-success]" : "text-[--cs-warning]")}>{intel.ready_to_submit ? "Ready to submit" : "Held for review before submission"}</p>
              {intel.blocks.length > 0 && <ul className="text-xs text-[--cs-warning] mt-1 list-disc ml-4">{intel.blocks.map((b, i) => <li key={i}>{b}</li>)}</ul>}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Card className="overflow-hidden">
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2">{intel.validation.passed ? <CheckCircle2 className="h-4 w-4 text-[--cs-success]" /> : <XCircle className="h-4 w-4 text-[--cs-risk]" />} Validation</CardTitle></CardHeader>
              <CardContent className="text-xs space-y-1">
                {intel.validation.issues.length === 0 && <p className="text-[--cs-success]">All mandatory fields present.</p>}
                {intel.validation.issues.map((iss, i) => (
                  <p key={i} className={cn("flex items-start gap-1", iss.severity === "error" ? "text-[--cs-risk]" : "text-[--cs-warning]")}>
            ...
            <Card className="overflow-hidden">
              ...
              <CardContent className="text-xs space-y-1">
                {!intel.duplicates.suspected && <p className="text-[--cs-success]">No existing event matches — safe to create.</p>}
                {intel.duplicates.matches.map((m, i) => (
                  <p key={i} className="text-[--cs-warning]">Matches <span className="font-mono">{m.event_id}</span> ({Math.round(m.similarity * 100)}%) — {m.reason}</p>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="overflow-hidden">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-1">
                  ...
                  {intel.routing.external_apis.map((a, i) => <Badge key={`e${i}`} className="text-[10px] bg-[--cs-risk-bg] text-[--cs-risk] border-[--cs-risk-soft]">{a} (gated)</Badge>)}
                </div>
                {intel.routing.requires_human_approval && <p className="text-[10px] text-[--cs-warning] mt-1.5">Requires human approval before external notifications send.</p>}
              </CardContent>
            </Card>
            ...
            <Card className="overflow-hidden">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-1">
                  {intel.evidence_categories.map((c, i) => <Badge key={i} className="text-[10px] bg-[--cs-success-bg] text-[--cs-success] border-[--cs-success-soft]">{c}</Badge>)}
                </div>
              </CardContent>
            </Card>
          </div>
    ...
    <Card className="overflow-hidden border-brand/30">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {capture.isError && <span className="text-xs text-[--cs-risk]">{(capture.error as Error)?.message ?? "Capture failed"}</span>}
        </div>
...
function CaptureOutcomeView({ outcome, onForce, forcing }: { outcome: any; onForce: () => void; forcing: boolean }) {
  ...
  return (
    <div className="space-y-2 pt-1">
      {outcome.persisted ? (
        <div className="rounded-lg border border-[--cs-success-soft] bg-[--cs-success-bg] p-3 text-xs text-[--cs-success] space-y-1">
        ...
        <div className="rounded-lg border border-[--cs-warning-soft] bg-[--cs-warning-bg] p-3 text-xs text-[--cs-warning] space-y-1.5">
          ...
          {c.duplicates?.suspected && c.validation?.passed && (
            <button onClick={onForce} disabled={forcing}
              className="inline-flex items-center gap-1 rounded border border-[--cs-warning-soft] bg-white px-2 py-1 text-[11px] font-medium text-[--cs-warning] disabled:opacity-60">
      ...
      <div className="grid sm:grid-cols-2 gap-2 text-[11px]">
        <div className={cn("rounded-lg border p-2.5 flex items-center gap-2", c.validation?.passed ? "text-[--cs-success]" : "text-[--cs-risk]")}>
        ...
        <div className={cn("rounded-lg border p-2.5 flex items-center gap-2", c.duplicates?.suspected ? "text-[--cs-warning]" : "text-[--cs-success]")}>
