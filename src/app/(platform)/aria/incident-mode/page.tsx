...
export default function AriaIncidentModePage() {
  ...
  return (
    <PageShell
      title="ARIA Incident Mode"
      subtitle="Calm, live support during an incident — co-regulation prompts, a timestamped timeline, the right workflow, and an audit-safe draft record afterwards. ARIA suggests; staff decide; the manager reviews."
      ariaContext={{ pageTitle: "ARIA Incident Mode", sourceType: "incident" }}
    >
    ...
    <>
      ...
      <Card className="border-l-4 border-l-[var(--cs-aria-gold)]">
        <CardContent className="py-4">
            ...
            <div className="mt-3 space-y-3">
              ...
              {draft.ai_draft && (
                <details className="rounded-xl border border-[var(--cs-border)] bg-[var(--cs-bg)] px-3 py-2">
                  <summary className="cursor-pointer text-xs font-bold text-[var(--cs-text-muted)]">View ARIA&rsquo;s suggested rewrite (AI-assisted)</summary>
          ...
          {!recordDone && !draft && (
            <p className="mt-2 text-sm text-[var(--cs-text-secondary)]">Generate a draft assembled from your timeline. ARIA can suggest a professional rewrite — you review, edit and confirm before anything is saved.</p>
          )}
        </CardContent>
      </Card>
    </>
