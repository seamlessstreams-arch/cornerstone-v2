...
const STATUS_CONFIG: Record<StatusKey, { label: string; colour: string; icon: React.ElementType }> = {
  ...
  unknown:  { label: "No Date",   colour: "text-slate-600 bg-slate-50 border-slate-200",    icon: Clock         },
};
  ...
  return (
    <PageShell
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <PrintButton title="Supervision Hub" subtitle="Oak House — Reg 34 Compliance" targetId="supervision-hub-content" />
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className={cn("rounded-xl border p-3 text-center", stats.overdue > 0 ? "border-red-200 bg-red-50" : "border-slate-200 bg-white")}>
          <AlertTriangle className={cn("h-4 w-4 mx-auto mb-1", stats.overdue > 0 ? "text-red-500" : "text-slate-300")} />
          <div className={cn("text-lg font-bold tabular-nums", stats.overdue > 0 ? "text-red-700" : "text-slate-400")}>{stats.overdue}</div>
          <div className="text-[10px] text-slate-500">Overdue</div>
        </div>
        <div className={cn("rounded-xl border p-3 text-center", stats.dueSoon > 0 ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white")}>
          <Clock className={cn("h-4 w-4 mx-auto mb-1", stats.dueSoon > 0 ? "text-amber-500" : "text-slate-300")} />
          <div className={cn("text-lg font-bold tabular-nums", stats.dueSoon > 0 ? "text-amber-700" : "text-slate-400")}>{stats.dueSoon}</div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3 text-center">
          <Calendar className={cn("h-4 w-4 mx-auto mb-1", stats.noDate > 0 ? "text-slate-500" : "text-slate-300")} />
          <div className={cn("text-lg font-bold tabular-nums", stats.noDate > 0 ? "text-slate-700" : "text-slate-400")}>{stats.noDate}</div>
          <div className="text-[10px] text-slate-500">No Date Set</div>
        </div>
      </div>

      {/* Compliance bar */}
      <div className="flex items-center gap-3">
        <span className="text-[10px] font-medium text-slate-500 shrink-0">Reg 34 Compliance</span>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest">Staff Supervision Status</p>
          <span className="text-[10px] text-slate-400">{filteredStaff.length} staff</span>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {member.is_on_leave_today && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium">On Leave</span>
                    )}
                  </div>
