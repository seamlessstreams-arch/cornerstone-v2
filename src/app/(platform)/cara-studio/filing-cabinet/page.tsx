...
export default function FilingCabinetPage() {
  ...
  useEffect(() => {
    fetch("/api/aria-studio/filing-cabinet?mode=structure")
      .then((r) => r.json())
      .then((data) => {
      ...
      <div key={folder.path}>
        <button
          ...
          className={cn(
            ...
            isSelected && "bg-[var(--cs-aria-gold-bg)] border border-[var(--cs-aria-gold-soft)]",
          )}
        ...
        >
          ...
          {hasChildren ? (
            isExpanded ? <FolderOpen className="h-4 w-4 text-[var(--cs-aria-gold)] shrink-0" /> : <Folder className="h-4 w-4 text-[var(--cs-aria-gold)] shrink-0" />
          ) : (
        ...
        <div className="rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-gradient-to-r from-[var(--cs-aria-gold-bg)] to-white p-5">
          <div className="flex items-start gap-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--cs-navy)]">
              <Archive className="h-5 w-5 text-[var(--cs-aria-gold)]" />
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="rounded-2xl border border-[var(--cs-border)] bg-white p-4 space-y-1">
                <h3 className="text-xs font-semibold text-[var(--cs-navy)] uppercase tracking-wide mb-3 flex items-center gap-1.5">
                  <Folder className="h-3.5 w-3.5 text-[var(--cs-aria-gold)]" />
                  Folders
                </h3>
                {structure.length === 0 ? (
                ...
                {selectedPath ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <FolderOpen className="h-5 w-5 text-[var(--cs-aria-gold)]" />
