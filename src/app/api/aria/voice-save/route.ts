...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error || !data) {
      console.error("[api/aria/voice-save] Insert error:", error?.message);
      const result: ServiceResult<never> = {
        ok: false,
    ...
    if (body.sessionId) {
      await (sb.from("aria_sessions") as SB)
        ...
        .catch((err: Error) => {
          console.error("[api/aria/voice-save] Session update error:", err.message);
        });
    }
    ...
    console.error("[api/aria/voice-save] Error:", error);
