...
import { checkAriaAccess, type AriaRole } from "@/lib/aria/aria-permissions";
...
export async function GET(req: NextRequest) {
  ...
  const actorRole = (url.searchParams.get("actorRole") ?? "none") as AriaRole;
  ...
  const access = checkAriaAccess(
    { userId: actorUserId, role: actorRole, homeId },
    { permission: "aria.approve_outputs", homeId },
  );
  if (!access.allowed) {
    return NextResponse.json(
