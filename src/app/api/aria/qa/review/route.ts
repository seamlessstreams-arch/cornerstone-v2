// ══════════════════════════════════════════════════════════════════════════════
// API: POST /api/aria/qa/review — Quality assurance record review
//
// Reviews records for quality, identifies gaps, and generates QA feedback.
// ══════════════════════════════════════════════════════════════════════════════

import { NextRequest, NextResponse } from "next/server";
import { AriaQualityAssuranceEngine } from "@/lib/aria/qa/qa-engine";
import { sanitiseErrorForClient } from "@/lib/aria/core/errors";
import type { AriaQAReviewRequest } from "@/lib/aria/core/types";

const qaEngine = new AriaQualityAssuranceEngine();
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.fullReview) {
      const request: AriaQAReviewRequest = {
