...
import { checkShiftSafety, type ShiftContext } from "@/lib/aria/shift-safety";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("[aria/shift-safety] GET error:", err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[aria/shift-safety] POST error:", err);
    return NextResponse.json({ error: "Failed to check shift safety" }, { status: 500 });
  }
}
