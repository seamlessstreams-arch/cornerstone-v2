...
import { orchestrate } from "@/lib/aria/orchestrator";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Build AriaRequest with voice_reflection hint ─────────────────────────
    const ariaRequest = {
    ...
    // ── Run orchestration ────────────────────────────────────────────────────
    const result = await orchestrate(ariaRequest);
    ...
    if (sessionId && isSupabaseEnabled()) {
      ...
      if (sb) {
        try {
          ...
          console.error("[api/aria/voice-process] Session persistence error:", err);
        }
      }
    }
    ...
    console.error("[api/aria/voice-process] Error:", error);
