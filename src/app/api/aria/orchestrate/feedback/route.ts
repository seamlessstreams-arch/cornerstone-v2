// ══════════════════════════════════════════════════════════════════════════════
// API: /api/aria/orchestrate/feedback
//
...
export async function POST(req: NextRequest) {
  ...
  if (body.issueType && !issueType) {
    console.warn(
      `[aria/orchestrate/feedback] Invalid issueType "${body.issueType}" — ignored. Valid types: ${VALID_ISSUE_TYPES.join(", ")}`,
    );
  }
  ...
  if (existingFeedback && existingFeedback.length > 0) {
    ...
    if (updateError) {
      console.error("[aria/orchestrate/feedback] Update error:", updateError.message);
      return NextResponse.json(
  ...
  if (insertError || !newFeedback) {
    console.error("[aria/orchestrate/feedback] Insert error:", insertError?.message);
    return NextResponse.json(
