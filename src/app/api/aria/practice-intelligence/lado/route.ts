...
import { analyzePractice } from "@/lib/aria-practice/aria-practice-engine";
...
export async function POST(req: Request) {
  ...
  const decision = checkAriaAccess(
    { userId, role: ariaRole, homeId: body.homeId, staffSelfId: userId },
    { permission: "aria.analyse_risk", homeId: body.homeId, childId: body.childId, staffId: body.staffId, isSafeguardingSensitive: true },
  );
  if (!decision.allowed) {
  ...
  if (body.persist !== false && ladoFlag) {
    const rec = db.ariaPracticeFlags.create({
