// ══════════════════════════════════════════════════════════════════════════════
// API: /api/aria/voice-of-child
//
// POST    /api/aria/voice-of-child                 — summarise voice across a
//                                                    batch of records, persist
//                                                    a draft summary
// GET     /api/aria/voice-of-child?id=...          — fetch a single summary
// GET     /api/aria/voice-of-child?childId=...     — list summaries for a child
// PATCH   /api/aria/voice-of-child                 — manager edit / approve /
...
import {
  ...
  type SourceRecord,
  ENGINE_VERSION,
} from "@/lib/aria/voiceOfChildSummariser";
...
export async function POST(req: NextRequest) {
  ...
  if (!persist || !isSupabaseEnabled()) {
    return NextResponse.json({
      data: { summary, persisted: false, ariaLabel: summary.ariaLabel },
    });
  }
  ...
  const { error: insertError } = await supabase
    ...
    .insert({
      ...
      aria_confidence: summary.ariaConfidence,
