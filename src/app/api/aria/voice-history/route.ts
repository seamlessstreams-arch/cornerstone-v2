...
export async function GET(req: NextRequest) {
  try {
    ...
    if (isSupabaseEnabled()) {
      ...
      if (sb) {
        try {
          ...
          if (error) {
            console.error("[api/aria/voice-history] Session query error:", error.message);
          ...
          console.error("[api/aria/voice-history] Database error:", err);
          // Fall through to demo data
        }
      }
    }
    ...
    console.error("[api/aria/voice-history] Error:", error);
