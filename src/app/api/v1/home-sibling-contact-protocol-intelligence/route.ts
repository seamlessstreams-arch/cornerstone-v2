...
export async function GET() {
  ...
  const raw = db.siblingContactProtocolRecords.getAll();
  const today = new Date().toISOString().slice(0, 10);
  const todayMs = new Date(today).getTime();

  const records: SiblingContactRecordInput[] = raw.map((r: any, i: number) => {
  ...
  const result = computeSiblingContactProtocol({ today, total_children: children.length, records });
