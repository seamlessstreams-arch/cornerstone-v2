...
// existing ariaRecordingReviews collection from Incident Mode is reused — one
...
import { analyseRecordingQuality, RECORD_TYPES, RECORDING_DISCLAIMER } from "@/lib/aria-incident/recording-assistant-engine";
import { ARIA_INCIDENT_SYSTEM_PROMPT, type AriaRecordingReview } from "@/lib/aria-incident/aria-incident-engine";
...
export async function GET() {
  ...
  const recent = ((store.ariaRecordingReviews ?? []) as AriaRecordingReview[])
    .slice()
...
export async function POST(req: Request) {
  ...
  const result = await generateText({
    systemPrompt: ARIA_INCIDENT_SYSTEM_PROMPT,
