...
} from "@/lib/aria-incident/post-incident-engine";
import { findSession, addTimelineEntry, currentUserId, logIncidentAudit, childName } from "@/lib/aria-incident/incident-service";

function recordsFor(sessionId: string) {
  const store = getStore() as any;
  return {
    restorative: ((store.ariaRestorativeConversations ?? []) as RestorativeConversationRecord[]).filter((r) => r.incident_session_id === sessionId),
    reflections: ((store.ariaPostIncidentReflections ?? []) as PostIncidentReflectionRecord[]).filter((r) => r.incident_session_id === sessionId),
  };
}
