...
import { INCIDENT_TYPES, INCIDENT_DISCLAIMER, type IncidentSession, type RiskLevel } from "@/lib/aria-incident/aria-incident-engine";
import { startSession, currentUserId, childName, sessionEntries } from "@/lib/aria-incident/incident-service";

const RISKS = new Set(["low", "medium", "high"]);

export async function GET() {
  const store = getStore() as any;
  const sessions = ((store.ariaIncidentSessions ?? []) as IncidentSession[])
    .slice()
