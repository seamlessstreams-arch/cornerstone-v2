...
export async function GET() {
  ...
  const result = computeSafeguardingOverview({
    today,
    incidents: (store.incidents ?? []).map((i: any) => ({
      id: String(i.id),
    ...
    missing: (store.missingEpisodes ?? []).map((m: any) => ({
      id: String(m.id),
    ...
    risk: (store.riskAssessments ?? []).map((r: any) => ({
      id: String(r.id),
    ...
    lado: (store.ladoReferrals ?? []).map((l: any) => ({
      id: String(l.id),
    ...
    notifiable: (store.notifiableEvents ?? []).map((n: any) => ({
      id: String(n.id),
