...
export async function PATCH(
  ...
  const permissionMap: Record<SuggestionAction, CaraPermission> = {
    approve: "cara.approve_outputs",
    commit: "cara.commit_to_records",
    reject: "cara.reject_outputs",
    no_action: "cara.approve_outputs",
  };

  const access = checkCaraAccess(actor, { permission: permissionMap[action] });
  if (!access.allowed) {
    return NextResponse.json({ error: "Access denied", reason: access.reason }, { status: 403 });
  }
