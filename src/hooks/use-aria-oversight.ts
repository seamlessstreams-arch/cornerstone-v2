...
import type { ManagerAlert, PatternInsight } from "@/lib/aria-incident/manager-oversight-engine";
...
export interface OversightData {
  ...
  awaiting_review: (AriaRecordingReview & { child_name: string; staff_name: string })[];
  disclaimer: string;
}
...
export function useAriaOversight() {
  return useQuery<OversightData>({
    queryKey: ["aria-oversight"],
    queryFn: () => fetch("/api/v1/aria-manager-oversight").then(json),
...
export function useOversightAction() {
  ...
  return useMutation({
    mutationFn: (payload: { action: "set_alert_status"; key: string; status: "resolved" | "dismissed" | "open" } | { action: "mark_reviewed"; review_id: string }) =>
      fetch("/api/v1/aria-manager-oversight", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }).then(json),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["aria-oversight"] }),
  });
}
