...
function makeRow(
  overrides?: Partial<ChildRadicalisationRiskRow>,
): ChildRadicalisationRiskRow {
  return {
...
describe("child-radicalisation-risk-service", () => {
  // ── Enum validation ──────────────────────────────────────────────────
  describe("Enum validation", () => {
    ...
    it("EXTREMISM_TYPES has 7 values", () => { expect(EXTREMISM_TYPES).toHaveLength(7); });
    it("EXTREMISM_TYPES contains Islamist", () => { expect(EXTREMISM_TYPES).toContain("Islamist"); });
    it("EXTREMISM_TYPES contains Far Right", () => { expect(EXTREMISM_TYPES).toContain("Far Right"); });
    it("EXTREMISM_TYPES contains Far Left", () => { expect(EXTREMISM_TYPES).toContain("Far Left"); });
    it("EXTREMISM_TYPES contains Incel", () => { expect(EXTREMISM_TYPES).toContain("Incel"); });
    it("EXTREMISM_TYPES contains Mixed/Unclear", () => { expect(EXTREMISM_TYPES).toContain("Mixed/Unclear"); });
    it("EXTREMISM_TYPES contains Single Issue", () => { expect(EXTREMISM_TYPES).toContain("Single Issue"); });
    it("EXTREMISM_TYPES contains Not Determined", () => { expect(EXTREMISM_TYPES).toContain("Not Determined"); });

    it("INDICATOR_TYPES has 8 values", () => { expect(INDICATOR_TYPES).toHaveLength(8); });
    it("INDICATOR_TYPES contains Online Activity", () => { expect(INDICATOR_TYPES).toContain("Online Activity"); });
    it("INDICATOR_TYPES contains Social Isolation", () => { expect(INDICATOR_TYPES).toContain("Social Isolation"); });
    it("INDICATOR_TYPES contains Expressed Views", () => { expect(INDICATOR_TYPES).toContain("Expressed Views"); });
    it("INDICATOR_TYPES contains Material Possession", () => { expect(INDICATOR_TYPES).toContain("Material Possession"); });
    it("INDICATOR_TYPES contains Behavioural Change", () => { expect(INDICATOR_TYPES).toContain("Behavioural Change"); });
    it("INDICATOR_TYPES contains Association", () => { expect(INDICATOR_TYPES).toContain("Association"); });
    it("INDICATOR_TYPES contains Travel Concerns", () => { expect(INDICATOR_TYPES).toContain("Travel Concerns"); });
    it("INDICATOR_TYPES contains Not Determined", () => { expect(INDICATOR_TYPES).toContain("Not Determined"); });
  ...
  describe("computeMetrics", () => {
    ...
    // prevent_referral_rate
    it("prevent_referral_rate 100 when all true", () => { expect(computeMetrics([makeRow({ prevent_referral_made: true })]).prevent_referral_rate).toBe(100); });
    it("prevent_referral_rate 0 when all false", () => { expect(computeMetrics([makeRow({ prevent_referral_made: false })]).prevent_referral_rate).toBe(0); });
    it("prevent_referral_rate mixed 2 of 3 gives 66.7", () => {
      const m = computeMetrics([
        makeRow({ prevent_referral_made: true }),
        makeRow({ prevent_referral_made: false }),
        makeRow({ prevent_referral_made: true }),
      ]);
      expect(m.prevent_referral_rate).toBe(66.7);
    });
    it("prevent_referral_rate mixed 1 of 3 gives 33.3", () => {
      const m = computeMetrics([
        makeRow({ prevent_referral_made: true }),
        makeRow({ prevent_referral_made: false }),
        makeRow({ prevent_referral_made: false }),
      ]);
      expect(m.prevent_referral_rate).toBe(33.3);
    });

    // channel_rate
    it("channel_rate 100 when all true", () => { expect(computeMetrics([makeRow({ channel_programme: true })]).channel_rate).toBe(100); });
    it("channel_rate 0 when all false", () => { expect(computeMetrics([makeRow({ channel_programme: false })]).channel_rate).toBe(0); });
    it("channel_rate mixed 1 of 2 gives 50", () => {
      const m = computeMetrics([
        makeRow({ channel_programme: true }),
        makeRow({ channel_programme: false }),
      ]);
    ...
    // internet_monitoring_rate
    it("internet_monitoring_rate 100 when all true", () => { expect(computeMetrics([makeRow({ internet_monitoring: true })]).internet_monitoring_rate).toBe(100); });
    it("internet_monitoring_rate 0 when all false", () => { expect(computeMetrics([makeRow({ internet_monitoring: false })]).internet_monitoring_rate).toBe(0); });
    it("internet_monitoring_rate mixed 2 of 4 gives 50", () => {
      const m = computeMetrics([
        makeRow({ internet_monitoring: true }),
        makeRow({ internet_monitoring: true }),
        makeRow({ internet_monitoring: false }),
        makeRow({ internet_monitoring: false }),
      ]);
      expect(m.internet_monitoring_rate).toBe(50);
    });
    ...
    it("multiple rows aggregate correctly", () => {
      const m = computeMetrics([
        makeRow({ risk_level: "High", prevent_referral_made: true, channel_programme: true, child_name: "A", assessor_name: "X" }),
        makeRow({ risk_level: "Immediate", prevent_referral_made: false, channel_programme: false, child_name: "B", assessor_name: "Y" }),
        makeRow({ risk_level: "Low", prevent_referral_made: true, channel_programme: true, child_name: "A", assessor_name: "X" }),
        makeRow({ risk_level: "Medium", prevent_referral_made: false, channel_programme: false, child_name: "C", assessor_name: "Z" }),
      ]);
    ...
    it("rate rounding uses Math.round with 1000/10 pattern", () => {
      ...
      const rows = Array.from({ length: 6 }, (_, i) =>
        makeRow({ prevent_referral_made: i === 0 }),
      );
      expect(computeMetrics(rows).prevent_referral_rate).toBe(16.7);
    });
    ...
    it("rate 2 of 7 gives 28.6", () => {
      const rows = Array.from({ length: 7 }, (_, i) =>
        makeRow({ channel_programme: i < 2 }),
      );
      expect(computeMetrics(rows).channel_rate).toBe(28.6);
    });
    ...
    it("rate 4 of 7 gives 57.1", () => {
      const rows = Array.from({ length: 7 }, (_, i) =>
        makeRow({ internet_monitoring: i < 4 }),
      );
      expect(computeMetrics(rows).internet_monitoring_rate).toBe(57.1);
    });
  });
  ...
  describe("computeAlerts", () => {
    ...
    // Critical: high_risk_no_prevent_referral
    it("fires high_risk_no_prevent_referral for High risk without Prevent referral", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", prevent_referral_made: false, child_name: "Jo" })]);
      const f = a.find((x) => x.type === "high_risk_no_prevent_referral");
    ...
    it("fires high_risk_no_prevent_referral for Immediate risk without Prevent referral", () => {
      const a = computeAlerts([makeRow({ risk_level: "Immediate", prevent_referral_made: false })]);
      expect(a.find((x) => x.type === "high_risk_no_prevent_referral")).toBeDefined();
    });
    it("does not fire high_risk_no_prevent_referral for Medium risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "Medium", prevent_referral_made: false })]);
      expect(a.find((x) => x.type === "high_risk_no_prevent_referral")).toBeUndefined();
    });
    it("does not fire high_risk_no_prevent_referral for Low risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "Low", prevent_referral_made: false })]);
      expect(a.find((x) => x.type === "high_risk_no_prevent_referral")).toBeUndefined();
    });
    it("does not fire high_risk_no_prevent_referral for No Identified Risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "No Identified Risk", prevent_referral_made: false })]);
      expect(a.find((x) => x.type === "high_risk_no_prevent_referral")).toBeUndefined();
    });
    it("does not fire high_risk_no_prevent_referral when Prevent referral made", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", prevent_referral_made: true })]);
      expect(a.find((x) => x.type === "high_risk_no_prevent_referral")).toBeUndefined();
    });
    it("high_risk_no_prevent_referral fires per-record", () => {
      const a = computeAlerts([
        makeRow({ id: "a-1", risk_level: "High", prevent_referral_made: false }),
        makeRow({ id: "a-2", risk_level: "Immediate", prevent_referral_made: false }),
      ]);
      expect(a.filter((x) => x.type === "high_risk_no_prevent_referral")).toHaveLength(2);
    });
    it("high_risk_no_prevent_referral message contains radicalisation", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", prevent_referral_made: false })]);
      const f = a.find((x) => x.type === "high_risk_no_prevent_referral");
      expect(f!.message).toContain("radicalisation");
    });
    ...
    it("immediate_no_safety_plan message contains radicalisation", () => {
    ...
    it("no_police_high_risk message contains radicalisation", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", police_notification: false })]);
      const f = a.find((x) => x.type === "no_police_high_risk");
      expect(f!.message).toContain("radicalisation");
    });

    // Medium: no_channel_high_risk
    it("fires no_channel_high_risk for High risk without Channel programme", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", channel_programme: false, child_name: "Zara" })]);
      const f = a.find((x) => x.type === "no_channel_high_risk");
      expect(f).toBeDefined();
      expect(f!.severity).toBe("medium");
      expect(f!.message).toContain("Zara");
    });
    it("fires no_channel_high_risk for Immediate risk without Channel programme", () => {
      const a = computeAlerts([makeRow({ risk_level: "Immediate", channel_programme: false })]);
      expect(a.find((x) => x.type === "no_channel_high_risk")).toBeDefined();
    });
    it("does not fire no_channel_high_risk for Medium risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "Medium", channel_programme: false })]);
      expect(a.find((x) => x.type === "no_channel_high_risk")).toBeUndefined();
    });
    it("does not fire no_channel_high_risk for Low risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "Low", channel_programme: false })]);
      expect(a.find((x) => x.type === "no_channel_high_risk")).toBeUndefined();
    });
    it("does not fire no_channel_high_risk for No Identified Risk", () => {
      const a = computeAlerts([makeRow({ risk_level: "No Identified Risk", channel_programme: false })]);
      expect(a.find((x) => x.type === "no_channel_high_risk")).toBeUndefined();
    });
    it("does not fire no_channel_high_risk when Channel programme engaged", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", channel_programme: true })]);
      expect(a.find((x) => x.type === "no_channel_high_risk")).toBeUndefined();
    });
    it("no_channel_high_risk fires per-record", () => {
      const a = computeAlerts([
        makeRow({ id: "a-1", risk_level: "High", channel_programme: false }),
        makeRow({ id: "a-2", risk_level: "Immediate", channel_programme: false }),
      ]);
      expect(a.filter((x) => x.type === "no_channel_high_risk")).toHaveLength(2);
    });
    it("no_channel_high_risk message contains radicalisation", () => {
      const a = computeAlerts([makeRow({ risk_level: "High", channel_programme: false })]);
      const f = a.find((x) => x.type === "no_channel_high_risk");
      expect(f!.message).toContain("radicalisation");
    });
    ...
    it("fires multiple alert types simultaneously", () => {
      ...
      expect(types).toContain("high_risk_no_prevent_referral");
      expect(types).toContain("immediate_no_safety_plan");
      expect(types).toContain("no_police_high_risk");
      expect(types).toContain("no_channel_high_risk");
    });
    ...
    it("mixed rows produce correct alert counts", () => {
      const a = computeAlerts([
        makeRow({ risk_level: "Immediate", prevent_referral_made: false, safety_plan_in_place: false, police_notification: false, channel_programme: false }),
        makeRow({ risk_level: "Low", prevent_referral_made: true }),
        makeRow({ risk_level: "No Identified Risk", prevent_referral_made: false }),
      ]);
      // Row 1: high_risk_no_prevent_referral, immediate_no_safety_plan, no_police_high_risk, no_channel_high_risk
      // Row 2: none (clean)
      // Row 3: No Identified Risk excluded from all alerts
      expect(a.filter((x) => x.type === "high_risk_no_prevent_referral")).toHaveLength(1);
      ...
      expect(a.filter((x) => x.type === "no_channel_high_risk")).toHaveLength(1);
    });
  ...
  describe("generateAriaInsights", () => {
    ...
    it("insight 1 starts with [red]", () => {
      expect(generateAriaInsights([])[0]).toMatch(/^\[red\]/);
    });
    ...
    it("insight 1 contains total assessments count", () => {
      const insights = generateAriaInsights([makeRow(), makeRow()]);
      expect(insights[0]).toContain("2 radicalisation risk assessments");
    });
    ...
    it("insight 1 contains Prevent referral rate", () => {
      const insights = generateAriaInsights([makeRow({ prevent_referral_made: true })]);
      expect(insights[0]).toContain("Prevent referral rate");
      expect(insights[0]).toContain("100%");
    });
    it("insight 2 mentions critical and high alert counts when present", () => {
      const rows = [
        makeRow({ risk_level: "Immediate", prevent_referral_made: false, safety_plan_in_place: false, police_notification: false }),
      ];
    ...
    it("insight 2 contains Channel programme rate", () => {
      const insights = generateAriaInsights([makeRow()]);
      expect(insights[1]).toContain("Channel programme rate");
    });
    it("insight 2 contains police notification rate", () => {
      const insights = generateAriaInsights([makeRow()]);
      expect(insights[1]).toContain("Police notification rate");
    });
    it("insight 2 contains internet monitoring rate", () => {
      const insights = generateAriaInsights([makeRow()]);
      expect(insights[1]).toContain("Internet monitoring rate");
    });
    it("insight 3 contains reflective question about Prevent referrals", () => {
      const insights = generateAriaInsights([]);
      expect(insights[2]).toContain("Prevent referrals");
    });
    it("insight 3 mentions Channel programme", () => {
      const insights = generateAriaInsights([]);
      expect(insights[2]).toContain("Channel programme");
    });
    it("insight 3 mentions internet monitoring", () => {
      const insights = generateAriaInsights([]);
      expect(insights[2]).toContain("internet monitoring");
    });
    ...
    it("empty array still produces meaningful content", () => {
      const insights = generateAriaInsights([]);
      expect(insights[0]).toContain("0 radicalisation risk assessments");
    ...
    it("insight 2 with alerts shows correct critical count", () => {
      const rows = [
        makeRow({ risk_level: "Immediate", prevent_referral_made: false, safety_plan_in_place: false }),
      ];
      const insights = generateAriaInsights(rows);
  ...
  describe("makeRow helper", () => {
    it("produces valid default row", () => {
      ...
      expect(r.extremism_type).toBe("Not Determined");
      expect(r.indicator_type).toBe("Not Determined");
      expect(r.prevent_referral_made).toBe(true);
      expect(r.channel_programme).toBe(true);
      expect(r.police_notification).toBe(true);
      expect(r.safety_plan_in_place).toBe(true);
      expect(r.multi_agency_referral).toBe(true);
      expect(r.internet_monitoring).toBe(true);
    ...
    it("overrides extremism_type", () => {
      const r = makeRow({ extremism_type: "Far Right" });
      expect(r.extremism_type).toBe("Far Right");
    });
    it("overrides indicator_type", () => {
      const r = makeRow({ indicator_type: "Online Activity" });
      expect(r.indicator_type).toBe("Online Activity");
    });
    ...
    it("overrides prevent_referral_made", () => {
      const r = makeRow({ prevent_referral_made: false });
      expect(r.prevent_referral_made).toBe(false);
    });
    it("overrides channel_programme", () => {
      const r = makeRow({ channel_programme: false });
      expect(r.channel_programme).toBe(false);
    });
    ...
    it("overrides internet_monitoring", () => {
      const r = makeRow({ internet_monitoring: false });
      expect(r.internet_monitoring).toBe(false);
    });
  });
  ...
  describe("Edge cases", () => {
    it("metrics handles single row correctly", () => {
      ...
      expect(m.immediate_count).toBe(1);
      expect(m.prevent_referral_rate).toBe(100);
      expect(m.channel_rate).toBe(100);
      expect(m.police_notification_rate).toBe(100);
      expect(m.safety_plan_rate).toBe(100);
      expect(m.multi_agency_rate).toBe(100);
      expect(m.internet_monitoring_rate).toBe(100);
    });
    ...
    it("handles all extremism types in metrics", () => {
      const types = ["Islamist", "Far Right", "Far Left", "Incel", "Mixed/Unclear", "Single Issue", "Not Determined"] as const;
      const rows = types.map((t) => makeRow({ extremism_type: t }));
      const m = computeMetrics(rows);
    ...
    it("handles all indicator types", () => {
      const types = ["Online Activity", "Social Isolation", "Expressed Views", "Material Possession", "Behavioural Change", "Association", "Travel Concerns", "Not Determined"] as const;
      const rows = types.map((t) => makeRow({ indicator_type: t }));
      const m = computeMetrics(rows);
    ...
    it("insights handle all-Immediate data set", () => {
      const rows = Array.from({ length: 3 }, () =>
        makeRow({ risk_level: "Immediate", prevent_referral_made: true, safety_plan_in_place: true }),
      );
      const insights = generateAriaInsights(rows);
      expect(insights).toHaveLength(3);
    ...
    it("insights handle mixed risk levels", () => {
      ...
      const insights = generateAriaInsights(rows);
      expect(insights[0]).toContain("5 radicalisation risk assessments");
      expect(insights[0]).toContain("2 at High or Immediate");
    });

    it("alerts for High risk without Prevent referral but with safety plan is only high_risk_no_prevent_referral", () => {
      const a = computeAlerts([makeRow({
        risk_level: "High",
        prevent_referral_made: false,
        safety_plan_in_place: true,
      ...
      expect(a[0].type).toBe("high_risk_no_prevent_referral");
    });
    ...
    it("alerts for High risk only missing Channel produces single medium alert type", () => {
      const a = computeAlerts([makeRow({
        risk_level: "High",
        prevent_referral_made: true,
        safety_plan_in_place: true,
      ...
      expect(a).toHaveLength(1);
      expect(a[0].type).toBe("no_channel_high_risk");
    });
    ...
    it("rate 1 of 7 gives 14.3", () => {
      const rows = Array.from({ length: 7 }, (_, i) =>
        makeRow({ channel_programme: i === 0 }),
      );
      expect(computeMetrics(rows).channel_rate).toBe(14.3);
    });

    it("rate 6 of 7 gives 85.7", () => {
      const rows = Array.from({ length: 7 }, (_, i) =>
        makeRow({ internet_monitoring: i < 6 }),
      );
      expect(computeMetrics(rows).internet_monitoring_rate).toBe(85.7);
    });
  });
});
