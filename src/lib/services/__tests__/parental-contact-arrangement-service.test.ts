...
const { computeParentalContactMetrics, computeParentalContactAlerts, generateParentalContactAriaInsights } = _testing;
...
describe("parental-contact-arrangement-service", () => {
  ...
  describe("generateParentalContactAriaInsights", () => {
    it("returns 3 insights for empty data", () => { const m = computeParentalContactMetrics([]); const a = computeParentalContactAlerts([]); const insights = generateParentalContactAriaInsights(m, a); expect(insights).toHaveLength(3); });
    it("insight 1 starts with [pink]", () => { const m = computeParentalContactMetrics([]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[0]).toMatch(/^\[pink\]/); });
    it("insight 2 starts with [amber]", () => { const m = computeParentalContactMetrics([]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[1]).toMatch(/^\[amber\]/); });
    it("insight 3 starts with [reflect]", () => { const m = computeParentalContactMetrics([]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[2]).toMatch(/^\[reflect\]/); });
    it("insight 1 contains total contacts count", () => { const m = computeParentalContactMetrics([makeRow(), makeRow()]); const a = computeParentalContactAlerts([makeRow(), makeRow()]); expect(generateParentalContactAriaInsights(m, a)[0]).toContain("2 parental contact arrangements"); });
    it("insight 1 contains unique children count", () => { const m = computeParentalContactMetrics([makeRow({ child_name: "A" }), makeRow({ child_name: "B" })]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[0]).toContain("2 children"); });
    it("insight 1 uses singular child for 1", () => { const m = computeParentalContactMetrics([makeRow()]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[0]).toContain("1 child"); });
    it("insight 1 contains negative count and percentage", () => { const m = computeParentalContactMetrics([makeRow({ contact_outcome: "negative" })]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[0]).toContain("1 (100%)"); });
    it("insight 1 contains cancelled and refused counts", () => { const m = computeParentalContactMetrics([makeRow({ contact_outcome: "cancelled_by_parent" }), makeRow({ child_experience: "refused_contact" })]); const a = computeParentalContactAlerts([]); const i = generateParentalContactAriaInsights(m, a)[0]; expect(i).toContain("1 cancelled"); expect(i).toContain("1 refused"); });
    it("insight 1 contains court compliance rate", () => { const m = computeParentalContactMetrics([makeRow({ court_order_status: "court_ordered", court_order_complied: true })]); const a = computeParentalContactAlerts([]); expect(generateParentalContactAriaInsights(m, a)[0]).toContain("100%"); });
    it("insight 2 mentions critical and high alert counts", () => { const rows = [makeRow({ court_order_status: "court_ordered", court_order_complied: false, contact_outcome: "negative", child_views_before: false, child_views_after: false }), makeRow({ child_views_before: false })]; const m = computeParentalContactMetrics(rows); const a = computeParentalContactAlerts(rows); const i = generateParentalContactAriaInsights(m, a)[1]; expect(i).toContain("1 critical"); expect(i).toContain("1 high-priority"); });
    it("insight 2 shows no concerns when none", () => { const m = computeParentalContactMetrics([makeRow()]); const a = computeParentalContactAlerts([makeRow()]); expect(generateParentalContactAriaInsights(m, a)[1]).toContain("No critical or high-priority concerns"); });
    it("insight 2 contains child views rates", () => { const m = computeParentalContactMetrics([makeRow()]); const a = computeParentalContactAlerts([]); const i = generateParentalContactAriaInsights(m, a)[1]; expect(i).toContain("Child views before rate"); expect(i).toContain("Child views after rate"); });
    it("insight 3 contains reflective question", () => { const m = computeParentalContactMetrics([]); const a = computeParentalContactAlerts([]); const i = generateParentalContactAriaInsights(m, a)[2]; expect(i).toContain("contact arrangements"); expect(i).toContain("child"); });
  });
});
