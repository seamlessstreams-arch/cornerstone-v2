...
import {
  ...
  type CaraProviderConfig,
} from "@/lib/cara/cara-provider";
import type {
  CaraCommandId,
  CaraCommandSpec,
  CaraConfidence,
  CaraGenerationResult,
  CaraInvocationInput,
...
export const CARA_COMMANDS: Record<CaraCommandId, CaraCommandSpec> = {
  ...
  summarise_text: {
    ...
    modules: [],
    requiredPermission: "cara.summarise",
    approvalRequired: true,
    canCreateTasks: false,
    canCommit: false,
    riskLevel: "low",
  ...
  create_task_list: {
    ...
    modules: [],
    requiredPermission: "cara.create_tasks",
    approvalRequired: true,
    canCreateTasks: true,
    canCommit: false,
    riskLevel: "medium",
...
export interface CaraInvokeArgs extends CaraInvocationInput {
  actor: CaraActor;
}
...
export async function invokeCaraCommand(
  args: CaraInvokeArgs,
): Promise<CaraInvokeOutcome> {
  const providerConfig = getCaraProviderConfig();
  const command = CARA_COMMANDS[args.commandId];
  if (!command) {
    return {
      ok: false,
  ...
  // Permission check
  const access = checkCaraAccess(args.actor, {
  ...
  const { tryRulesFirst } = await import("@/lib/cara/rules-engine");
  ...
  if (ruleResult) {
    ...
    const { lookupLearnedAnswer, learnAnswer } = await import("@/lib/cara/resolution/learned-cache");
...
export type CaraApprovalDecision = "approve" | "reject" | "request_changes" | "commit" | "withdraw";

export interface ApplyApprovalArgs {
  outputId: string;
  decision: CaraApprovalDecision;
  decisionText?: string;
  editedText?: string;
...
export async function applyApprovalDecision(args: ApplyApprovalArgs): Promise<{
  ...
  const access = checkCaraAccess(args.actor, { permission: args.requiredPermission });
  if (!access.allowed) {
...
function buildSystemPrompt(command: CaraCommandSpec): string {
  return [
...
function buildUserPrompt(args: CaraInvokeArgs): string {
  const lines: string[] = [];
...
function inferConfidence(command: CaraCommandSpec, text: string): CaraConfidence {
...
function redactedContextSummaryFor(args: CaraInvokeArgs): string {
  const parts: string[] = [];
...
function contextRecordIdsFor(args: CaraInvokeArgs): string[] {
  const ids: string[] = [];
