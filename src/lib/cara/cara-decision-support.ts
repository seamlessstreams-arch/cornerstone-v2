...
function evidenceFromRisk(r: RiskAssessment): CaraSafeguardingEvidenceRef {
  return {
...
function priorityFor(severity: "low" | "medium" | "high" | "critical"): CaraDecisionPriority {
  switch (severity) {
...
function dueDateFor(priority: CaraDecisionPriority): string {
  const offsets: Record<CaraDecisionPriority, number> = { p1: 1, p2: 3, p3: 7, p4: 14 };
  const d = new Date();
...
function buildFormulationForChild(params: {
...
}): Omit<CaraFormulation, "id"> {
  const { homeId, childId, patterns, warnings, risks } = params;
  const factors: CaraFormulationFactor[] = [];
  ...
  const protectiveItems: { strategy: string; refs: CaraSafeguardingEvidenceRef[] }[] = [];
...
export function runDecisionSupport(
  ...
  // Source records
  const allPatterns = db.caraSafeguardingPatterns
    .findOpen(homeId)
    .filter((p) => (childIdFilter ? p.child_id === childIdFilter : true));
  const allWarnings = db.caraEarlyWarnings
    .findActive(homeId)
  ...
  const formulations: CaraFormulation[] = [];
  for (const childId of childIds) {
    ...
    const prior = db.caraFormulations
      .findByChild(homeId, childId)
      .find((f) => f.status === "ai_draft");
    if (prior) {
      db.caraFormulations.patch(prior.id, { status: "superseded" });
    }
    const created = db.caraFormulations.create(draft);
    if (prior) {
      db.caraFormulations.patch(prior.id, { superseded_by: created.id });
    }
  ...
  const recommendations: CaraDecisionRecommendation[] = [];
  for (const d of drafts) {
    ...
    const existing = db.caraDecisionRecommendations
      .findOpen(homeId)
    ...
    const payload: Omit<CaraDecisionRecommendation, "id"> = {
      home_id: homeId,
      child_id: childId,
    ...
    if (existing) {
      const patched = db.caraDecisionRecommendations.patch(existing.id, payload);
      if (patched) recommendations.push(patched);
    } else {
      const created = db.caraDecisionRecommendations.create(payload);
...
export function loadDecisionSupport(
  ...
  const formulations = db.caraFormulations
    .findAll(homeId)
    .filter((f) => f.status !== "superseded" && f.status !== "rejected")
    .filter((f) => (childId ? f.child_id === childId : true));
  const recommendations = db.caraDecisionRecommendations
    .findAll(homeId)
    .filter((r) =>
