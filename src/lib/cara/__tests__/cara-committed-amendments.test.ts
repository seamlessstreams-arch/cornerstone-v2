...
describe("cara-committed-amendments engine", () => {
  beforeEach(() => clearAll());
  ...
  it("parent suggestion's committed_record_id re-points to the new head", () => {
    const c = commitOne("reflection");
    const sug = db.caraSuggestedRecords.findById(c.suggested_record_id);
    ...
    const sug2 = db.caraSuggestedRecords.findById(c.suggested_record_id);
