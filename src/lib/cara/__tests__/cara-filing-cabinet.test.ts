...
} from "@/lib/cara/cara-filing-cabinet";

function seed(overrides: Partial<CaraArtifact> = {}): CaraArtifact {
  ...
  return db.caraArtifacts.create(base);
}

describe("Cara Studio — filing cabinet writeback", () => {
  it("buildCaraFilingPath uses scope, category, year, month, type and title", () => {
    const art = seed({
      child_id: "yp_001",
    ...
    const path = buildCaraFilingPath(art, "daily_care");
  ...
  it("re-filing the same artifact is idempotent (upsert by care_event_id+category)", () => {
    ...
    const refreshed = db.caraArtifacts.findById(art.id)!;
  ...
  it("audit trail records the commit with the filing path", () => {
    ...
    const events = db.caraStudioAuditLog
      .findAll()
