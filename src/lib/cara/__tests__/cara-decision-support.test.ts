// ══════════════════════════════════════════════════════════════════════════════
// Cara Decision Support engine tests
// ══════════════════════════════════════════════════════════════════════════════
...
describe("runDecisionSupport", () => {
  beforeEach(() => {
    ...
    for (const f of db.caraFormulations.findAll(HOME_ID)) {
      if (f.status !== "rejected") {
        db.caraFormulations.patch(f.id, { status: "rejected" });
      }
    }
    for (const r of db.caraDecisionRecommendations.findOpen(HOME_ID)) {
      db.caraDecisionRecommendations.patch(r.id, { status: "rejected" });
    }
  });
  ...
  it("is idempotent: rerunning supersedes the prior formulation and patches the prior recommendation", () => {
    ...
    const active = db.caraFormulations
