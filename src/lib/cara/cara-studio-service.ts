...
import { generateCaraStudioContent, getCaraStudioProviderConfig } from "./cara-studio-provider";
import { buildArtifactPrompt } from "./cara-studio-generators";
import { gatherSourcesForRequest } from "./cara-studio-sources";
...
function buildSystemPrompt(framework: CaraFramework, tone: CaraTone): string {
  return `${CARA_PROFESSIONAL_IDENTITY_PROMPT}
...
export async function generateArtifact(
...
): Promise<CaraGenerationResult> {
  const config = getCaraStudioProviderConfig();
  ...
  const structuredContent: CaraStructuredContent = {
    sections: [
      {
        id: "main",
        title: request.title,
  ...
  db.caraArtifactVersions.create({
...
export function submitArtifactForReview(
  ...
  const updated = db.caraArtifacts.patch(artifactId, {
    status: "in_review",
...
export function commitArtifact(
  ...
  const latestQc = db.caraQualityChecks.findLatestByArtifact(artifactId);
  if (latestQc && !latestQc.overall_passed) return null;

  const now = new Date().toISOString();
  const committed = db.caraArtifacts.patch(artifactId, {
    status: "committed",
...
export function editArtifact(
  ...
  const prevVersions = db.caraArtifactVersions.findByArtifact(artifactId);
...
function detectRegulationRelevance(artifactType: CaraArtifactType): Record<string, string> {
...
function detectSafeguardingLevel(
  ...
  const highTypes: CaraArtifactType[] = ["safeguarding_review", "risk_review", "incident_learning_review"];
