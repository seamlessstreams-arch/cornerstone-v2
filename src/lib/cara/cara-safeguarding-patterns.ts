...
//   - CaraSafeguardingPattern records (the detected pattern with evidence)
//   - CaraEarlyWarning records for the highest-severity patterns
//
...
function severityToWarning(severity: CaraPatternSeverity): boolean {
...
export function runSafeguardingScan(
  ...
  const persistedPatterns: CaraSafeguardingPattern[] = [];
  const persistedWarnings: CaraEarlyWarning[] = [];
  const detectedAt = new Date().toISOString();

  for (const p of detected) {
    const existing = db.caraSafeguardingPatterns
      .findOpen(homeId)
      .find(
        (e) => e.pattern_type === p.pattern_type && e.child_id === p.child_id,
      );

    let pattern: CaraSafeguardingPattern;
    if (existing) {
      const updated = db.caraSafeguardingPatterns.patch(existing.id, {
        title: p.title,
      ...
      pattern = db.caraSafeguardingPatterns.create({
        home_id: homeId,
        child_id: p.child_id,
    ...
    if (severityToWarning(p.severity)) {
      const existingWarning = db.caraEarlyWarnings
        .findActive(homeId)
        .find(
          (w) =>
      ...
      if (!existingWarning) {
        const w = db.caraEarlyWarnings.create({
          home_id: homeId,
          child_id: p.child_id,
