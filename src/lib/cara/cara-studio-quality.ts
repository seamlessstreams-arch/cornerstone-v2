...
interface QualityCheckResult {
  check: Omit<CaraQualityCheck, "id" | "created_at">;
  issues: string[];
}

export function runQualityCheck(artifact: CaraArtifact): CaraQualityCheck {
  const result = evaluateArtifact(artifact);

  // Persist the check
  const check = db.caraQualityChecks.create(result.check);

  // Update artifact with quality score and pass/fail
  const score = calculateScore(result.check);
  db.caraArtifacts.patch(artifact.id, {
    quality_score: score,
...
function evaluateArtifact(artifact: CaraArtifact): QualityCheckResult {
...
function calculateScore(check: Omit<CaraQualityCheck, "id" | "created_at">): number {
  const booleanChecks = [
