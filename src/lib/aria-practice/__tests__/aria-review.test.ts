...
import { validateReview, buildFlagResolution, buildAssessmentDecision, buildThresholdDecision } from "../aria-review";
...
describe("store integration — flags resolve, never delete", () => {
  it("resolving a flag persists resolution + keeps the record on file", () => {
    const flag = db.ariaPracticeFlags.create({
    ...
    db.ariaPracticeFlags.patch(flag.id, buildFlagResolution("staff_darren", "Child made safe; plan in place.", NOW));
    const after = db.ariaPracticeFlags.findById(flag.id);
