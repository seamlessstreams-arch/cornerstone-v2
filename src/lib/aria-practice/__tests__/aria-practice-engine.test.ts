...
import { analyzePractice, ARIA_GUIDANCE_RULES } from "../aria-practice-engine";
import type { AriaPracticeInput, AriaFlagType, PracticeSourceType } from "../types";

const TODAY = "2026-06-07";

function run(text: string, sourceType: PracticeSourceType = "daily_record", extra: Partial<AriaPracticeInput> = {}) {
  return analyzePractice({ text, sourceType, today: TODAY, ...extra });
}
function flagTypes(out: ReturnType<typeof analyzePractice>): AriaFlagType[] {
...
describe("guardrails & scoring", () => {
  ...
  it("ships the 10 seed guidance rules", () => {
    expect(ARIA_GUIDANCE_RULES.length).toBe(10);
    expect(ARIA_GUIDANCE_RULES.every((r) => r.rule_key && r.active)).toBe(true);
    expect(new Set(ARIA_GUIDANCE_RULES.map((r) => r.rule_key)).size).toBe(10);
  });
});
