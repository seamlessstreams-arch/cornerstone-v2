...
import { buildDraftScaffold, ARIA_DRAFT_DISCLAIMER, type AriaDraftType } from "../aria-draft";
import type { PracticeSourceType } from "../types";

const TODAY = "2026-06-07";
function build(draftType: AriaDraftType, content: string, sourceType: PracticeSourceType = "daily_record") {
...
describe("Cara draft scaffold — common contract", () => {
  ...
  it("every draft type produces editable, deterministic, disclaimed sections", () => {
    for (const t of types) {
      ...
      expect(r.disclaimer).toBe(ARIA_DRAFT_DISCLAIMER);
