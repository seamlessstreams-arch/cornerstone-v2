...
// The route layers role-gating (aria.approve_outputs) on top of this.
// ══════════════════════════════════════════════════════════════════════════════

import type { AriaPracticeFlag, AriaPracticeAssessment, AriaThresholdConsultation } from "./types";
...
export function buildFlagResolution(userId: string, rationale: string, now: string): Partial<AriaPracticeFlag> {
  return { resolved: true, resolved_at: now, resolved_by: userId, resolution_rationale: rationale.trim() };
}

export function buildAssessmentDecision(userId: string, decision: string, rationale: string, now: string): Partial<AriaPracticeAssessment> {
...
export function buildThresholdDecision(decision: string, rationale: string): Partial<AriaThresholdConsultation> {
