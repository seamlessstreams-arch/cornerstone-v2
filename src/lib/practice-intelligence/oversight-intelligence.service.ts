...
// approve, and commit. ARIA drafts — managers decide.
// ══════════════════════════════════════════════════════════════════════════════
...
export async function generateOversightDraft(opts: {
  ...
  if (sb && opts.recordId) {
    ...
    const { data: source } = await (sb.from("aria_studio_sources") as any)
      .select("*")
      .eq("id", opts.recordId)
      .maybeSingle();
  ...
  if (sb && opts.childId) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: recentSources } = await (sb.from("aria_studio_sources") as any)
...
function getDefaultOversightContent(oversightType: OversightType): OversightContent {
  return {
    summary: `${OVERSIGHT_TYPE_LABELS[oversightType]} — draft oversight comment generated by ARIA.`,
  ...
  return [
    {
      ...
      content: {
        ...
        human_review_note: "ARIA-generated draft. Registered Manager to review, personalise, and approve before committing to the record.",
      },
      evidence_links: [],
      regulatory_refs: [
