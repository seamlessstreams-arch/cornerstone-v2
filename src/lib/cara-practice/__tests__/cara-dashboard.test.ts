...
import { buildPracticeDashboard, type PracticeDashboardInput } from "../cara-dashboard";
import type { CaraPracticeFlag, CaraStaffWellbeingSignal, CaraThresholdConsultation, CaraSeverity, CaraFlagType } from "../types";

let n = 0;
function flag(flag_type: CaraFlagType, over: Partial<CaraPracticeFlag> = {}): CaraPracticeFlag {
  return {
    id: `f_${++n}`, tenant_id: null, child_id: "yp_alex", staff_id: null, home_id: "home_oak",
    source_type: "daily_record", source_id: null, flag_type, severity: "medium" as CaraSeverity,
...
describe("buildPracticeDashboard", () => {
  ...
  it("includes open threshold consultations in the watchlist", () => {
    const consult: CaraThresholdConsultation = {
  ...
  it("restricts wellbeing signals by role", () => {
    const w: CaraStaffWellbeingSignal = {
