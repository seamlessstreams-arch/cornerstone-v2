...
export interface PracticeDashboardInput {
  flags: CaraPracticeFlag[];
  thresholdConsultations: CaraThresholdConsultation[];
  wellbeingSignals: CaraStaffWellbeingSignal[];
  developmentalGaps: CaraDevelopmentalGapRecord[];
  protectiveFactorReviews: CaraProtectiveFactorReview[];
  relationshipDepthReviews: CaraRelationshipDepthReview[];
  assessments: CaraPracticeAssessment[];
...
export interface PracticeDashboard {
  ...
  openFlags: CaraPracticeFlag[];
  developmentalGapHeatmap: {
    byChild: Record<string, Record<string, number>>;
    byDomain: Record<string, number>;
  };
  practiceDriftWarnings: CaraPracticeFlag[];
  protectiveFactorWeaknesses: Array<{ source: "flag" | "review"; childId: string | null; description: string; severity: CaraSeverity }>;
  relationshipDepthMap: Array<{ childId: string | null; stageOrFlag: string; detail: string }>;
  thresholdWatchlist: Array<{ id: string; childId: string | null; kind: string; severity: CaraSeverity; title: string; strategyDiscussion?: boolean; lado?: boolean }>;
  staffWellbeingSignals: CaraStaffWellbeingSignal[];
...
function sev(count: number): CaraSeverity {
