...
import type {
  ...
  AriaStudioSafeguardingPatternType,
...
const PATTERN_KEYWORDS: Record<AriaStudioSafeguardingPatternType, string[]> = {
...
const HIGH_SEVERITY_PATTERNS: AriaStudioSafeguardingPatternType[] = [
  "exploitation_indicator", "self_harm_escalation", "allegation_pattern", "missing_episode_escalation",
];

export async function scanSafeguardingPatterns(hId: string, childId?: string): Promise<AriaStudioSafeguardingPattern[]> {
  ...
  const patterns: AriaStudioSafeguardingPattern[] = [];
  ...
  for (const [key, data] of Object.entries(patternCounts)) {
    ...
    const isHigh = HIGH_SEVERITY_PATTERNS.includes(patternType as AriaStudioSafeguardingPatternType);
    ...
    patterns.push(buildPattern({
      ...
      pattern_type: patternType as AriaStudioSafeguardingPatternType,
      risk_level: severity,
  ...
  if (patterns.length > 0) {
    ...
    if (insertErr) console.error("[aria-studio/safeguarding] Insert error:", insertErr);
  }

  return patterns;
}

export async function listSafeguardingPatterns(hId: string, childId?: string, status?: string): Promise<AriaStudioSafeguardingPattern[]> {
  const sb = createServerClient();
  if (!sb) return getDemoPatterns();
  ...
  if (error) { console.error("[aria-studio/safeguarding] List error:", error); return []; }
  return (data ?? []) as AriaStudioSafeguardingPattern[];
}
...
export async function reviewSafeguardingPattern(patternId: string, _reviewedBy: string, status: "acknowledged" | "escalated" | "dismissed"): Promise<boolean> {
  ...
  if (error) { console.error("[aria-studio/safeguarding] Review error:", error); return false; }
  return true;
}

function buildPattern(partial: Partial<AriaStudioSafeguardingPattern> & { home_id: string; pattern_type: AriaStudioSafeguardingPatternType; risk_level: string; title: string }): AriaStudioSafeguardingPattern {
...
function getDemoPatterns(): AriaStudioSafeguardingPattern[] {
  return [
