...
import type { AriaStudioHomeDynamics } from "@/types/aria-studio";

function homeId(): string {
  return process.env.SUPABASE_HOME_ID ?? "a0000000-0000-0000-0000-000000000001";
}

export async function generateHomeDynamicsSnapshot(hId: string, snapshotDate?: string): Promise<AriaStudioHomeDynamics> {
  ...
  if (error) { console.error("[aria-studio/home-dynamics] Snapshot error:", error); return getDemoSnapshot(hId, date); }
  return data as AriaStudioHomeDynamics;
}

export async function getLatestSnapshot(hId: string): Promise<AriaStudioHomeDynamics | null> {
  const sb = createServerClient();
...
export async function listSnapshots(hId: string, days: number = 30): Promise<AriaStudioHomeDynamics[]> {
  const sb = createServerClient();
  if (!sb) return [getDemoSnapshot(hId, new Date().toISOString().slice(0, 10))];
  ...
  if (error) { console.error("[aria-studio/home-dynamics] List error:", error); return []; }
  return (data ?? []) as AriaStudioHomeDynamics[];
}
...
function getDemoSnapshot(hId: string, date: string): AriaStudioHomeDynamics {
  return {
