...
export async function generate(request: GenerationRequest): Promise<GenerationResult> {
  ...
  let profile: AriaChildProfile | undefined;
  if (request.childId) {
    try {
      ...
      console.error("[aria-studio/generator] Profile build failed:", err);
  ...
  try {
    ...
    console.error("[aria-studio/generator] LLM call failed:", err);
    return {
      success: false,
      safety: preSafety,
      profile,
