...
export async function indexSource(source: Partial<AriaStudioSource>): Promise<AriaStudioSource | null> {
  ...
  if (error) {
    console.error("[aria-studio] Failed to index source:", error);
    return null;
  }
  return data;
}
...
export async function getSource(id: string): Promise<AriaStudioSource | null> {
  const sb = createServerClient();
  if (!sb) return null;
...
export async function searchSources(hid: string, query: string): Promise<AriaStudioSource[]> {
  return listSources(hid, { search: query });
}

function getDemoSources(): AriaStudioSource[] {
