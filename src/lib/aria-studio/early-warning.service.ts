...
export async function runEarlyWarningChecks(hId: string, childId?: string): Promise<AriaStudioEarlyWarning[]> {
  const sb = createServerClient();
  if (!sb) return getDemoWarnings();

  const warnings: AriaStudioEarlyWarning[] = [];
  ...
  for (const check of WARNING_CHECKS) {
    try {
      ...
      console.error(`[aria-studio/early-warning] Check "${check.type}" failed:`, err);
    }
  }
  ...
  if (warnings.length > 0) {
    ...
    if (error) console.error("[aria-studio/early-warning] Insert error:", error);
  }

  return warnings;
}

export async function listEarlyWarnings(hId: string, childId?: string, status?: string): Promise<AriaStudioEarlyWarning[]> {
  const sb = createServerClient();
  if (!sb) return getDemoWarnings();
  ...
  if (error) { console.error("[aria-studio/early-warning] List error:", error); return []; }
  return (data ?? []) as AriaStudioEarlyWarning[];
}
...
export async function reviewEarlyWarning(warningId: string, status: "acknowledged" | "escalated" | "dismissed"): Promise<boolean> {
  ...
  if (error) { console.error("[aria-studio/early-warning] Review error:", error); return false; }
  return true;
}
...
function getDemoWarnings(): AriaStudioEarlyWarning[] {
  const now = new Date().toISOString();
  return [
