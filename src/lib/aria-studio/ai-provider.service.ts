// ══════════════════════════════════════════════════════════════════════════════
// ARIA STUDIO — AI PROVIDER ABSTRACTION
//
...
export async function generateStudioContent(
  ...
  if (config.configured && config.provider === "openai") {
    try {
      ...
      console.error("[aria-studio] OpenAI generation failed:", err);
  ...
  if (config.configured && config.provider === "anthropic") {
    try {
      ...
      console.error("[aria-studio] Anthropic generation failed:", err);
      return generateStubContent(systemPrompt, userPrompt);
    }
  }
...
function generateStubContent(_system: string, userPrompt: string): StudioAIResponse {
  ...
  return {
    content: `⚠️ DEMO MODE — This content was generated by the ARIA Studio stub provider. Configure AI_PROVIDER and the appropriate API key for live generation.\n\n${content}`,
    model: "stub",
    provider: "stub",
  };
}
...
ARIA has identified potential gaps in the evidence base. The manager should review and address these before the Reg 45 report is finalised.
...
## Human Review Required
This is an ARIA-generated draft. It must be reviewed, amended where necessary, and approved by an authorised person before it becomes part of the official record.`,
...
## Possible Indicators
ARIA has identified patterns that may require professional review. These are presented as possible indicators, not conclusions.
