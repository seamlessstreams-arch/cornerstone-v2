...
describe("ARIA_STUDIO_SYSTEM_PROMPT", () => {
  it("exists and is non-empty", () => {
    expect(ARIA_STUDIO_SYSTEM_PROMPT).toBeDefined();
    expect(ARIA_STUDIO_SYSTEM_PROMPT.length).toBeGreaterThan(100);
  });

  it("includes human review requirement", () => {
    const lower = ARIA_STUDIO_SYSTEM_PROMPT.toLowerCase();
