...
import type { AriaStudioContradiction } from "@/types/aria-studio";
...
export async function detectContradictions(
  hId: string,
  childId?: string,
): Promise<AriaStudioContradiction[]> {
  ...
  const contradictions: AriaStudioContradiction[] = [];
  ...
  if (contradictions.length > 0) {
    ...
    if (insertErr) console.error("[aria-studio/contradiction] Insert error:", insertErr);
  }
...
export async function listContradictions(hId: string, childId?: string, status?: string): Promise<AriaStudioContradiction[]> {
  const sb = createServerClient();
  if (!sb) return getDemoContradictions();
  ...
  if (error) { console.error("[aria-studio/contradiction] List error:", error); return []; }
  return (data ?? []) as AriaStudioContradiction[];
}
...
export async function resolveContradiction(contradictionId: string, reviewedBy: string, resolution: string): Promise<boolean> {
  ...
  if (error) { console.error("[aria-studio/contradiction] Resolve error:", error); return false; }
  return true;
}

// ── Builder ─────────────────────────────────────────────────────────────────

function buildContradiction(partial: Partial<AriaStudioContradiction> & { home_id: string; contradiction_type: string; severity: string }): AriaStudioContradiction {
...
function getDemoContradictions(): AriaStudioContradiction[] {
  return [
