...
import type {
  AriaStudioCareGraphNode,
  AriaStudioCareGraphEdge,
  AriaStudioNodeType,
...
export async function upsertGraphNode(
  node: Omit<AriaStudioCareGraphNode, "id" | "created_at" | "updated_at"> & { id?: string },
): Promise<AriaStudioCareGraphNode | null> {
  ...
  if (error) {
    console.error("[aria-studio/care-graph] upsertGraphNode error:", error);
    return null;
  }
  return data as AriaStudioCareGraphNode;
}

// ── Create edge ─────────────────────────────────────────────────────────────

export async function createGraphEdge(
  edge: Omit<AriaStudioCareGraphEdge, "id" | "created_at" | "updated_at">,
): Promise<AriaStudioCareGraphEdge | null> {
  const sb = createServerClient();
  if (!sb) return null;
  ...
  if (error) {
    console.error("[aria-studio/care-graph] createGraphEdge error:", error);
    return null;
  }
  return data as AriaStudioCareGraphEdge;
}

// ── Query: get nodes by type ────────────────────────────────────────────────

export async function getNodesByType(
  nodeType: AriaStudioNodeType,
  childId?: string,
): Promise<AriaStudioCareGraphNode[]> {
  const sb = createServerClient();
  ...
  if (error) {
    console.error("[aria-studio/care-graph] getNodesByType error:", error);
    return [];
  }
  return (data ?? []) as AriaStudioCareGraphNode[];
}
...
export async function getEdgesForNode(
...
): Promise<AriaStudioCareGraphEdge[]> {
  const sb = createServerClient();
  if (!sb) return [];
  ...
  return [
    ...((fromData ?? []) as AriaStudioCareGraphEdge[]),
    ...((toData ?? []) as AriaStudioCareGraphEdge[]),
  ];
}
...
export interface ChildKnowledgeGraph {
  childNode: AriaStudioCareGraphNode | null;
  nodes: AriaStudioCareGraphNode[];
  edges: AriaStudioCareGraphEdge[];
}
...
export async function getChildKnowledgeGraph(childId: string): Promise<ChildKnowledgeGraph> {
  ...
  const childNode = ((childNodes as AriaStudioCareGraphNode[] | null)?.[0]) ?? null;
  ...
  let nodes: AriaStudioCareGraphNode[] = [childNode];
  if (relatedNodeIds.size > 0) {
    ...
    nodes = [childNode, ...((relatedNodes ?? []) as AriaStudioCareGraphNode[])];
  }
...
export async function findConnections(nodeAId: string, nodeBId: string): Promise<AriaStudioCareGraphEdge[]> {
  const sb = createServerClient();
  if (!sb) return [];
...
function mapSourceTypeToNodeType(sourceType: string): AriaStudioNodeType {
  const map: Record<string, AriaStudioNodeType> = {
...
function getDemoNodes(nodeType: AriaStudioNodeType): AriaStudioCareGraphNode[] {
...
function getDemoChildGraph(childId: string): ChildKnowledgeGraph {
  ...
  const edges: AriaStudioCareGraphEdge[] = [
