...
import type { AriaStudioFormulation } from "@/types/aria-studio";
...
export async function createFormulation(
  formulation: Omit<AriaStudioFormulation, "id" | "created_at" | "approved_at"> & { id?: string },
): Promise<AriaStudioFormulation | null> {
  ...
  if (error) { console.error("[aria-studio/formulation] Create error:", error); return null; }
  return data as AriaStudioFormulation;
}

export async function getFormulationForChild(childId: string): Promise<AriaStudioFormulation | null> {
  const sb = createServerClient();
...
export async function listFormulations(hId: string, childId?: string): Promise<AriaStudioFormulation[]> {
  const sb = createServerClient();
  ...
  if (error) { console.error("[aria-studio/formulation] List error:", error); return []; }
  return (data ?? []) as AriaStudioFormulation[];
}

export async function updateFormulation(formulationId: string, updates: Partial<AriaStudioFormulation>): Promise<AriaStudioFormulation | null> {
  const sb = createServerClient();
  if (!sb) return null;
  ...
  if (error) { console.error("[aria-studio/formulation] Update error:", error); return null; }
  return data as AriaStudioFormulation;
}
...
export async function approveFormulation(formulationId: string, approvedBy: string): Promise<boolean> {
  ...
  if (error) { console.error("[aria-studio/formulation] Approve error:", error); return false; }
  return true;
}

function getDemoFormulation(childId: string): AriaStudioFormulation {
