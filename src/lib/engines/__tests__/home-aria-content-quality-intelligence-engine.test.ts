...
import {
  computeAriaContentQuality,
  type AriaArtifactInput,
  type AriaContentQualityInput,
} from "../home-aria-content-quality-intelligence-engine";

// ── Helpers ─────────────────────────────────────────────────────────────────

const TODAY = "2026-05-28";

function makeArtifact(overrides: Partial<AriaArtifactInput> = {}): AriaArtifactInput {
  return {
    id: "art_1",
...
function baseInput(overrides: Partial<AriaContentQualityInput> = {}): AriaContentQualityInput {
  return {
    today: TODAY,
    total_staff: 8,
    total_children: 4,
...
describe("Home ARIA Content Quality Intelligence Engine", () => {
  describe("structure and shape", () => {
    ...
    it("recommendations have rank, recommendation, and urgency", () => {
      const r = computeAriaContentQuality(baseInput({
    ...
    it("total_artifacts matches input length", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: arts }));
  ...
  describe("special cases", () => {
    it("0 artifacts, 0 staff, 0 children → insufficient_data", () => {
      const r = computeAriaContentQuality(baseInput({ total_staff: 0, total_children: 0, artifacts: [] }));
    ...
    it("0 artifacts with staff present → good rating with score 72", () => {
      const r = computeAriaContentQuality(baseInput({ total_staff: 5, total_children: 3, artifacts: [] }));
    ...
    it("0 artifacts with only staff (no children) → good rating with score 72", () => {
      const r = computeAriaContentQuality(baseInput({ total_staff: 5, total_children: 0, artifacts: [] }));
      expect(r.content_rating).toBe("good");
      expect(r.content_score).toBe(72);
    });

    it("0 artifacts with only children (no staff) → good rating with score 72", () => {
      const r = computeAriaContentQuality(baseInput({ total_staff: 0, total_children: 3, artifacts: [] }));
      expect(r.content_rating).toBe("good");
      expect(r.content_score).toBe(72);
    });
  ...
  describe("rating bands", () => {
    it("outstanding: score >= 80", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 4, artifacts: arts }));
  ...
  describe("boundary conditions", () => {
    ...
    it("score cannot exceed 100", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 5, artifacts: arts }));
      expect(r.content_score).toBeLessThanOrEqual(100);
    });
    ...
    it("approval_rate boundary at exactly 90%", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...approved, ...submitted] }));
    ...
    it("framework_usage_rate boundary at exactly 80%", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...withFw, ...without] }));
    ...
    it("evidence_sourced_rate boundary at exactly 90%", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...sourced, ...unsourced] }));
  ...
  describe("metric calculations", () => {
    ...
    it("child_coverage_rate is 0 when total_children is 0", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 0, artifacts: arts }));
      expect(r.child_coverage_rate).toBe(0);
    });
    ...
    it("child_coverage_rate can exceed 100 if more unique children than total_children (defensive)", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 2, artifacts: arts }));
  ...
  describe("scoring bonuses", () => {
    it("quality_score >= 80 adds +5", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: arts, total_children: 0 }));
    ...
    it("approval_rate >= 90 adds +5", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: approved, total_children: 0 }));
    ...
    it("approval_rate >= 75 but < 90 adds +3", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...approved, ...rejected], total_children: 0 }));
      expect(r.approval_rate).toBe(75);
    });
    ...
    it("framework_usage >= 50% but < 80% adds +1", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...withFw, ...without], total_children: 0 }));
      expect(r.framework_usage_rate).toBe(50);
    });
    ...
    it("evidence_sourced >= 70% but < 90% adds +1", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...sourced, ...unsourced], total_children: 0 }));
      expect(r.evidence_sourced_rate).toBe(70);
    });
  ...
  describe("strengths generation", () => {
    ...
    it("includes strength for approval rate >= 75%", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ artifacts: [...approved, ...other] }));
      expect(r.strengths.some(s => s.includes("approval rate"))).toBe(true);
    });
    ...
    it("no strengths generated for poor metrics", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 10, artifacts: arts }));
      expect(r.strengths).toHaveLength(0);
    });
  });
  ...
  describe("determinism", () => {
    it("same input always produces same output", () => {
      ...
      const r1 = computeAriaContentQuality(input);
      const r2 = computeAriaContentQuality(input);
      expect(r1).toEqual(r2);
    });

    it("running multiple times gives identical scores", () => {
      const input = baseInput({ artifacts: [makeArtifact()] });
      const scores = Array.from({ length: 5 }, () => computeAriaContentQuality(input).content_score);
      expect(new Set(scores).size).toBe(1);
    });

    it("running multiple times gives identical ratings", () => {
      const input = baseInput({ artifacts: [makeArtifact()] });
      const ratings = Array.from({ length: 5 }, () => computeAriaContentQuality(input).content_rating);
  ...
  describe("large input handling", () => {
    ...
    it("handles 500 artifacts with mixed properties", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 20, artifacts: arts }));
  ...
  describe("edge cases", () => {
    ...
    it("total_staff 0 with artifacts still computes", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_staff: 0, total_children: 4, artifacts: arts }));
      expect(r.total_artifacts).toBe(1);
  ...
  describe("mixed scenario integration tests", () => {
    ...
    it("struggling home with low metrics", () => {
      ...
      const r = computeAriaContentQuality(baseInput({ total_children: 6, artifacts: arts }));
      expect(r.content_rating).toBe("inadequate");
  ...
  describe("regression and safety", () => {
    it("function is exported as computeAriaContentQuality", () => {
      expect(typeof computeAriaContentQuality).toBe("function");
    });
    ...
    it("does not mutate input", () => {
      ...
      computeAriaContentQuality(input);
    ...
    it("rating is always a valid string", () => {
      ...
      scenarios.forEach(input => {
        const r = computeAriaContentQuality(input);
