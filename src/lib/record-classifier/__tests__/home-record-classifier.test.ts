...
describe("Home Record Classifier", () => {
  describe("fire safety", () => {
    it("detects a fire drill", () => {
      const r = classifyHomeRecord("Completed the monthly fire drill. All residents evacuated within 3 minutes.", "Oak House");
      expect(r.primary_type).toBe("fire_drill");
      expect(r.tags).toContain("fire-safety");
    });
    it("flags a fire hazard for immediate action", () => {
      const r = classifyHomeRecord("Fire door faulty on the first floor landing and the fire exit blocked by boxes.", "Oak House");
  ...
  describe("health & safety", () => {
    it("detects an H&S check", () => {
      const r = classifyHomeRecord("Carried out the weekly health and safety check. Legionella water temperatures recorded.", "Oak House");
      expect(r.primary_type).toBe("health_safety_check");
      expect(r.tags).toContain("legionella");
    });
    it("flags a serious safety hazard", () => {
      const r = classifyHomeRecord("Possible gas leak detected in the kitchen during the health and safety check.", "Oak House");
  ...
  describe("maintenance", () => {
    it("detects a repair need", () => {
      const r = classifyHomeRecord("The washing machine is broken and not working. Needs repairing urgently.", "Oak House");
    ...
    it("detects damp and mould", () => {
      const r = classifyHomeRecord("Damp and mould appearing in the downstairs bathroom ceiling.", "Oak House");
      expect(r.primary_type).toBe("maintenance_request");
  ...
  describe("vehicle", () => {
    it("detects a vehicle check", () => {
      const r = classifyHomeRecord("Completed the minibus vehicle check. MOT due next month, tyres fine.", "Oak House");
  ...
  describe("audit", () => {
    it("detects a home audit", () => {
      const r = classifyHomeRecord("Manager's monthly walk-round audit of the premises completed.", "Oak House");
      expect(r.primary_type).toBe("home_audit");
      expect(r.tags).toContain("compliance");
    });
    it("detects reg 44 visit", () => {
      const r = classifyHomeRecord("Regulation 44 visit carried out by the independent visitor.", "Oak House");
      expect(r.primary_type).toBe("home_audit");
    });
  });
  ...
  describe("default + confidence", () => {
    it("defaults to a home note for general text", () => {
      const r = classifyHomeRecord("Held a house meeting with the young people about weekend activities.", "Oak House");
      expect(r.primary_type).toBe("observation");
    });
    it("short text has low confidence", () => {
      const r = classifyHomeRecord("All fine today.", "Oak House");
  ...
  describe("flags & titles", () => {
    it("flags expiring compliance", () => {
      const r = classifyHomeRecord("Gas safety certificate is expiring and overdue for renewal.", "Oak House");
      expect(r.flags.some((f) => f.type === "compliance")).toBe(true);
    });
    it("flags a passed check as positive", () => {
      const r = classifyHomeRecord("Fire alarm test completed and all systems compliant and satisfactory.", "Oak House");
    ...
    it("flows to home record and compliance", () => {
      const r = classifyHomeRecord("Health and safety check completed", "Oak House");
