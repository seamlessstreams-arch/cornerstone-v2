...
import type {
  AriaCareGraphNode,
  AriaCareGraphEdge,
  AriaCareGraphSnapshot,
  AriaCareGraphNodeType,
  AriaCareGraphEdgeType,
  AriaPatternSeverity,
...
function severityFromIncident(s: string): AriaPatternSeverity {
  if (s === "critical" || s === "high" || s === "medium" || s === "low") return s;
  return "low";
}

function severityFromRisk(level: string): AriaPatternSeverity {
  switch (level) {
...
export function buildCareGraph(homeId: string, options: BuildOptions = {}): AriaCareGraphSnapshot {
  ...
  const sgps = db.ariaSafeguardingPatterns.findOpen(homeId);
  for (const p of sgps) {
  ...
  const warnings = db.ariaEarlyWarnings.findActive(homeId);
  ...
  if (persist) {
    ...
    const existingNodes = db.ariaCareGraphNodes.findAll(homeId).filter((n) => {
    ...
    db.ariaCareGraphEdges.deleteByNodeIds(existingNodeIds);
    db.ariaCareGraphNodes.deleteByHome(homeId, childIdFilter ?? undefined);
  }

  // Materialise nodes
  const keyToId = new Map<string, string>();
  const persistedNodes: AriaCareGraphNode[] = [];
  for (const draft of draftNodes.values()) {
    if (persist) {
      const created = db.ariaCareGraphNodes.create({
        home_id: homeId,
      ...
      const synthetic: AriaCareGraphNode = {
...
export function loadCareGraph(homeId: string, childId?: string | null): AriaCareGraphSnapshot {
  const allNodes = db.ariaCareGraphNodes.findAll(homeId).filter((n) => {
    if (!childId) return true;
    return n.child_id === childId || n.child_id === null;
  });
  const nodeIds = new Set(allNodes.map((n) => n.id));
  const allEdges = db.ariaCareGraphEdges.findAll(homeId).filter(
