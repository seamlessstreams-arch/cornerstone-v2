...
export function lookupLearnedAnswer(args: {
  ...
  const bucket = db.ariaResponseCache.findByBucket(args.commandId, args.childId ?? null);
  ...
  db.ariaResponseCache.recordHit(best.id);
  hits++;
...
export function getLearnedCacheStats(): {
  ...
  const entries = db.ariaResponseCache.findAll();
