...
export function scanReg40Candidates(homeId: string): AriaReg40Triage[] {
  ...
  for (const e of events) {
    const existing = db.ariaReg40Triages.findBySourceEvent(e.id);
    if (existing) continue;
    const cat = suggestCategory(e);
    const triage = db.ariaReg40Triages.create({
      home_id: e.home_id,
      child_id: e.child_id,
...
export function loadReg40Queue(
  ...
  const rows = db.ariaReg40Triages.findAll(homeId);
...
export type Reg40DecisionError =
  | { code: "not_found" }
  | { code: "not_pending"; record: AriaReg40Triage }
...
export function decideReg40Triage(
...
): AriaReg40Triage | Reg40DecisionError {
  const existing = db.ariaReg40Triages.findById(input.triageId);
  ...
  const patched = db.ariaReg40Triages.patch(input.triageId, {
    status,
