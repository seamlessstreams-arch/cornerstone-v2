...
export type AmendCommittedError =
  | { code: "not_found" }
  | { code: "not_current"; record: AriaCommittedRecord }
...
export function amendCommittedRecord(
  ...
  const previous = db.ariaCommittedRecords.findById(input.recordId);
  ...
  const current = db.ariaCommittedRecords.create({
  ...
  if (previous.suggested_record_id) {
    db.ariaSuggestedRecords.patch(previous.suggested_record_id, {
...
export function loadCommittedVersionHistory(
  ...
  const seed = db.ariaCommittedRecords.findById(recordId);
  if (!seed) return [];
