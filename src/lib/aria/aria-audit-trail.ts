...
export interface AppendAriaAuditInput {
  homeId: string;
  actorId: string;
  actionType: AriaAuditAction;
 ...
 */
export function appendAriaAudit(input: AppendAriaAuditInput): AriaStudioAuditLog | null {
  try {
    return db.ariaStudioAuditLog.create({
...
export function loadAuditTrail(
  ...
  let rows = db.ariaStudioAuditLog.findAll(homeId);
...
export function loadAuditActors(homeId: string): string[] {
  const rows = db.ariaStudioAuditLog.findAll(homeId);
 ...
 */
export const ARIA_AUDIT_ACTION_LABELS: Record<AriaAuditAction, string> = {
