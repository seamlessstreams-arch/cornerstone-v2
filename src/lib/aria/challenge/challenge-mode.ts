...
import { challengeOutputSchema } from "@/lib/aria/ai/schemas";
...
export async function runChallengeMode(
  ...
  if (sb) {
    try {
      ...
      console.error("[aria-challenge] AI deep analysis failed:", err);
...
export async function generateSuggestedActions(
  ...
  if (sb) {
    try {
      ...
      console.error("[aria-challenge] AI action generation failed:", err);
    }
  }
...
async function generateAIActions(
  ...
  const response = await generateAriaContent({
    systemPrompt:
