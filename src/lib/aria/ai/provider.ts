...
//   4. Returns a structured AriaAIResponse
//
// Also provides generateAriaJSON<T>() for structured output with Zod
...
export async function generateAriaContent(request: AriaAIRequest): Promise<AriaAIResponse> {
  const fullSystemPrompt = `${ARIA_SYSTEM_PREAMBLE}\n\n${request.systemPrompt}`;
  ...
  try {
    ...
    console.error("[aria-reports] AI generation failed:", err);
...
// ── generateAriaJSON ───────────────────────────────────────────────────────
// Calls generateAriaContent, parses the result as JSON, and validates against
...
export async function generateAriaJSON<T>(
  request: AriaAIRequest,
  schema: z.ZodSchema<T>,
): Promise<{ data: T | null; raw: string; error?: string }> {
  const response = await generateAriaContent(request);
