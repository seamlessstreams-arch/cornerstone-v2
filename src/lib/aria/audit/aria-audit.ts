...
//
// Safety: writeAriaAudit never throws — a failed audit write must not break
...
import type { AriaAuditEvent } from "@/types/aria-reports";

// ── Write Audit Event ─────────────────────────────────────────────────────

export async function writeAriaAudit(event: {
  ...
  try {
    ...
    console.error("[aria-audit] Failed to write audit event:", err);
  }
}
...
export async function getAuditTrail(opts: {
  ...
  if (error) {
    console.error("[aria-audit] Failed to fetch audit trail:", error);
    return [];
  }

  return (data ?? []) as AriaAuditEvent[];
}

// ── Demo Data ─────────────────────────────────────────────────────────────

function getDemoAuditTrail(homeId: string): AriaAuditEvent[] {
