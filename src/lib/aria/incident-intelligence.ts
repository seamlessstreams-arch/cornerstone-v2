...
import { runAriaIntelligence } from "./engine";
import type { AriaOutput } from "./types";
...
type NextAction = {
  ...
  rationale: string;
  source: "deterministic" | "pattern" | "aria";
};
...
export async function runIncidentIntelligence(
  ...
  // Permission check
  if (!hasIntelligencePermission(input.actorRole, "askAria")) {
      ...
      const result = await runAriaIntelligence(
        {
          homeId: input.homeId,
          childId: input.childId,
          roleMode: mapRoleToAriaMode(input.actorRole),
...
function mapRoleToAriaMode(role: string): "practitioner" | "senior" | "deputy_manager" | "registered_manager" | "responsible_individual" | "operations" | "director" | "commissioner" | "ofsted_mock" {
  ...
  return (mapping[role] ?? "practitioner") as ReturnType<typeof mapRoleToAriaMode>;
}
