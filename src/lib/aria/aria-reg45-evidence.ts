...
import { ARIA_REG45_THEME_LABELS } from "@/types/aria-studio";
...
function emptyThemeMap(): Record<AriaReg45Theme, AriaReg45EvidenceItem[]> {
  const out = {} as Record<AriaReg45Theme, AriaReg45EvidenceItem[]>;
  for (const t of Object.keys(ARIA_REG45_THEME_LABELS) as AriaReg45Theme[]) {
    out[t] = [];
  }
  return out;
}

function emptyByTheme(): Record<AriaReg45Theme, number> {
  const out = {} as Record<AriaReg45Theme, number>;
  for (const t of Object.keys(ARIA_REG45_THEME_LABELS) as AriaReg45Theme[]) {
    out[t] = 0;
  }
  return out;
...
function collectSafeguardingPatterns(homeId: string, start: string, end: string): DraftItem[] {
  return db.ariaSafeguardingPatterns
    .findAll(homeId)
...
function collectRecommendations(homeId: string, start: string, end: string): DraftItem[] {
  return db.ariaDecisionRecommendations
    .findAll(homeId)
    .filter(
      (r) =>
...
function upsertEvidenceItem(
  ...
  const existing = db.ariaReg45EvidenceItems.findBySource(homeId, draft.source_table, draft.source_id);

  if (existing) {
    // Refresh draft fields but never silently flip a manager decision.
    const patched = db.ariaReg45EvidenceItems.patch(existing.id, {
      title: draft.title,
  ...
  return db.ariaReg45EvidenceItems.create({
    home_id: homeId,
...
function buildSnapshot(
  ...
  for (const t of Object.keys(themes) as AriaReg45Theme[]) {
...
export function runReg45EvidenceBuild(homeId: string, options: RunOptions = {}): AriaReg45Snapshot {
  ...
  const items = db.ariaReg45EvidenceItems
    .findAll(homeId)
