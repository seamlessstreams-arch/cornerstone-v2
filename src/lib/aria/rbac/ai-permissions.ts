...
const ROLE_TASK_PERMISSIONS: Record<AriaRole, Partial<Record<AriaTaskType, PermissionLevel>>> = {
  support_worker: {
 ...
 * Throws AriaPermissionDeniedError if not permitted.
 */
export function validateRolePermission(role: AriaRole, taskType: AriaTaskType): void {
  const permissions = ROLE_TASK_PERMISSIONS[role];
  if (!permissions) {
    throw new AriaPermissionDeniedError(role, `access AI task '${taskType}'`);
  }

  const level = permissions[taskType];
  if (level === "none" || level === undefined) {
    throw new AriaPermissionDeniedError(role, `use AI for '${taskType}'`);
  }
}
 ...
 */
export function canApprove(role: AriaRole, taskType: AriaTaskType): boolean {
  const permissions = ROLE_TASK_PERMISSIONS[role];
 ...
 */
export function getMaxApprovalRisk(role: AriaRole): AriaRiskLevel {
  switch (role) {
 ...
 */
export function getAccessibleTasks(role: AriaRole): AriaTaskType[] {
  const permissions = ROLE_TASK_PERMISSIONS[role];
  if (!permissions) return [];
  return Object.entries(permissions)
    .filter(([_, level]) => level !== "none")
    .map(([task]) => task as AriaTaskType);
}

/**
 * Get the permission level for a specific role+task combination.
 */
export function getPermissionLevel(role: AriaRole, taskType: AriaTaskType): PermissionLevel {
  const permissions = ROLE_TASK_PERMISSIONS[role];
