...
export function loadOversightQueue(homeId: string): ManagerOversightQueue {
  ...
  for (const r of pending) {
    ...
    items.push({
      ...
      summary: `${ARIA_SUGGESTED_RECORD_LABELS[r.record_type]} awaiting commit decision`,
  ...
  const committed = db.ariaCommittedRecords.findAll(homeId);
  ...
  const recentlyReturned = db.ariaSuggestedRecords
    .findAll(homeId)
    .filter(
      (r) =>
        r.status === "rejected" &&
