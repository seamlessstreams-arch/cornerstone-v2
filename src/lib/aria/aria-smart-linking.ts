...
export interface RecordAriaUsage {
  outputId: string;
  outputLabel: string;
  commandId: string;
  generatedAt: string;
...
const ARIA_FLAGGABLE_TABLES = new Set([
  "daily_log_entries",
  "incidents",
  "key_work_sessions",
  "care_forms",
...
export async function writeSmartLinks(
  ...
  if (error) {
    console.warn("[aria-smart-linking] writeSmartLinks failed:", error.message);
    return { written: 0 };
  }
 ...
 */
export async function flagRecordAsAriaAssisted(
  sourceTable: string,
  sourceRecordId: string,
): Promise<boolean> {
  if (!isSupabaseEnabled()) return false;
  if (!ARIA_FLAGGABLE_TABLES.has(sourceTable)) return false;
  ...
  if (error) {
    console.warn(`[aria-smart-linking] flagRecord failed on ${sourceTable}:`, error.message);
    return false;
  }
...
export async function linkCommittedOutput(
  ...
  await flagRecordAsAriaAssisted(committedRecordType, committedRecordId);
}
