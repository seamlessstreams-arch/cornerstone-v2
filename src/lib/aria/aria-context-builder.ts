...
export interface AriaContextRequest {
  actor: AriaActor;
  homeId?: string;
  childId?: string;
  staffId?: string;
...
export interface AriaContextRecord {
  sourceTable: string;
  sourceRecordId: string;
  summary: string;
  date?: string;
  ...
  incident: [
    {
      ...
      requiredPermission: "aria.view_sensitive_context",
    },
  ],
  key_work: [
    {
...
export async function buildAriaContext(
  request: AriaContextRequest,
): Promise<AriaContextResult> {
  const empty: AriaContextResult = {
    contextSnippet: "",
    records: [],
  ...
  const allRecords: AriaContextRecord[] = [];
...
function buildContextSnippet(records: AriaContextRecord[]): string {
  ...
  const grouped = new Map<string, AriaContextRecord[]>();
