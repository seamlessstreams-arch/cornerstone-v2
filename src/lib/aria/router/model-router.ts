...
export class AriaModelRouter {
   ...
   */
  async executeTask(input: AriaTaskRequest): Promise<AriaTaskResult> {
    ...
    if (decision.blocked) {
      throw new AriaRoutingBlockedError(decision.blockReason ?? "Task routing blocked");
    }
    ...
    let output = "";
    let tokenUsage: AriaTokenUsage = { promptTokens: 0, completionTokens: 0, totalTokens: 0 };
    ...
    const approvalStatus: AriaApprovalStatus = decision.requiresApproval
      ? "draft_ai_generated"
   ...
   */
  explainRouting(decision: AriaRouteDecision): string {
    const lines: string[] = [
  ...
  private determineRiskLevel(
    input: AriaTaskRequest,
    sensitivity: AriaDataSensitivity,
  ): AriaRiskLevel {
  ...
  private selectProvider(
    ...
    provider: AriaProviderName | null;
    model: string | null;
    fallbacks: AriaProviderName[];
    blocked: boolean;
    blockReason?: string;
  } {
  ...
  private estimateLatency(provider: AriaProviderName): number {
    const estimates: Partial<Record<AriaProviderName, number>> = {
      openai: 2000,
      anthropic: 3000,
      azure_openai: 2500,
      mistral: 1500,
