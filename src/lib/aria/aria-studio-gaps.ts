...
import type { AriaGenerationRequest, AriaGap, AriaGapType } from "@/types/aria-studio";
...
export async function detectGapsForRequest(
...
): Promise<Omit<AriaGap, "id" | "created_at">[]> {
  const gaps: Omit<AriaGap, "id" | "created_at">[] = [];
  ...
  if (request.artifact_type === "management_oversight" || !childId) {
    try {
      const recentArtifacts = db.ariaArtifacts.findAll(homeId).filter(
...
export async function detectAllGaps(homeId: string): Promise<Omit<AriaGap, "id" | "created_at">[]> {
  const gaps: Omit<AriaGap, "id" | "created_at">[] = [];

  // Run checks for the home as a whole
  const fakeRequest: AriaGenerationRequest = {
