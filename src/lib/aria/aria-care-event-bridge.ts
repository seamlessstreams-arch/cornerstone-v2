 ...
 */
const RECORDS_BY_CATEGORY: Partial<Record<CareEventCategory, AriaSuggestedRecordType[]>> = {
...
function draftTitle(event: CareEvent, recordType: AriaSuggestedRecordType): string {
  switch (recordType) {
...
): AriaSuggestedRecord | undefined {
  return db.ariaSuggestedRecords.findAll(homeId).find(
    (r) =>
...
export interface BridgeResult {
  careEventId: string;
  proposed: AriaSuggestedRecord[];
  reused: AriaSuggestedRecord[];
  skipped: boolean;
  reason?: string;
}
...
export function proposeRecordsFromCareEvent(
  ...
  const sourceRef: AriaSuggestedSourceRef = {
    type: "care_event",
    id: event.id,
    label: `Care event – ${event.title}`,
  };

  const proposed: AriaSuggestedRecord[] = [];
  const reused: AriaSuggestedRecord[] = [];
