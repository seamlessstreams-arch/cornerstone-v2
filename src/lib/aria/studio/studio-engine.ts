...
import type {
  AriaStudioRequest,
  AriaStudioOutput,
  AriaStudioSessionType,
  AriaTaskRequest,
  AriaTaskResult,
} from "../core/types";
import { AriaModelRouter } from "../router/model-router";

// ── AriaStudioEngine ──────────────────────────────────────────────────────

export class AriaStudioEngine {
  private router: AriaModelRouter;

  constructor(router?: AriaModelRouter) {
    this.router = router ?? new AriaModelRouter();
  }

  /**
   * Generate a structured session plan or resource.
   */
  async generate(request: AriaStudioRequest): Promise<AriaStudioOutput> {
  ...
  private buildPrompt(request: AriaStudioRequest): string {
    const parts: string[] = [];
  ...
  private mapSessionTypeToTaskType(type: AriaStudioSessionType): any {
    switch (type) {
  ...
  private parseStudioOutput(result: AriaTaskResult, type: AriaStudioSessionType): AriaStudioOutput {
...
const SESSION_TYPE_GUIDANCE: Partial<Record<AriaStudioSessionType, string>> = {
  keywork_session:
...
function getSessionTypeLabel(type: AriaStudioSessionType): string {
  const labels: Record<AriaStudioSessionType, string> = {
...
// Singleton
export const ariaStudioEngine = new AriaStudioEngine();
