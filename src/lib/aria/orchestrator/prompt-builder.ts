...
import { ARIA_SYSTEM_PREAMBLE } from "../ai/safety";
import { WRITING_STYLE_RULES } from "../writing/humanised-writing";
import type { AriaRequest, EvidenceItem, RiskLevel, RouteDecision } from "./types";
...
function buildContextSection(request: AriaRequest): string {
  const parts: string[] = [];
...
export function buildUserPrompt(request: AriaRequest): string {
  const parts: string[] = [];
