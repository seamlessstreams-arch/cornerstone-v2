...
import { retrieveAriaEvidence } from "../evidence";
import { generateAriaJson } from "../provider";
...
import type { AriaRequest, AriaResponse, EvidenceItem } from "./types";
...
export async function orchestrate(request: AriaRequest): Promise<AriaResponse> {
  ...
  if (routeDecision.requiresRAG) {
    try {
      const rawEvidence = await retrieveAriaEvidence({
      ...
      console.error("[aria-orchestrator] Evidence retrieval failed:", err);
  ...
  try {
    ...
    const result = await generateAriaJson({
    ...
    // Extract the text response — generateAriaJson returns parsed JSON or raw text
    ...
    console.error("[aria-orchestrator] Model invocation failed:", err);
  ...
  if (costEntry) {
    persistCost({
      ...
      console.error("[aria-orchestrator] Cost persistence failed:", err);
    });
  }
...
async function writeAuditEntry(input: {
  ...
  const demoId = `aria-orch-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  try {
    ...
    if (error || !data) {
      console.error("[aria-orchestrator] Audit write failed:", error?.message);
      return demoId;
    }

    return data.id;
  } catch (err) {
    console.error("[aria-orchestrator] Audit write exception:", err);
    return demoId;
  }
}
...
export type { AriaRequest, AriaResponse } from "./types";
