...
import {
  ARIA_ANNEX_A_SECTIONS,
  type AriaAnnexAReadiness,
  type AriaAnnexASectionKey,
  type AriaAnnexASectionReading,
  type AriaAnnexASnapshot,
...
function readinessFromCounts(recordCount: number, gapCount: number, isRequired: boolean): AriaAnnexAReadiness {
...
function readinessScore(sections: AriaAnnexASectionReading[]): number {
  let total = 0;
  let earned = 0;
...
function overallReadiness(score: number): AriaAnnexAReadiness {
...
function buildSection1(homeId: string): AriaAnnexASectionReading {
...
function buildSection2(homeId: string): AriaAnnexASectionReading {
...
function buildSection3(homeId: string, asOf: string): AriaAnnexASectionReading {
...
function buildSection4(homeId: string, periodStart: string, periodEnd: string): AriaAnnexASectionReading {
...
function buildSection5(homeId: string, periodStart: string, periodEnd: string): AriaAnnexASectionReading {
...
function buildSection6(homeId: string, periodStart: string, periodEnd: string): AriaAnnexASectionReading {
  const childIds = new Set(
    db.youngPeople.findAll().filter((y) => y.home_id === homeId).map((y) => y.id),
  );
...
function buildSection7(homeId: string, periodStart: string, periodEnd: string): AriaAnnexASectionReading {
  const childIds = new Set(
    db.youngPeople.findAll().filter((y) => y.home_id === homeId).map((y) => y.id),
  );
...
function buildSection8(homeId: string, periodStart: string, periodEnd: string, asOf: string): AriaAnnexASectionReading {
  ...
  let readiness: AriaAnnexAReadiness;
...
function buildSection9(homeId: string, periodStart: string, periodEnd: string): AriaAnnexASectionReading {
  const reg45Items = db.ariaReg45EvidenceItems
    .findAll(homeId)
    .filter(
      (e) =>
...
const SECTION_BUILDERS: Record<
  AriaAnnexASectionKey,
  (homeId: string, periodStart: string, periodEnd: string, asOf: string) => AriaAnnexASectionReading
> = {
...
export function buildAnnexASnapshotData(
  ...
  const sections = ARIA_ANNEX_A_SECTIONS.map(({ key }) =>
...
export function runAnnexASnapshot(
  ...
  const existing = db.ariaAnnexASnapshots
    .findAll(homeId)
    .find(
      (s) =>
        s.status === "draft" &&
  ...
  if (existing) {
    const patched = db.ariaAnnexASnapshots.patch(existing.id, {
  ...
  return db.ariaAnnexASnapshots.create({
    home_id: homeId,
...
export function lockAnnexASnapshot(
  ...
  const snap = db.ariaAnnexASnapshots.findById(id);
  if (!snap) return null;
  if (snap.status === "locked") return snap; // already locked, immutable
  return db.ariaAnnexASnapshots.patch(id, {
    status: "locked",
    locked_by: lockedBy,
...
export function loadAnnexASnapshots(homeId: string): AriaAnnexASnapshot[] {
  return [...db.ariaAnnexASnapshots.findAll(homeId)].sort((a, b) =>
