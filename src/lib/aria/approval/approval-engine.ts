...
import { AriaPermissionDeniedError } from "../core/errors";
import { canApprove, getMaxApprovalRisk } from "../rbac/ai-permissions";

// ── AriaApprovalEngine ────────────────────────────────────────────────────

export class AriaApprovalEngine {
  private approvals: Map<string, AriaApprovalRecord> = new Map();
  ...
  createApprovalRecord(
    ...
    const record: AriaApprovalRecord = {
   ...
   */
  submitForReview(approvalId: string, submittedBy: string): AriaApprovalRecord {
  ...
  approve(
    ...
    if (!canApprove(reviewerRole, record.taskType)) {
      throw new AriaPermissionDeniedError(reviewerRole, `approve '${record.taskType}'`);
    }
    ...
    if (!canApproveRiskLevel(maxRisk, record.riskLevel)) {
      throw new AriaPermissionDeniedError(
        reviewerRole,
  ...
  reject(
    ...
    if (!canApprove(reviewerRole, record.taskType)) {
      throw new AriaPermissionDeniedError(reviewerRole, `review '${record.taskType}'`);
    }
   ...
   */
  archive(approvalId: string): AriaApprovalRecord {
    const record = this.getRecord(approvalId);
   ...
   */
  requiresApproval(taskType: AriaTaskType): boolean {
   ...
   * Get a record by ID.
   */
  getRecord(approvalId: string): AriaApprovalRecord {
...
const RISK_ORDER: AriaRiskLevel[] = ["low", "medium", "high", "critical"];

function canApproveRiskLevel(maxRisk: AriaRiskLevel, targetRisk: AriaRiskLevel): boolean {
  return RISK_ORDER.indexOf(maxRisk) >= RISK_ORDER.indexOf(targetRisk);
}

// Singleton
export const ariaApprovalEngine = new AriaApprovalEngine();
