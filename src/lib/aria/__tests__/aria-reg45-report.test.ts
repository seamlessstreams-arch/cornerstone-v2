...
function seedAcceptedEvidence(
  theme: AriaReg45EvidenceItem["theme"],
  sentiment: AriaReg45EvidenceItem["sentiment"],
  title: string,
...
function rejectAllEvidence() {
  for (const e of db.ariaReg45EvidenceItems.findAll(HOME_ID)) {
    if (e.status !== "rejected") {
      db.ariaReg45EvidenceItems.patch(e.id, { status: "rejected" });
    }
  }
}
...
describe("Reg 45 Report Builder", () => {
  ...
  it("locking promotes linked evidence chips to included_in_report", () => {
    ...
    const after = db.ariaReg45EvidenceItems.findById(ev.id);
