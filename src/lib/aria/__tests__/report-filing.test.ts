...
// ══════════════════════════════════════════════════════════════════════════════
// ARIA — REPORT FILING INTEGRATION TESTS
// ══════════════════════════════════════════════════════════════════════════════
...
import type { ChildReport } from "@/types/aria-reports";
...
describe("buildReportFilingPath", () => {
  it("builds correct path for weekly child report", () => {
    ...
    expect(path).toBe("young-people/child-jayden/aria-reports/weekly/2026-05-11");
  });
...
describe("fileLockedReport (demo mode)", () => {
  ...
  it("demo filing path includes aria-reports", async () => {
    const result = await fileLockedReport("rpt-demo-2");
    expect(result.filingPath).toContain("aria-reports");
  });
});
