...
import { detectGapsForRequest } from "@/lib/aria/aria-studio-gaps";
import { gatherSourcesForRequest } from "@/lib/aria/aria-studio-sources";
import { generateAriaStudioContent } from "@/lib/aria/aria-studio-provider";

// ── Helpers ───────────────────────────────────────────────────────────────────

function makeArtifact(overrides: Partial<AriaArtifact> = {}): AriaArtifact {
...
function seedArtifact(artifact: AriaArtifact): AriaArtifact {
  return db.ariaArtifacts.create(artifact);
}
...
describe("ARIA Studio — editing and versioning", () => {
  it("creates a version snapshot on edit", () => {
    ...
    const versions = db.ariaArtifactVersions.findByArtifact(art.id);
...
describe("ARIA Studio — quality checks", () => {
  ...
  it("quality check score is between 0 and 100", () => {
    ...
    const result = runQualityCheck(art);
    const updated = db.ariaArtifacts.findById(art.id);
    expect(updated!.quality_score).toBeGreaterThanOrEqual(0);
    expect(updated!.quality_score).toBeLessThanOrEqual(100);
  });

  it("persists quality check to ariaQualityChecks store", () => {
    const art = seedArtifact(makeArtifact({ status: "approved" }));
    runQualityCheck(art);
    const checks = db.ariaQualityChecks.findByArtifact(art.id);
  ...
  it("writes audit log entry on quality check", () => {
    const art = seedArtifact(makeArtifact({ status: "approved" }));
    runQualityCheck(art);
    const log = db.ariaStudioAuditLog.findByArtifact(art.id);
...
describe("ARIA Studio — store integrity", () => {
  ...
  it("db.ariaArtifacts.stats returns expected shape", () => {
    const stats = db.ariaArtifacts.stats("home_oak");
    expect(typeof stats.total).toBe("number");
    expect(typeof stats.draft).toBe("number");
    expect(typeof stats.committed).toBe("number");
  });

  it("db.ariaArtifacts.findByStatus filters correctly", () => {
    seedArtifact(makeArtifact({ status: "draft", home_id: "home_oak" }));
    const drafts = db.ariaArtifacts.findByStatus("draft", "home_oak");
    expect(drafts.every((a) => a.status === "draft")).toBe(true);
  });

  it("db.ariaArtifacts.findById returns null/undefined for unknown id", () => {
    const result = db.ariaArtifacts.findById("art_nonexistent_xyz");
