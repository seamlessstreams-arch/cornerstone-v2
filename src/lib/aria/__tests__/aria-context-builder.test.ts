...
import { buildAriaContext, _testing } from "../aria-context-builder";
import type { AriaActor } from "../aria-permissions";
...
// ─── buildAriaContext (integration, with Supabase off) ──────────────────────

describe("buildAriaContext", () => {
  it("returns empty context when Supabase is disabled", async () => {
    const result = await buildAriaContext({
      actor: defaultActor,
      homeId: "home_oak",
      childId: "child_1",
...
describe("permission-gated sources", () => {
  it("incident source requires aria.view_sensitive_context", () => {
    const incidentConfig = SOURCE_TABLES.incident[0];
    expect(incidentConfig.requiredPermission).toBe("aria.view_sensitive_context");
  });

  it("HR sources require aria.hr", () => {
    for (const config of SOURCE_TABLES.hr) {
      expect(config.requiredPermission).toBe("aria.hr");
    }
  });

  it("safer_recruitment sources require aria.recruitment", () => {
    for (const config of SOURCE_TABLES.safer_recruitment) {
      expect(config.requiredPermission).toBe("aria.recruitment");
    }
  });

  it("ri_dashboard sources require aria.ri_qa", () => {
    for (const config of SOURCE_TABLES.ri_dashboard) {
      expect(config.requiredPermission).toBe("aria.ri_qa");
    }
  });
