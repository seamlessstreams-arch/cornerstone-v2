...
const { computeDeterministicAlerts, computeNextActions, buildIncidentReviewQuestion, mapRoleToAriaMode } = _testing;
...
describe("computeNextActions", () => {
  ...
  it("all actions have title, ownerRole, priority, rationale and source", () => {
    ...
    for (const action of actions) {
      ...
      expect(["deterministic", "pattern", "aria"]).toContain(action.source);
    }
  });
...
describe("mapRoleToAriaMode", () => {
  it("maps rsw to practitioner", () => {
    expect(mapRoleToAriaMode("rsw")).toBe("practitioner");
  });

  it("maps senior to senior", () => {
    expect(mapRoleToAriaMode("senior")).toBe("senior");
  });

  it("maps registered_manager to registered_manager", () => {
    expect(mapRoleToAriaMode("registered_manager")).toBe("registered_manager");
  });

  it("maps deputy_manager to deputy_manager", () => {
    expect(mapRoleToAriaMode("deputy_manager")).toBe("deputy_manager");
  });

  it("maps ri to responsible_individual", () => {
    expect(mapRoleToAriaMode("ri")).toBe("responsible_individual");
  });

  it("maps operations to operations", () => {
    expect(mapRoleToAriaMode("operations")).toBe("operations");
  });

  it("maps director to director", () => {
    expect(mapRoleToAriaMode("director")).toBe("director");
  });

  it("maps commissioner to commissioner", () => {
    expect(mapRoleToAriaMode("commissioner")).toBe("commissioner");
  });

  it("defaults unknown roles to practitioner", () => {
    expect(mapRoleToAriaMode("unknown_role")).toBe("practitioner");
    expect(mapRoleToAriaMode("")).toBe("practitioner");
    expect(mapRoleToAriaMode("admin")).toBe("practitioner");
  });
});
