...
import {
  ...
  ARIA_SAFETY_RULES,
  ARIA_SYSTEM_PREAMBLE,
  BANNED_PHRASES,
} from "../ai/safety";

describe("ARIA_SAFETY_RULES", () => {
  it("contains exactly 10 safety rules", () => {
    expect(ARIA_SAFETY_RULES).toHaveLength(10);
  });

  it("includes the 'must not diagnose' rule", () => {
    const hasDiagnoseRule = ARIA_SAFETY_RULES.some((r) =>
  ...
  it("includes the 'must not invent evidence' rule", () => {
    const hasEvidenceRule = ARIA_SAFETY_RULES.some((r) =>
  ...
  it("includes the 'must not blame children' rule", () => {
    const hasBlameRule = ARIA_SAFETY_RULES.some((r) =>
...
describe("ARIA_SYSTEM_PREAMBLE", () => {
  ...
  it("requires UK English", () => {
    expect(ARIA_SYSTEM_PREAMBLE).toContain("UK English");
    expect(ARIA_SYSTEM_PREAMBLE).toContain("behaviour");
    expect(ARIA_SYSTEM_PREAMBLE).toContain("organisation");
  });

  it("embeds all safety rules", () => {
    for (const rule of ARIA_SAFETY_RULES) {
      expect(ARIA_SYSTEM_PREAMBLE).toContain(rule);
    }
  });
});
