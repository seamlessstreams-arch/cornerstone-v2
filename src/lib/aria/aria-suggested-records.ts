...
import {
  ...
  type AriaSuggestedRecordStatus,
  type AriaSuggestedRecordType,
  type AriaSuggestedSourceRef,
} from "@/types/aria-studio";

export interface ProposeSuggestedRecordInput {
  homeId: string;
  childId?: string | null;
  recordType: AriaSuggestedRecordType;
  suggestedTitle: string;
  suggestedBody: string;
  suggestedFields?: Record<string, string | number | boolean | null>;
  sourceEvidence?: AriaSuggestedSourceRef[];
  generatedBy: string;
  targetLabel?: string;
}
...
export function proposeSuggestedRecord(
  ...
  return db.ariaSuggestedRecords.create({
    home_id: input.homeId,
    child_id: input.childId ?? null,
    record_type: input.recordType,
    target_label: input.targetLabel ?? ARIA_SUGGESTED_RECORD_LABELS[input.recordType],
...
export function editSuggestedRecord(
  ...
  return db.ariaSuggestedRecords.patch(id, {
...
export interface CommitResult {
  suggestion: AriaSuggestedRecord;
  committed: AriaCommittedRecord;
}
...
export function commitSuggestedRecord(
  ...
  const committed = db.ariaCommittedRecords.create({
...
export function loadSuggestedRecords(
  ...
  const rows = status
    ? db.ariaSuggestedRecords.findByStatus(homeId, status)
    : db.ariaSuggestedRecords.findAll(homeId);
 ...
 */
export const SAFEGUARDING_SENSITIVE_RECORD_TYPES: AriaSuggestedRecordType[] = [
  "risk_update",
  "incident_summary",
  "behaviour_note",
];
