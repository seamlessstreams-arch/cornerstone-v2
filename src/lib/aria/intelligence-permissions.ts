...
export const ARIA_INTELLIGENCE_PERMISSIONS = {
  askAria: ["rsw", "senior", "deputy_manager", "registered_manager", "ri", "operations", "director"],
...
export type AriaIntelligencePermission = keyof typeof ARIA_INTELLIGENCE_PERMISSIONS;
...
export function hasIntelligencePermission(
  userRole: string,
  permission: AriaIntelligencePermission,
): boolean {
  const allowedRoles = ARIA_INTELLIGENCE_PERMISSIONS[permission] as readonly string[];
