...
function evidenceForPrompt(evidence: Awaited<ReturnType<typeof retrieveAriaEvidence>>) {
...
function buildSystemPrompt(request: AriaRequest) {
  return `
${ARIA_CORE_GUARDRAILS}
${buildHumanWritingInstruction()}
...
export async function runAriaIntelligence(rawRequest: unknown, requestedBy: string): Promise<{
  aiRunId: string;
  output: AriaOutput;
}> {
  const request = AriaRequestSchema.parse(rawRequest);

  const evidence = await retrieveAriaEvidence({
  ...
  const parsed = AriaOutputSchema.parse(raw);
...
export async function approveAriaRun(input: {
  homeId: string;
  aiRunId: string;
  actorId: string;
  notes?: string;
...
export async function rejectAriaRun(input: {
  homeId: string;
  aiRunId: string;
  actorId: string;
  reason: string;
