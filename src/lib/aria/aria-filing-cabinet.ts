...
import type { AriaArtifact, AriaArtifactType } from "@/types/aria-studio";
import type { FilingCategory, FilingCabinetItem } from "@/types/care-events";

// ── Artifact type → filing category mapping ───────────────────────────────────

const ARTIFACT_TO_CATEGORY: Record<AriaArtifactType, FilingCategory> = {
  // Regulatory
...
export function buildAriaFilingPath(artifact: AriaArtifact, category: FilingCategory): string {
...
export function fileCommittedArtifact(artifact: AriaArtifact): FileArtifactResult {
  ...
  const path = buildAriaFilingPath(artifact, category);
