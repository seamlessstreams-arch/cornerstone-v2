...
import { BaseAriaProvider } from "./base-provider";
...
export { BaseAriaProvider } from "./base-provider";
...
const providerInstances: Map<AriaProviderName, BaseAriaProvider> = new Map();

function createProvider(name: AriaProviderName): BaseAriaProvider {
  switch (name) {
 ...
 */
export function getProvider(name: AriaProviderName): BaseAriaProvider {
 ...
 */
export function getAvailableProviders(): BaseAriaProvider[] {
  const allNames: AriaProviderName[] = [
 ...
 */
export async function testProviderConnection(name: AriaProviderName): Promise<{
  available: boolean;
  latencyMs: number;
  error?: string;
}> {
...
class UnavailableProvider extends BaseAriaProvider {
  readonly name: AriaProviderName;
  readonly displayName: string;

  constructor(name: AriaProviderName) {
    super();
    this.name = name;
