...
import type { AriaProviderCapabilities } from "../core/types";
import { AriaProviderError, AriaTimeoutError, AriaRateLimitError } from "../core/errors";
...
export class VoyageProvider extends BaseAriaProvider {
  ...
  getCapabilities(): AriaProviderCapabilities {
    return {
      generateText: false,
  ...
  async generateText(_request: ProviderTextRequest): Promise<ProviderTextResponse> {
    throw new AriaProviderError("Voyage AI does not support text generation", "voyage", false);
  }

  async generateStructured(
    _request: ProviderTextRequest & { schema: Record<string, unknown> },
  ): Promise<ProviderTextResponse> {
    throw new AriaProviderError("Voyage AI does not support structured generation", "voyage", false);
  }

  async *streamText(_request: ProviderTextRequest): AsyncGenerator<ProviderStreamChunk> {
    throw new AriaProviderError("Voyage AI does not support streaming", "voyage", false);
  }
  ...
  private async fetchWithRetry(url: string, init: RequestInit, attempt = 0): Promise<Response> {
    try {
      ...
      if (error?.name === "TimeoutError" || error?.name === "AbortError") throw new AriaTimeoutError("voyage", DEFAULT_TIMEOUT_MS);
      throw new AriaProviderError(`Voyage request failed: ${error?.message}`, "voyage", true);
    }
  }

  private handleAPIError(data: any, status: number): never {
    if (status === 429) throw new AriaRateLimitError("voyage");
    throw new AriaProviderError(`Voyage API error: ${data?.detail ?? "Unknown"}`, "voyage", status >= 500);
  }
}
