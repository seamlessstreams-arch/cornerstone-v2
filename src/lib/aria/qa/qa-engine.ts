...
// ── AriaQualityAssuranceEngine ────────────────────────────────────────────

export class AriaQualityAssuranceEngine {
   ...
   */
  async reviewRecord(request: AriaQAReviewRequest): Promise<AriaQAReviewResult> {
  ...
  private buildQAPrompt(request: AriaQAReviewRequest): string {
    return [
  ...
  private parseQAOutput(result: any, request: AriaQAReviewRequest): AriaQAReviewResult {
    ...
    return {
      ...
      escalationLevel: (parsed.escalationLevel as AriaRiskLevel) ?? "low",
...
// Singleton
export const ariaQAEngine = new AriaQualityAssuranceEngine();
