...
import { reportGenerationRequestSchema, reportOutputSchema } from "@/lib/aria/ai/schemas";
import type { ReportOutput } from "@/lib/aria/ai/schemas";
import { getAgent } from "@/lib/aria/agents/agent-registry";
import { classifyRisk } from "@/lib/aria/risk-tiers";
import {
  retrieveChildProfile,
  retrieveEvidence,
  groupEvidenceByType,
  summariseEvidence,
} from "@/lib/aria/evidence/evidence-retrieval";
import { getSectionsForReportType } from "@/lib/aria/reports/report-templates";
import type { SectionTemplate } from "@/lib/aria/reports/report-templates";
import { buildWritingPrompt, rewriteSection as buildRewritePrompt } from "@/lib/aria/writing/humanised-writing";
import { generateAriaJSON, generateAriaContent } from "@/lib/aria/ai/provider";
import { sanitiseOutput, validateOutputSafety } from "@/lib/aria/ai/safety";
import { writeAriaAudit } from "@/lib/aria/audit/aria-audit";
...
- Return ONLY valid JSON — no markdown fences, no explanation.`;
  ...
  // ── 8. Call AI ──────────────────────────────────────────────────────────
  const aiResult = await generateAriaJSON<ReportOutput>(
  ...
  if (sb) {
    try {
      ...
      console.error("[aria-report-generator] Database persistence failed:", err);
...
export async function updateReportSection(
  ...
  if (error || !data) {
    console.error("[aria-report-generator] Failed to update section:", error);
    return null;
  }
...
export async function rewriteSection(
  ...
  // Call AI
  const aiResponse = await generateAriaContent({
    systemPrompt:
  ...
  if (!safetyResult.safe) {
    console.warn("[aria-report-generator] Rewrite safety warnings:", safetyResult.warnings);
  }
