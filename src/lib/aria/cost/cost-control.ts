...
import { AriaCostLimitError } from "../core/errors";
import { getProvider } from "../providers";

// ── AriaCostControlService ────────────────────────────────────────────────

export class AriaCostControlService {
  private usageLog: AriaCostUsage[] = [];
  ...
  /**
   * Enforce cost limits. Throws AriaCostLimitError if over budget.
   */
  enforceLimits(
    ...
    if (context.userId) {
      ...
      if (userDaily + estimatedCostGBP > this.limits.dailyPerUser) {
        throw new AriaCostLimitError("daily-per-user", userDaily + estimatedCostGBP, this.limits.dailyPerUser);
      }
    }
    ...
    if (context.homeId) {
      ...
      if (homeDaily + estimatedCostGBP > this.limits.dailyPerHome) {
        throw new AriaCostLimitError("daily-per-home", homeDaily + estimatedCostGBP, this.limits.dailyPerHome);
      }
    }
    ...
    if (orgDaily + estimatedCostGBP > this.limits.dailyPerOrganisation) {
      throw new AriaCostLimitError("daily-per-organisation", orgDaily + estimatedCostGBP, this.limits.dailyPerOrganisation);
    }
    ...
    if (orgMonthly + estimatedCostGBP > this.limits.monthlyPerOrganisation) {
      throw new AriaCostLimitError("monthly-per-organisation", orgMonthly + estimatedCostGBP, this.limits.monthlyPerOrganisation);
    }
  }
   ...
   */
  recordUsage(usage: AriaCostUsage): void {
...
function isSimpleTask(taskType: AriaTaskType): boolean {
  return [
    "admin_summary",
    "email_draft",
...
// Singleton
export const ariaCostControl = new AriaCostControlService();
