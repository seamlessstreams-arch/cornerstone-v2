...
import { checkAriaAccess, type AriaActor } from "@/lib/aria/aria-permissions";
import type {
  AriaSuggestion,
  AriaSuggestionLink,
  AriaSuggestionType,
  AriaSuggestionRiskLevel,
  AriaSuggestionConfidence,
  AriaSuggestionStatus,
  AriaSuggestionAuditAction,
  AriaSuggestionAuditEntry,
  IncidentInput,
} from "@/lib/aria/aria-suggestions-types";
...
const SEVERITY_SUGGESTIONS: Record<string, AriaSuggestionType[]> = {
  critical: [
...
function templateFor(type: AriaSuggestionType, severity: string, incidentType: string): SuggestionTemplate {
  const sev = severity.toLowerCase();
  const isHighSeverity = sev === "critical" || sev === "high";

  const templates: Record<AriaSuggestionType, SuggestionTemplate> = {
...
export async function generateIncidentSuggestions(
  input: IncidentInput,
): Promise<{ suggestions: Omit<AriaSuggestion, "id" | "createdAt" | "updatedAt">[]; mockMode: boolean }> {
  ...
  const suggestions: Omit<AriaSuggestion, "id" | "createdAt" | "updatedAt">[] = [];

  for (const type of types) {
    const template = templateFor(type, severity, input.incidentType);

    const linkedRecords: Omit<AriaSuggestionLink, "id" | "suggestionId" | "createdAt">[] = [];
    if (template.linkedRecordType) {
      linkedRecords.push({
        linkedRecordType: template.linkedRecordType as AriaSuggestionLink["linkedRecordType"],
    ...
    suggestions.push({
      ...
      linkedRecords: linkedRecords as AriaSuggestionLink[],
    });
  }
...
export async function generateManagementOversightDraft(
  input: IncidentInput & { existingSuggestions?: AriaSuggestion[] },
  ...
  if (providerConfig.configured) {
    ...
    if (result.llmUsed) {
      return {
        draftText: applyAriaPostprocessor(result.text),
        mockMode: false,
        llmUsed: true,
      };
    }
...
export async function persistSuggestions(
  suggestions: Omit<AriaSuggestion, "id" | "createdAt" | "updatedAt">[],
  actor: AriaActor,
  ...
  for (const s of suggestions) {
    ...
    saved.push({ ...s, id, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() } as AriaSuggestion);
  }
...
export async function getSuggestions(filters: SuggestionFilters): Promise<AriaSuggestion[]> {
  if (!isSupabaseEnabled()) return [];
  const supabaseRaw = createServerClient();
  if (!supabaseRaw) return [];
  const supabase = loose(supabaseRaw);
...
export async function getSuggestionById(id: string, actor: AriaActor): Promise<AriaSuggestion | null> {
...
export async function getAuditTimeline(suggestionId: string): Promise<AriaSuggestionAuditEntry[]> {
  if (!isSupabaseEnabled()) return [];
  const supabaseRaw = createServerClient();
  if (!supabaseRaw) return [];
  const supabase = loose(supabaseRaw);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToSuggestion(row: any): AriaSuggestion {
  return {
    id: row.id,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToLink(row: any): AriaSuggestionLink {
  return {
    id: row.id,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function rowToAudit(row: any): AriaSuggestionAuditEntry {
  return {
    id: row.id,
    organisationId: row.organisation_id ?? undefined,
    homeId: row.home_id ?? undefined,
