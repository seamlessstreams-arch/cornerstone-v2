...
export function StudioQuickActions({
  ...
  return (
    <div className={cn("rounded-xl border border-[var(--cs-aria-gold-soft)] bg-[var(--cs-aria-gold-bg)]/40 overflow-hidden", className)}>
      {/* Header */}
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-2 px-4 py-3 hover:bg-[var(--cs-aria-gold-bg)] transition-colors"
      >
        ...
        <div className="px-3 pb-3 grid grid-cols-2 gap-1.5">
          {actions.map((action) => (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-[var(--cs-aria-gold-soft)] bg-white px-2.5 py-2 text-[11px] font-medium text-[var(--cs-navy)] transition-all hover:border-[var(--cs-aria-gold)] hover:bg-[var(--cs-aria-gold-bg)] hover:shadow-sm"
            >
              <Sparkles className="h-2.5 w-2.5 text-[var(--cs-aria-gold)] shrink-0" />
              {action.label}
            </button>
          ))}
        </div>
