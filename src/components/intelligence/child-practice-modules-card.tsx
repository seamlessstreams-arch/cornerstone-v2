...
// Pure read: each panel calls the module's own per-child hook and deep-links to
// the full module page — it summarises, never duplicates. Empty states are
// neutral (never a false-red): "no data yet" ≠ "a concern".
// ══════════════════════════════════════════════════════════════════════════════

import Link from "next/link";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import {
  Scale, ShieldCheck, Network, RefreshCcw, ChevronRight, Loader2,
} from "lucide-react";
...
/** Highest-severity flag across a set, or null. */
...
/** A small key→value stat chip row. */
...
function RightsRestrictionPanel({ childId }: { childId: string }) {
...
function StayingSafePanel({ childId }: { childId: string }) {
...
function RelationshipsPanel({ childId }: { childId: string }) {
  const { data, isLoading } = useChildRelationships(childId);
  const entries = data?.entries ?? [];
  const a = data?.analysis;
  // Only show the engine's RAG status when relationships have actually been
  // mapped — an unmapped child is "no data", not "fragile" (avoid a false-red).
...
function ReflectionPanel({ childId }: { childId: string }) {
