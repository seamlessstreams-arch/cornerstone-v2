...
import { useAriaActivity } from "@/hooks/use-aria-activity";
import {
  Sparkles,
  CheckCircle2,
  XCircle,
...
interface AriaActivityCardProps {
  homeId?: string;
  days?: number;
  className?: string;
}
...
export function AriaActivityCard({
  ...
  days = 30,
  className,
}: AriaActivityCardProps) {
  const { data: stats, isLoading } = useAriaActivity({ homeId, days });

  if (isLoading) {
    return (
      <div
        className={cn(
          "rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-white p-5",
          className,
        )}
      >
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center gap-2.5 px-5 py-3 bg-gradient-to-r from-[var(--cs-aria-gold-bg)] to-blue-50 border-b border-[var(--cs-aria-gold-soft)]">
            ...
            {stats.topCommands.slice(0, 5).map((cmd) => (
              <div key={cmd.commandId} className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  <div
                    className="h-1.5 rounded-full bg-[var(--cs-aria-gold)]"
                    style={{
                      width: `${Math.max(
                        16,
