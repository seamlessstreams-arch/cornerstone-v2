// ══════════════════════════════════════════════════════════════════════════════
// AriaAssistantPanel — Main AI assistant interface with governance awareness
// ══════════════════════════════════════════════════════════════════════════════

"use client";

import { useState, useRef } from "react";
import type { AriaTaskType, AriaRole, AriaTaskResult, AriaRouteDecision } from "@/lib/aria/core/types";
import { AriaModelDecisionBadge } from "./AriaModelDecisionBadge";
import { AriaHumanApprovalBanner } from "./AriaHumanApprovalBanner";
import { AriaRedactionNotice } from "./AriaRedactionNotice";
import { AriaCostEstimate } from "./AriaCostEstimate";
import { AriaSafetyWarning } from "./AriaSafetyWarning";
...
interface Props {
  ...
  homeId?: string;
  childId?: string;
  defaultTaskType?: AriaTaskType;
  onResult?: (result: AriaTaskResult) => void;
}
...
interface Message {
  ...
  routeDecision?: AriaRouteDecision;
  error?: string;
  timestamp: string;
}

const TASK_TYPE_LABELS: Partial<Record<AriaTaskType, string>> = {
...
export function AriaAssistantPanel({
  userRole,
  organisationId,
  homeId,
  childId,
  ...
  const [taskType, setTaskType] = useState<AriaTaskType>(defaultTaskType ?? "form_prompt_support");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        if (data.code === "ARIA_ROUTING_BLOCKED" || data.code === "ARIA_SAFETY_BLOCK") {
        ...
        const result = data.result as AriaTaskResult;
  ...
  return (
    <div className="flex flex-col h-full rounded-lg border border-border bg-card">
      ...
      <div className="flex items-center justify-between p-3 border-b border-border">
        ...
        <select
          value={taskType}
          onChange={(e) => setTaskType(e.target.value as AriaTaskType)}
