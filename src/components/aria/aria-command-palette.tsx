...
// AriaCommandPanel via a callback.
// ══════════════════════════════════════════════════════════════════════════════
...
interface AriaCommandPaletteProps {
  onSelectCommand?: (commandId: string) => void;
  commands?: PaletteCommand[];
}

export function AriaCommandPalette({
  onSelectCommand,
  commands = DEFAULT_COMMANDS,
}: AriaCommandPaletteProps) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 top-[15%] z-50 mx-auto w-full max-w-lg">
        <div className="rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-white shadow-2xl overflow-hidden">
          {/* Search header */}
          <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--cs-border)]">
            <Sparkles className="h-4 w-4 text-[var(--cs-aria-gold)] shrink-0" />
            <input
              ref={inputRef}
              type="text"
