...
const COMMAND_GROUPS: {
  group: string;
  commands: { id: AriaCommandId; label: string; modules: string[] }[];
}[] = [
  {
    group: "Writing",
    commands: [
...
interface AriaCommandPanelProps {
...
export function AriaCommandPanel({
  module,
  sourceContent,
  childId,
  homeId,
...
}: AriaCommandPanelProps) {
  const aria = useAriaCommand();

  const [collapsed, setCollapsed] = useState(defaultCollapsed);
  const [selectedCommand, setSelectedCommand] = useState<AriaCommandId | "">("");
  ...
  const handleCopy = () => {
    const text = isEditing ? editedText : aria.result?.generatedText;
    if (text) {
  ...
  const handleEdit = () => {
    if (aria.result) {
      setEditedText(aria.result.generatedText);
      setIsEditing(true);
    }
  };

  const handleApprove = async () => {
    if (!aria.result?.outputId) return;
    await aria.decide({
  ...
  const confidenceColor = aria.result?.confidence === "high"
    ? "text-green-700 bg-green-50"
    : aria.result?.confidence === "medium"
      ? "text-amber-700 bg-amber-50"
      : "text-red-700 bg-red-50";

  return (
    <div className={cn(
      "rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-white overflow-hidden",
      className,
    )}>
      {/* Header */}
      <button
        type="button"
        className="flex w-full items-center justify-between px-4 py-3 bg-gradient-to-r from-[var(--cs-aria-gold-bg)] to-blue-50 border-b border-[var(--cs-aria-gold-soft)]"
        ...
        <div className="p-4 space-y-3">
          ...
          {aria.result ? (
            <div className="space-y-3">
              ...
              {aria.result.guardrails?.flagged && (
                <AriaGuardrailBanner
                  flagged={aria.result.guardrails.flagged}
                  mandatoryReview={aria.result.guardrails.mandatoryReview}
                  flags={aria.result.guardrails.flags}
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {aria.result.outputId && aria.result.approvalRequired && (
                  <>
                    <Button
                      size="sm"
                      variant="outline"
              ...
              {aria.result.llmUsed && (
                <div className="text-[9px] text-[var(--cs-text-muted)]">
                  Generated by {aria.result.modelId} via {aria.result.providerId}
                </div>
              )}
              {!aria.result.llmUsed && (
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-[var(--cs-border)] bg-[var(--cs-surface)] pl-7 pr-2 py-1 text-[10px] text-[var(--cs-text-secondary)] focus:outline-none focus:ring-1 focus:ring-[var(--cs-aria-gold)]"
                    />
                  </div>
                </div>
              ...
              {selectedCommand && (
                <>
                  ...
                  {sourceContent && (
                    <div className="rounded-lg bg-[var(--cs-surface)] border border-[var(--cs-border)] px-3 py-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1">
                        ...
                        {childId && (
                          <Badge className="text-[9px] rounded-full bg-[var(--cs-aria-gold-bg)] text-[var(--cs-aria-gold)] border-0">
                            Child context
                          </Badge>
                        )}
                      </div>
                  ...
                  <Button
                    ...
                    disabled={aria.loading || (!inputText.trim() && !sourceContent)}
