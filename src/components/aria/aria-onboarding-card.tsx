...
interface AriaOnboardingCardProps {
  className?: string;
}
...
export function AriaOnboardingCard({ className }: AriaOnboardingCardProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-[var(--cs-aria-gold-soft)] bg-gradient-to-br from-[var(--cs-aria-gold-bg)] via-white to-blue-50 overflow-hidden",
        className,
      )}
    >
      {/* Header */}
      ...
      <div className="px-5 pb-4 grid grid-cols-2 gap-2.5">
        {FEATURES.map((feature) => {
          ...
          return (
            ...
            >
              <Icon className="h-4 w-4 text-[var(--cs-aria-gold)] mb-1.5" />
      ...
      <div className="px-5 pb-4 flex items-center gap-2 flex-wrap">
        ...
        <button
          onClick={handleDismiss}
          className="flex items-center gap-1 text-[10px] text-[var(--cs-text-muted)] hover:text-[var(--cs-aria-gold)] transition-colors"
        >
          Got it, dismiss
