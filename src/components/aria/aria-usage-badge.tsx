...
export function AriaUsageBadge({
  ariaAssisted,
  sourceTable,
  recordId,
  size = "sm",
  ...
  const [usages, setUsages] = useState<AriaUsage[]>([]);
  ...
  const fetchUsage = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/aria/usage?sourceTable=${encodeURIComponent(sourceTable)}&recordId=${encodeURIComponent(recordId)}`,
      );
      if (res.ok) {
    ...
    <div className={cn("inline-flex flex-col", className)}>
      <button
        ...
        className={cn(
          ...
          "border-[var(--cs-aria-gold-soft)] bg-[var(--cs-aria-gold-bg)] hover:bg-[var(--cs-aria-gold-soft)]",
