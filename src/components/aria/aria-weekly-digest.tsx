...
//
// Usage:
//   <AriaWeeklyDigest homeId="demo-home" userId="staff_darren" />
// ══════════════════════════════════════════════════════════════════════════════
...
interface WeeklyDigest {
  ...
  stats: {
    ...
    ariaOutputsGenerated: number;
    ariaOutputsApproved: number;
  };
...
function getDemoDigest(): WeeklyDigest {
  ...
  return {
    ...
    stats: {
      ...
      pendingOversight: 3,
      supervisionsDue: 2,
      ariaOutputsGenerated: 14,
      ariaOutputsApproved: 11,
    },
    topActions: [
      {
        id: "da_001",
...
interface AriaWeeklyDigestProps {
  homeId?: string;
  userId?: string;
  className?: string;
}

export function AriaWeeklyDigest({
  homeId,
  userId,
  className,
}: AriaWeeklyDigestProps) {
  ...
  return (
    <div className={cn("rounded-2xl border border-[var(--cs-border)] bg-white overflow-hidden", className)}>
      {/* Header */}
      <div className="bg-[var(--cs-aria-gold-bg)] px-5 py-3 flex items-center justify-between">
      ...
      <div className="px-5 py-3 grid grid-cols-5 gap-2 border-b border-[var(--cs-border-subtle)]">
        ...
        <MiniStat label="Generated" value={digest.stats.ariaOutputsGenerated} colour="text-[var(--cs-text-secondary)]" />
        <MiniStat label="Approved" value={digest.stats.ariaOutputsApproved} colour="text-emerald-700" />
      </div>
              ...
              <div key={action.id} className="flex items-start gap-2.5">
                ...
                {action.actionUrl && (
                  <Link
                    ...
                    className="text-[var(--cs-text-muted)] hover:text-[var(--cs-aria-gold)] transition-colors shrink-0 mt-0.5"
                  >
