...
interface AriaAuditTimelineProps {
  events: AuditEvent[];
  className?: string;
}

const EVENT_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  generated: { label: "Generated", icon: Sparkles, color: "text-[var(--cs-aria-gold)]" },
...
export function AriaAuditTimeline({ events, className }: AriaAuditTimelineProps) {
  if (events.length === 0) {
    return (
