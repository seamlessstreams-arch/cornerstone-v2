...
import type { AriaSource } from "@/types/aria-studio";
import { ARIA_SOURCE_TYPE_LABELS } from "@/types/aria-studio";
import { formatDistanceToNow } from "date-fns";

interface AriaStudioEvidencePanelProps {
  sources: AriaSource[];
...
export function AriaStudioEvidencePanel({
  sources,
  confidenceScore,
  onAddSource,
}: AriaStudioEvidencePanelProps) {
  return (
            ...
            <div key={source.id} className="border rounded-md p-2.5 space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-[10px] h-4 px-1.5">
                  {ARIA_SOURCE_TYPE_LABELS[source.source_type as keyof typeof ARIA_SOURCE_TYPE_LABELS] ?? source.source_type}
                </Badge>
                <span
