...
import type { AriaRoleMode } from "@/lib/aria/types";

type Props = {
  homeId: string;
  childId?: string | null;
  userId: string;
  defaultRoleMode?: AriaRoleMode;
};

const roleModes: { value: AriaRoleMode; label: string }[] = [
...
export function AriaCommandCentre({
  homeId,
  childId,
  userId,
  ...
  const [roleMode, setRoleMode] = useState<AriaRoleMode>(defaultRoleMode);
  const [includeStaff, setIncludeStaff] = useState(false);
  const { loading, error, output, aiRunId, askAria, reviewAria } = useAria(userId);
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[260px_1fr]">
        <aside className="rounded-2xl bg-slate-50 p-4">
          ...
          <select
            ...
            value={roleMode}
            onChange={(e) => setRoleMode(e.target.value as AriaRoleMode)}
          >
            {roleModes.map((mode) => (
