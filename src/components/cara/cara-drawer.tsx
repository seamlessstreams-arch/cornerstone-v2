...
export interface CaraDrawerContext {
  sourceType?:     CaraSourceType;
  sourceId?:       string;
  childId?:        string;
  childName?:      string;
  staffId?:        string;
...
function getActionsForContext(ctx: CaraDrawerContext): CaraAction[] {
  const base: CaraAction[] = [
    {
      id:       "summarise",
  ...
  const byType: Record<CaraSourceType, CaraAction[]> = {
    incident: [
...
interface CaraDrawerProps {
  open:     boolean;
  onClose:  () => void;
  context?: CaraDrawerContext;
}

export function CaraDrawer({ open, onClose, context = {} }: CaraDrawerProps) {
  ...
  const groupedActions = actions.reduce<Record<string, CaraAction[]>>((acc, a) => {
  ...
  async function runAction(action: CaraAction, customPrompt?: string) {
    abortRef.current?.abort();
...
interface CaraDrawerContextValue {
  openDrawer:  (ctx?: CaraDrawerContext) => void;
  closeDrawer: () => void;
}

export const CaraDrawerCtx = createContext<CaraDrawerContextValue>({
  openDrawer:  () => {},
  closeDrawer: () => {},
});

export function useCaraDrawer() {
  return useContext(CaraDrawerCtx);
}
