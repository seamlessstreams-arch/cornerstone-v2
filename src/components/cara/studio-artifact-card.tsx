...
interface CaraStudioArtifactCardProps {
...
const STATUS_CONFIG: Record<CaraArtifactStatus, {
  color: string;
...
export function CaraStudioArtifactCard({
  artifact,
  onSubmit,
  onArchive,
  onDelete,
  ...
  const typeLabel = CARA_ARTIFACT_TYPE_LABELS[artifact.artifact_type as CaraArtifactType] ?? artifact.artifact_type;
  const statusLabel = CARA_STATUS_LABELS[artifact.status] ?? artifact.status;
