...
import { useCaraStream } from "@/hooks/use-cara";
...
const MODES: { id: CaraMode; label: string; icon: React.ElementType; description: string }[] = [
...
const STYLES: { id: CaraStyle; label: string }[] = [
...
const MODE_COLORS: Partial<Record<CaraMode, string>> & Record<string, string> = {
...
export function CaraPanel({
  ...
  const { stream, isStreaming, abort } = useCaraStream();
...
export function CaraFloatingTrigger({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
