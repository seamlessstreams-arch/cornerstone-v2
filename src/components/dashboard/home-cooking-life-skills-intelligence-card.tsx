...
export function HomeCookingLifeSkillsIntelligenceCard() {
  const { data, isLoading } = useHomeCookingLifeSkillsIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.cooking_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.cooking_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-cyan-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ChefHat className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
            <span className="text-slate-900 font-bold">Cooking & Life Skills</span>
      ...
      <CardContent className="space-y-4">
        {d.cooking_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.cooking_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.independence_rate >= 60 ? "bg-green-50" : d.independence_rate >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.independence_rate >= 60 ? "text-green-600" : d.independence_rate >= 30 ? "text-amber-600" : "text-red-600")}>{d.independence_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_engaged_rate >= 90 ? "bg-green-50" : d.children_engaged_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_engaged_rate >= 90 ? "text-green-600" : d.children_engaged_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.children_engaged_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.recipe_success_rate >= 80 ? "bg-green-50" : d.recipe_success_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.recipe_success_rate >= 80 ? "text-green-600" : d.recipe_success_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.recipe_success_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.category_variety >= 6 ? "bg-green-50" : d.category_variety >= 3 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.category_variety >= 6 ? "text-green-600" : d.category_variety >= 3 ? "text-amber-600" : "text-red-600")}>{d.category_variety}</p>
