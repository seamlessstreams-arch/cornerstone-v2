...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Dumbbell } from "lucide-react";
...
const RATING_STYLES: Record<PhysicalActivityRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePhysicalActivityRecreationIntelligenceCard() {
  const { data, isLoading } = useHomePhysicalActivityRecreationIntelligence();
  ...
  const __emptyState = d.activity_rating === "inadequate" && (d.activity_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      activity_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.activity_rating as PhysicalActivityRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.activity_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-yellow-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Dumbbell className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-yellow-600")} />
        ...
        {d.activity_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.exercise_engagement_rate >= 90 ? "bg-green-50" : d.exercise_engagement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.exercise_engagement_rate >= 90 ? "text-green-600" : d.exercise_engagement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.exercise_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.recreational_diversity_score >= 80 ? "bg-green-50" : d.recreational_diversity_score >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.recreational_diversity_score >= 80 ? "text-green-600" : d.recreational_diversity_score >= 60 ? "text-amber-600" : "text-red-600")}>{d.recreational_diversity_score}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.outdoor_participation_rate >= 90 ? "bg-green-50" : d.outdoor_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.outdoor_participation_rate >= 90 ? "text-green-600" : d.outdoor_participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.outdoor_participation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.fitness_assessment_coverage_rate >= 90 ? "bg-green-50" : d.fitness_assessment_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.fitness_assessment_coverage_rate >= 90 ? "text-green-600" : d.fitness_assessment_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.fitness_assessment_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.activity_accessibility_rate >= 90 ? "bg-green-50" : d.activity_accessibility_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.activity_accessibility_rate >= 90 ? "text-green-600" : d.activity_accessibility_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.activity_accessibility_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_choice_rate >= 80 ? "bg-green-50" : d.child_choice_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_choice_rate >= 80 ? "text-green-600" : d.child_choice_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_choice_rate}%</p>
