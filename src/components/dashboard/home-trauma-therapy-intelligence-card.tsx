...
export function HomeTraumaTherapyIntelligenceCard() {
  const { data, isLoading } = useHomeTraumaTherapyIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.therapy_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.therapy_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-indigo-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartPulse className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
            <span className="text-slate-900 font-bold">Trauma Therapy</span>
      ...
      <CardContent className="space-y-4">
        {d.therapy_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.therapy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_sessions > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_sessions > 0 ? "text-green-600" : "text-amber-600")}>{d.total_sessions}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.attendance_rate >= 85 ? "bg-green-50" : d.attendance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.attendance_rate >= 85 ? "text-green-600" : d.attendance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.mood_improvement_rate >= 70 ? "bg-green-50" : d.mood_improvement_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mood_improvement_rate >= 70 ? "text-green-600" : d.mood_improvement_rate >= 40 ? "text-amber-600" : "text-red-600")}>{d.mood_improvement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.engagement_rate >= 75 ? "bg-green-50" : d.engagement_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.engagement_rate >= 75 ? "text-green-600" : d.engagement_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.modality_diversity >= 4 ? "bg-green-50" : d.modality_diversity >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.modality_diversity >= 4 ? "text-green-600" : d.modality_diversity >= 2 ? "text-amber-600" : "text-red-600")}>{d.modality_diversity}</p>
              <p className="text-[9px] text-muted-foreground">Modalities</p>
            </div>
          </div>
        )}
