...
import { useHomeTeethBrushingOralRoutineIntelligence } from "@/hooks/use-home-teeth-brushing-oral-routine-intelligence";
...
export function HomeTeethBrushingOralRoutineIntelligenceCard() {
  const { data, isLoading } = useHomeTeethBrushingOralRoutineIntelligence();
  ...
  const __emptyState = d.brushing_rating === "inadequate" && (d.brushing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      brushing_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.brushing_rating as TeethBrushingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.brushing_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Teeth Brushing & Oral Routine</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.brushing_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.brushing_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.brushing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.brushing_adherence_rate >= 90 ? "bg-green-50" : d.brushing_adherence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.brushing_adherence_rate >= 90 ? "text-green-600" : d.brushing_adherence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.brushing_adherence_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.fluoride_use_rate >= 90 ? "bg-green-50" : d.fluoride_use_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.fluoride_use_rate >= 90 ? "text-green-600" : d.fluoride_use_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.fluoride_use_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.supervision_rate >= 90 ? "bg-green-50" : d.supervision_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.supervision_rate >= 90 ? "text-green-600" : d.supervision_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.supervision_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.toothbrush_replacement_rate >= 90 ? "bg-green-50" : d.toothbrush_replacement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.toothbrush_replacement_rate >= 90 ? "text-green-600" : d.toothbrush_replacement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.toothbrush_replacement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.independence_rate >= 90 ? "bg-green-50" : d.independence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.independence_rate >= 90 ? "text-green-600" : d.independence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.independence_rate}%</p>
