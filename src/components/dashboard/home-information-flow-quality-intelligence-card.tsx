...
import type { InformationFlowRating } from "@/lib/engines/home-information-flow-quality-intelligence-engine";

const RATING_STYLES: Record<InformationFlowRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeInformationFlowQualityIntelligenceCard() {
  const { data, isLoading } = useHomeInformationFlowQualityIntelligence();
  ...
  const __emptyState = d.flow_rating === "inadequate" && (d.flow_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      flow_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.flow_rating as InformationFlowRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.flow_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-pink-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ArrowRightLeft className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Information Flow</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.flow_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.flow_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.flow_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.handover_completion_rate >= 95 ? "bg-green-50" : d.handover_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.handover_completion_rate >= 95 ? "text-green-600" : d.handover_completion_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.handover_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.daily_log_coverage_rate >= 90 ? "bg-green-50" : d.daily_log_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.daily_log_coverage_rate >= 90 ? "text-green-600" : d.daily_log_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.daily_log_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.significant_event_handover_rate >= 90 ? "bg-green-50" : d.significant_event_handover_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.significant_event_handover_rate >= 90 ? "text-green-600" : d.significant_event_handover_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.significant_event_handover_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.care_event_verification_rate >= 90 ? "bg-green-50" : d.care_event_verification_rate >= 75 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.care_event_verification_rate >= 90 ? "text-green-600" : d.care_event_verification_rate >= 75 ? "text-amber-600" : "text-red-600")}>{d.care_event_verification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.notification_read_rate >= 90 ? "bg-green-50" : d.notification_read_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.notification_read_rate >= 90 ? "text-green-600" : d.notification_read_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.notification_read_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.information_continuity_score >= 80 ? "bg-green-50" : d.information_continuity_score >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.information_continuity_score >= 80 ? "text-green-600" : d.information_continuity_score >= 60 ? "text-amber-600" : "text-red-600")}>{d.information_continuity_score}%</p>
