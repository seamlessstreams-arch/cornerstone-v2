...
import { useHomeWaterSafetyHydrationIntelligence } from "@/hooks/use-home-water-safety-hydration-intelligence";
...
export function HomeWaterSafetyHydrationIntelligenceCard() {
  const { data, isLoading } = useHomeWaterSafetyHydrationIntelligence();
  ...
  const __emptyState = d.water_safety_rating === "inadequate" && (d.water_safety_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      water_safety_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.water_safety_rating as WaterSafetyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.water_safety_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-sky-600 border-2")} aria-label="Cara Water Safety & Hydration Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Droplets className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
        ...
        {d.water_safety_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.temperature_check_rate >= 90 ? "bg-green-50" : d.temperature_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.temperature_check_rate >= 90 ? "text-green-600" : d.temperature_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.temperature_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.legionella_compliance_rate >= 90 ? "bg-green-50" : d.legionella_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.legionella_compliance_rate >= 90 ? "text-green-600" : d.legionella_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.legionella_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hydration_monitoring_rate >= 90 ? "bg-green-50" : d.hydration_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hydration_monitoring_rate >= 90 ? "text-green-600" : d.hydration_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hydration_monitoring_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.swimming_competency_rate >= 90 ? "bg-green-50" : d.swimming_competency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.swimming_competency_rate >= 90 ? "text-green-600" : d.swimming_competency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.swimming_competency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.water_activity_safety_rate >= 90 ? "bg-green-50" : d.water_activity_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.water_activity_safety_rate >= 90 ? "text-green-600" : d.water_activity_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.water_activity_safety_rate}%</p>
