...
export function HomeHolisticChildProgressIntelligenceCard() {
  const { data, isLoading } = useHomeHolisticChildProgressIntelligence();
  ...
  const __emptyState = d.progress_rating === "inadequate" && (d.progress_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.progress_rating as HolisticProgressRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.progress_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-teal-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <TrendingUp className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Child Progress</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.progress_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.progress_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.progress_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_data > 0 ? "bg-blue-50" : "bg-slate-50")}>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.outcome_improvement_rate >= 70 ? "bg-green-50" : d.outcome_improvement_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.outcome_improvement_rate >= 70 ? "text-green-600" : d.outcome_improvement_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.outcome_improvement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.education_engagement_rate >= 90 ? "bg-green-50" : d.education_engagement_rate >= 75 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.education_engagement_rate >= 90 ? "text-green-600" : d.education_engagement_rate >= 75 ? "text-amber-600" : "text-red-600")}>{d.education_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.key_work_completion_rate >= 90 ? "bg-green-50" : d.key_work_completion_rate >= 75 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.key_work_completion_rate >= 90 ? "text-green-600" : d.key_work_completion_rate >= 75 ? "text-amber-600" : "text-red-600")}>{d.key_work_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.domain_coverage >= 5 ? "bg-green-50" : d.domain_coverage >= 3 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.domain_coverage >= 5 ? "text-green-600" : d.domain_coverage >= 3 ? "text-amber-600" : "text-red-600")}>{d.domain_coverage}</p>
