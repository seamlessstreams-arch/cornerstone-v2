...
const DOMAIN_LABELS: Record<string, string> = {
  child_voice: "Voice", participation: "Participation", key_working: "Key Working",
  cultural_identity: "Culture", life_story: "Life Story", therapeutic_climate: "Therapeutic",
  social_connectedness: "Social",
};

export function HomeQualityOfCareCompositeCard() {
  const { data, isLoading } = useHomeQualityOfCareComposite();
  ...
  const d = data?.data;
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.quality_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.quality_rating === "inadequate";
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-cyan-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Star className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
      ...
      <CardContent className="space-y-4">
        {d.quality_rating !== "insufficient_data" && d.domain_scores.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-1.5">
            {d.domain_scores.map(ds => (
              <div key={ds.name} className={cn("text-center rounded-lg p-1.5", ds.quality_met ? "bg-green-50" : "bg-red-50")}>
                <p className={cn("text-sm font-bold tabular-nums", ds.quality_met ? "text-green-600" : "text-red-600")}>{ds.score}/{ds.max}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> ARIA Care Quality Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
