...
// ══════════════════════════════════════════════════════════════════════════════
// CARA — HOME CHILDREN'S RIGHTS & PARTICIPATION INTELLIGENCE CARD
// Rights compliance, child-led meetings, feedback loops, pledges,
...
import type { ChildrensRightsRating } from "@/lib/engines/home-childrens-rights-participation-intelligence-engine";

// ── Style Maps ──────────────────────────────────────────────────────────────

const RATING_STYLES: Record<ChildrensRightsRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeChildrensRightsParticipationIntelligenceCard() {
  const { data, isLoading } = useHomeChildrensRightsParticipationIntelligence();
  ...
  const __emptyState = d.rights_rating === "inadequate" && (d.rights_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.rights_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.rights_rating === "inadequate" || d.rights_compliance.not_met_count >= 3;
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-200")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            {d.rights_rating !== "insufficient_data" && (
              <span className="text-xs font-bold tabular-nums text-slate-600">{d.rights_score}%</span>
            )}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.rights_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        ...
        {d.insights.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-xs font-semibold flex items-center gap-1 text-purple-700">
              ...
              Cara Rights &amp; Participation Intelligence
            </p>
