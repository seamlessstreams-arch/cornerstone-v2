...
import { useHomeSanctionRewardBalanceIntelligence } from "@/hooks/use-home-sanction-reward-balance-intelligence";
...
export function HomeSanctionRewardBalanceIntelligenceCard() {
  const { data, isLoading } = useHomeSanctionRewardBalanceIntelligence();
  ...
  const __emptyState = d.reward_rating === "inadequate" && (d.reward_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.reward_rating as SanctionRewardBalanceRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.reward_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-stone-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-stone-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Scale className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-stone-600")} />
            <span className="text-slate-900 font-bold">Rewards & Sanctions</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.reward_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.reward_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.reward_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.reward_count > d.sanction_count ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reward_count > d.sanction_count ? "text-green-600" : "text-amber-600")}>{d.reward_count}</p>
              <p className="text-[9px] text-muted-foreground">Rewards</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.sanction_count === 0 ? "bg-green-50" : d.sanction_count <= 3 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sanction_count === 0 ? "text-green-600" : d.sanction_count <= 3 ? "text-amber-600" : "text-red-600")}>{d.sanction_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reward_ratio >= 70 ? "bg-green-50" : d.reward_ratio >= 55 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reward_ratio >= 70 ? "text-green-600" : d.reward_ratio >= 55 ? "text-amber-600" : "text-red-600")}>{d.reward_ratio}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.unique_children > 0 ? "bg-blue-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.unique_children > 0 ? "text-blue-600" : "text-slate-600")}>{d.unique_children}</p>
