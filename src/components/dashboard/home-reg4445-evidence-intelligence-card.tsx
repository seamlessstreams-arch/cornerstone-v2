...
export function HomeReg4445EvidenceIntelligenceCard() {
  const { data, isLoading } = useHomeReg4445EvidenceIntelligence();
  ...
  const d = data?.data;
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.reg4445_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.reg4445_rating === "inadequate";
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-200")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ClipboardCheck className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
      ...
      <CardContent className="space-y-4">
        {d.reg4445_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.reg44_visits.visit_completed_rate >= 90 ? "text-green-600" : d.reg44_visits.visit_completed_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.reg44_visits.total_packs > 0 ? `${d.reg44_visits.visit_completed_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Reg 44 Visits</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.reg45.strong_evidence_rate >= 70 ? "text-green-600" : d.reg45.strong_evidence_rate >= 40 ? "text-blue-600" : "text-red-600")}>{d.reg45.total_evidence > 0 ? `${d.reg45.strong_evidence_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Reg 45 Strong</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.reg44_actions.overdue_count === 0 ? "text-green-600" : d.reg44_actions.overdue_count <= 3 ? "text-amber-600" : "text-red-600")}>{d.reg44_actions.overdue_count}</p>
              <p className="text-[10px] text-muted-foreground">Overdue Actions</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.annex_a.gaps_identified === 0 ? "text-green-600" : d.annex_a.gaps_identified <= 3 ? "text-amber-600" : "text-red-600")}>{d.annex_a.gaps_identified}</p>
              <p className="text-[10px] text-muted-foreground">Annex A Gaps</p>
            </div>
          </div>
        )}
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> ARIA Regulatory Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
