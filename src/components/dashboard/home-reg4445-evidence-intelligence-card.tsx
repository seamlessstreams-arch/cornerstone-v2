      ...
      <CardContent className="space-y-4">
        ...
        {d.reg4445_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.reg44_actions.overdue_count === 0 ? "text-[--cs-success]" : d.reg44_actions.overdue_count <= 3 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.reg44_actions.overdue_count}</p>
              <p className="text-[10px] text-muted-foreground">Overdue Actions</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.annex_a.gaps_identified === 0 ? "text-[--cs-success]" : d.annex_a.gaps_identified <= 3 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.annex_a.gaps_identified}</p>
