...
export function HomeEmotionalSafetyClimateIntelligenceCard() {
  const { data, isLoading } = useHomeEmotionalSafetyClimateIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.safety_rating as EmotionalSafetyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.safety_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-blue-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartHandshake className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
        ...
        {d.safety_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_debrief_rate >= 90 ? "bg-green-50" : d.child_debrief_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_debrief_rate >= 90 ? "text-green-600" : d.child_debrief_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_debrief_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reward_to_sanction_ratio >= 3 ? "bg-green-50" : d.reward_to_sanction_ratio >= 1 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reward_to_sanction_ratio >= 3 ? "text-green-600" : d.reward_to_sanction_ratio >= 1 ? "text-amber-600" : "text-red-600")}>{d.reward_to_sanction_ratio}:1</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.de_escalation_attempt_rate >= 100 ? "bg-green-50" : d.de_escalation_attempt_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.de_escalation_attempt_rate >= 100 ? "text-green-600" : d.de_escalation_attempt_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.de_escalation_attempt_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.positive_achievement_count > 0 ? "bg-green-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.positive_achievement_count > 0 ? "text-green-600" : "text-slate-600")}>{d.positive_achievement_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.body_map_completion_rate >= 100 ? "bg-green-50" : d.body_map_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.body_map_completion_rate >= 100 ? "text-green-600" : d.body_map_completion_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.body_map_completion_rate}%</p>
