...
import type { LocalitySafeguardingRating } from "@/lib/engines/home-locality-safeguarding-intelligence-engine";

const RATING_STYLES: Record<LocalitySafeguardingRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeLocalitySafeguardingIntelligenceCard() {
  const { data, isLoading } = useHomeLocalitySafeguardingIntelligence();
  ...
  const __emptyState = d.locality_rating === "inadequate" && (d.locality_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.locality_rating as LocalitySafeguardingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.locality_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-orange-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MapPin className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">Locality Safeguarding</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.locality_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.locality_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.locality_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_risks > 0 ? "bg-amber-50" : "bg-green-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_risks > 0 ? "text-amber-600" : "text-green-600")}>{d.total_risks}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.mitigation_effectiveness >= 80 ? "bg-green-50" : d.mitigation_effectiveness >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mitigation_effectiveness >= 80 ? "text-green-600" : d.mitigation_effectiveness >= 60 ? "text-amber-600" : "text-red-600")}>{d.mitigation_effectiveness}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.review_currency_rate >= 90 ? "bg-green-50" : d.review_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_currency_rate >= 90 ? "text-green-600" : d.review_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.review_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.screening_coverage >= 100 ? "bg-green-50" : d.screening_coverage >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.screening_coverage >= 100 ? "text-green-600" : d.screening_coverage >= 80 ? "text-amber-600" : "text-red-600")}>{d.screening_coverage}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.safety_plan_rate >= 100 ? "bg-green-50" : d.safety_plan_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safety_plan_rate >= 100 ? "text-green-600" : d.safety_plan_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.safety_plan_rate}%</p>
