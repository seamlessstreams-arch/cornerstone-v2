...
import { useHomeWashingMachineDryerMaintenanceIntelligence } from "@/hooks/use-home-washing-machine-dryer-maintenance-intelligence";
...
export function HomeWashingMachineDryerMaintenanceIntelligenceCard() {
  const { data, isLoading } = useHomeWashingMachineDryerMaintenanceIntelligence();
  ...
  const __emptyState = d.appliance_rating === "inadequate" && (d.appliance_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      appliance_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.appliance_rating as WashingMachineDryerRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.appliance_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <WashingMachine className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
            <span className="text-slate-900 font-bold">Washing Machine & Dryer Maintenance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.appliance_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.appliance_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.appliance_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.servicing_rate >= 90 ? "bg-green-50" : d.servicing_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.servicing_rate >= 90 ? "text-green-600" : d.servicing_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.servicing_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.breakdown_response_rate >= 90 ? "bg-green-50" : d.breakdown_response_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.breakdown_response_rate >= 90 ? "text-green-600" : d.breakdown_response_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.breakdown_response_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_access_rate >= 90 ? "bg-green-50" : d.child_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_access_rate >= 90 ? "text-green-600" : d.child_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_access_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hygiene_cycle_rate >= 90 ? "bg-green-50" : d.hygiene_cycle_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hygiene_cycle_rate >= 90 ? "text-green-600" : d.hygiene_cycle_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hygiene_cycle_rate}%</p>
