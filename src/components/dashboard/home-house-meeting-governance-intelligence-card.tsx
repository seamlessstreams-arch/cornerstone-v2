...
export function HomeHouseMeetingGovernanceIntelligenceCard() {
  const { data, isLoading } = useHomeHouseMeetingGovernanceIntelligence();
  ...
  const isAlert = d.meeting_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-orange-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">House Meetings</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.meeting_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.meeting_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {d.meeting_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_meetings > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_meetings > 0 ? "text-green-600" : "text-amber-600")}>{d.total_meetings}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_attendance_rate >= 85 ? "bg-green-50" : d.child_attendance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_attendance_rate >= 85 ? "text-green-600" : d.child_attendance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_feedback_rate >= 90 ? "bg-green-50" : d.child_feedback_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_feedback_rate >= 90 ? "text-green-600" : d.child_feedback_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_feedback_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_duration >= 30 ? "bg-green-50" : d.average_duration >= 20 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_duration >= 30 ? "text-green-600" : d.average_duration >= 20 ? "text-amber-600" : "text-red-600")}>{d.average_duration}m</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Meeting Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
