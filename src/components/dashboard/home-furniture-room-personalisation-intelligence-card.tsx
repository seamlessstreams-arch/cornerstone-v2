...
export function HomeFurnitureRoomPersonalisationIntelligenceCard() {
  const { data, isLoading } = useHomeFurnitureRoomPersonalisationIntelligence();
  ...
  const __emptyState = d.room_rating === "inadequate" && (d.room_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      room_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.room_rating as FurnitureRoomRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.room_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Armchair className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">Furniture & Room Personalisation</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.room_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.room_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.room_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.furniture_adequacy_rate >= 90 ? "bg-green-50" : d.furniture_adequacy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.furniture_adequacy_rate >= 90 ? "text-green-600" : d.furniture_adequacy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.furniture_adequacy_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Furniture</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.personalisation_rate >= 90 ? "bg-green-50" : d.personalisation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.personalisation_rate >= 90 ? "text-green-600" : d.personalisation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.personalisation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.comfort_rate >= 90 ? "bg-green-50" : d.comfort_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.comfort_rate >= 90 ? "text-green-600" : d.comfort_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.comfort_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Comfort</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.dignity_rate >= 90 ? "bg-green-50" : d.dignity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dignity_rate >= 90 ? "text-green-600" : d.dignity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dignity_rate}%</p>
