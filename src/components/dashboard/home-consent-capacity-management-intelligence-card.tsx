...
export function HomeConsentCapacityManagementIntelligenceCard() {
  const { data, isLoading } = useHomeConsentCapacityManagementIntelligence();
  ...
  const __emptyState = d.consent_rating === "inadequate" && (d.consent_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      consent_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.consent_rating as ConsentCapacityRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.consent_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-emerald-600 border-2")} aria-label="Cara Consent &amp; Capacity Intelligence">
        ...
        {d.consent_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.consent_coverage_rate >= 90 ? "bg-green-50" : d.consent_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.consent_coverage_rate >= 90 ? "text-green-600" : d.consent_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.consent_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.gillick_assessment_rate >= 90 ? "bg-green-50" : d.gillick_assessment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.gillick_assessment_rate >= 90 ? "text-green-600" : d.gillick_assessment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.gillick_assessment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.capacity_review_rate >= 90 ? "bg-green-50" : d.capacity_review_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.capacity_review_rate >= 90 ? "text-green-600" : d.capacity_review_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.capacity_review_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.informed_consent_rate >= 90 ? "bg-green-50" : d.informed_consent_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.informed_consent_rate >= 90 ? "text-green-600" : d.informed_consent_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.informed_consent_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.withdrawal_handling_rate >= 90 ? "bg-green-50" : d.withdrawal_handling_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.withdrawal_handling_rate >= 90 ? "text-green-600" : d.withdrawal_handling_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.withdrawal_handling_rate}%</p>
