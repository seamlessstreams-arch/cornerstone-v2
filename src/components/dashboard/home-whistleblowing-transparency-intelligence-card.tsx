...
export function HomeWhistleblowingTransparencyIntelligenceCard() {
  ...
  const __emptyState = d.whistleblowing_rating === "inadequate" && (d.whistleblowing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-blue-200 border-2")}>
      ...
      <CardContent className="space-y-4">
        {d.whistleblowing_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.whistleblowing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_concerns > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_concerns > 0 ? "text-[--cs-success]" : "text-[--cs-warning]")}>{d.total_concerns}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Transparency Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
