...
const RATING_STYLES: Record<RecruitmentRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSaferRecruitmentIntelligenceCard() {
  ...
  const hasConcerns = d.checks_profile.concern_count > 0;
  const hasHighRisk = d.compliance_profile.high_risk_count > 0;
  const isAlert = hasOverdue || hasConcerns || hasHighRisk || d.recruitment_rating === "inadequate";
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            {d.recruitment_rating !== "insufficient_data" && (
              <span className="text-xs font-bold tabular-nums text-slate-600">{d.recruitment_score}%</span>
            )}
          </CardTitle>
        </div>
            ...
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <div className="flex items-center justify-center gap-1">
                ...
                <p className="text-lg font-bold tabular-nums text-slate-700">
                  {d.vacancy_profile.total_candidates}
                </p>
              </div>
        ...
        {d.recruitment_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 gap-1.5">
            <div className="rounded border p-2 text-xs">
              <p className="font-medium text-slate-700 mb-1">Pipeline</p>
              <div className="space-y-0.5 text-[10px] text-muted-foreground">
                <p>Vacancies: <span className="font-medium text-slate-600">{d.vacancy_profile.total_vacancies}</span> ({d.vacancy_profile.open_count} open)</p>
                <p>Shortlisted: <span className="font-medium text-slate-600">{d.vacancy_profile.shortlisted_count}</span></p>
                <p>Appointed: <span className="font-medium text-slate-600">{d.vacancy_profile.appointed_count}</span></p>
              </div>
            </div>
            <div className="rounded border p-2 text-xs">
              <p className="font-medium text-slate-700 mb-1">Checks</p>
              <div className="space-y-0.5 text-[10px] text-muted-foreground">
                <p>Total: <span className="font-medium text-slate-600">{d.checks_profile.total_checks}</span></p>
                {d.checks_profile.overdue_count > 0 && (
