...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, MessageSquareWarning } from "lucide-react";
...
import type { ComplaintAdvocacyRating } from "@/lib/engines/home-complaint-advocacy-responsiveness-intelligence-engine";

const RATING_STYLES: Record<ComplaintAdvocacyRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeComplaintAdvocacyResponsivenessIntelligenceCard() {
  const { data, isLoading } = useHomeComplaintAdvocacyResponsivenessIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.responsiveness_rating as ComplaintAdvocacyRating] ?? RATING_STYLES.insufficient_data;
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MessageSquareWarning className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-rose-600")} />
            <span className="text-slate-900 font-bold">Complaint & Advocacy Responsiveness</span>
      ...
      <CardContent className="space-y-4">
        {d.responsiveness_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.complaint_resolution_rate >= 90 ? "bg-green-50" : d.complaint_resolution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.complaint_resolution_rate >= 90 ? "text-green-600" : d.complaint_resolution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.complaint_resolution_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.advocacy_access_rate >= 90 ? "bg-green-50" : d.advocacy_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.advocacy_access_rate >= 90 ? "text-green-600" : d.advocacy_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.advocacy_access_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.feedback_loop_completion_rate >= 90 ? "bg-green-50" : d.feedback_loop_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.feedback_loop_completion_rate >= 90 ? "text-green-600" : d.feedback_loop_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.feedback_loop_completion_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Feedback</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.participation_rate >= 90 ? "bg-green-50" : d.participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.participation_rate >= 90 ? "text-green-600" : d.participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.participation_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Particip.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.learning_implemented_rate >= 90 ? "bg-green-50" : d.learning_implemented_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.learning_implemented_rate >= 90 ? "text-green-600" : d.learning_implemented_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.learning_implemented_rate}%</p>
