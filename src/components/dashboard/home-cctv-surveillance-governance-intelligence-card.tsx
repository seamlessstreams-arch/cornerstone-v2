...
export function HomeCctvSurveillanceGovernanceIntelligenceCard() {
  const { data, isLoading } = useHomeCctvSurveillanceGovernanceIntelligence();
  if (isLoading) return <Card className="overflow-hidden border-slate-600"><CardContent className="flex items-center justify-center py-12"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></CardContent></Card>;
  let d = data?.data ?? data;
  if (!d) return null;
  // Calm reframe: an empty-with-children engine result (inadequate + score<=15) is
  // 'not yet recorded', not a failing home — render it as honest, neutral insufficient_data.
  const __emptyState = d.cctv_rating === "inadequate" && (d.cctv_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      cctv_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.cctv_rating as CctvGovernanceRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.cctv_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Camera className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
            <span className="text-slate-900 font-bold">CCTV & Surveillance Governance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.cctv_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.cctv_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.cctv_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.policy_compliance_rate >= 90 ? "bg-green-50" : d.policy_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.policy_compliance_rate >= 90 ? "text-green-600" : d.policy_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.policy_compliance_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Policy</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.privacy_impact_rate >= 90 ? "bg-green-50" : d.privacy_impact_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.privacy_impact_rate >= 90 ? "text-green-600" : d.privacy_impact_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.privacy_impact_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Privacy</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.retention_compliance_rate >= 90 ? "bg-green-50" : d.retention_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.retention_compliance_rate >= 90 ? "text-green-600" : d.retention_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.retention_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.data_protection_rate >= 90 ? "bg-green-50" : d.data_protection_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.data_protection_rate >= 90 ? "text-green-600" : d.data_protection_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.data_protection_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Data Prot.</p>
            </div>
