...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Calculator } from "lucide-react";
...
import type { PocketMoneyAuditRating } from "@/lib/engines/home-pocket-money-audit-reconciliation-intelligence-engine";

const RATING_STYLES: Record<PocketMoneyAuditRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePocketMoneyAuditReconciliationIntelligenceCard() {
  const { data, isLoading } = useHomePocketMoneyAuditReconciliationIntelligence();
  ...
  const __emptyState = d.audit_rating === "inadequate" && (d.audit_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.audit_rating as PocketMoneyAuditRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.audit_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Calculator className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-amber-600")} />
            <span className="text-slate-900 font-bold">Pocket Money Audit & Reconciliation</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.audit_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.audit_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.audit_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reconciliation_accuracy_rate >= 90 ? "bg-green-50" : d.reconciliation_accuracy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reconciliation_accuracy_rate >= 90 ? "text-green-600" : d.reconciliation_accuracy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.reconciliation_accuracy_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Recon.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.discrepancy_resolution_rate >= 90 ? "bg-green-50" : d.discrepancy_resolution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.discrepancy_resolution_rate >= 90 ? "text-green-600" : d.discrepancy_resolution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.discrepancy_resolution_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.transparency_rate >= 90 ? "bg-green-50" : d.transparency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.transparency_rate >= 90 ? "text-green-600" : d.transparency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.transparency_rate}%</p>
