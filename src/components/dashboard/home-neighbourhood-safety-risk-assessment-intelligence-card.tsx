...
import { useHomeNeighbourhoodSafetyRiskAssessmentIntelligence } from "@/hooks/use-home-neighbourhood-safety-risk-assessment-intelligence";
import type { NeighbourhoodSafetyRating } from "@/lib/engines/home-neighbourhood-safety-risk-assessment-intelligence-engine";

const RATING_STYLES: Record<NeighbourhoodSafetyRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeNeighbourhoodSafetyRiskAssessmentIntelligenceCard() {
  const { data, isLoading } = useHomeNeighbourhoodSafetyRiskAssessmentIntelligence();
  ...
  const __emptyState = d.neighbourhood_rating === "inadequate" && (d.neighbourhood_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      neighbourhood_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.neighbourhood_rating as NeighbourhoodSafetyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.neighbourhood_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MapPin className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-red-600")} />
            <span className="text-slate-900 font-bold">Neighbourhood Safety & Risk Assessment</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.neighbourhood_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.neighbourhood_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.neighbourhood_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.safety_mapping_rate >= 90 ? "bg-green-50" : d.safety_mapping_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safety_mapping_rate >= 90 ? "text-green-600" : d.safety_mapping_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.safety_mapping_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hazard_identification_rate >= 90 ? "bg-green-50" : d.hazard_identification_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hazard_identification_rate >= 90 ? "text-green-600" : d.hazard_identification_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hazard_identification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.route_safety_rate >= 90 ? "bg-green-50" : d.route_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.route_safety_rate >= 90 ? "text-green-600" : d.route_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.route_safety_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.community_partnership_rate >= 90 ? "bg-green-50" : d.community_partnership_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.community_partnership_rate >= 90 ? "text-green-600" : d.community_partnership_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.community_partnership_rate}%</p>
