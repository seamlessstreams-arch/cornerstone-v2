...
const RATING_STYLES: Record<AdvocacyVisitorRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeAdvocacyIndependentVisitorIntelligenceCard() {
  const { data, isLoading } = useHomeAdvocacyIndependentVisitorIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.advocacy_rating as AdvocacyVisitorRating] ?? RATING_STYLES.insufficient_data;
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Megaphone className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Advocacy & Independent Visitor</span>
        ...
        {d.advocacy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.visitor_allocation_rate >= 90 ? "bg-green-50" : d.visitor_allocation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.visitor_allocation_rate >= 90 ? "text-green-600" : d.visitor_allocation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.visitor_allocation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.representation_quality_rate >= 90 ? "bg-green-50" : d.representation_quality_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.representation_quality_rate >= 90 ? "text-green-600" : d.representation_quality_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.representation_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.visit_compliance_rate >= 90 ? "bg-green-50" : d.visit_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.visit_compliance_rate >= 90 ? "text-green-600" : d.visit_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.visit_compliance_rate}%</p>
