...
import type { AdvocacyVoiceRating } from "@/lib/engines/home-advocacy-independent-voice-intelligence-engine";

const RATING_STYLES: Record<AdvocacyVoiceRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeAdvocacyIndependentVoiceIntelligenceCard() {
  const { data, isLoading } = useHomeAdvocacyIndependentVoiceIntelligence();
  ...
  const d = data?.data;
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.advocacy_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.advocacy_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-blue-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Advocacy & Independent Voice</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.advocacy_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.advocacy_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.advocacy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_records > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_records > 0 ? "text-green-600" : "text-amber-600")}>{d.total_records}</p>
              <p className="text-[9px] text-muted-foreground">Records</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.active_rate >= 80 ? "bg-green-50" : d.active_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_rate >= 80 ? "text-green-600" : d.active_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.active_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Active</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_advocacy_rate >= 80 ? "bg-green-50" : d.children_with_advocacy_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_advocacy_rate >= 80 ? "text-green-600" : d.children_with_advocacy_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.children_with_advocacy_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Coverage</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.independent_rate >= 50 ? "bg-green-50" : d.independent_rate >= 25 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.independent_rate >= 50 ? "text-green-600" : d.independent_rate >= 25 ? "text-amber-600" : "text-red-600")}>{d.independent_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_voice_rate >= 90 ? "bg-green-50" : d.child_voice_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_voice_rate >= 90 ? "text-green-600" : d.child_voice_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_voice_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Voice</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.advocacy_type_variety >= 4 ? "bg-green-50" : d.advocacy_type_variety >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.advocacy_type_variety >= 4 ? "text-green-600" : d.advocacy_type_variety >= 2 ? "text-amber-600" : "text-red-600")}>{d.advocacy_type_variety}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> ARIA Advocacy Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
