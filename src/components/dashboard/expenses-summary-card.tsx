      ...
      <CardContent className="pt-0 space-y-2.5">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className={cn("rounded-xl p-2 text-center", meta.pending_count > 0 ? "bg-amber-50" : "bg-slate-50")}>
            ...
            <div className={cn("text-[9px]", meta.pending_count > 0 ? "text-amber-500" : "text-[var(--cs-text-muted)]")}>Pending</div>
          </div>
        ...
        {/* Approved amount */}
        <div className="flex items-center justify-between text-[10px] px-1">
          <span className="text-[var(--cs-text-muted)]">Approved amount</span>
          <span className="font-bold text-emerald-600 tabular-nums">{formatCurrency(approvedAmount)}</span>
        </div>

        {/* Total claims */}
        <div className="flex items-center justify-between text-[10px] px-1">
          <span className="text-[var(--cs-text-muted)]">Total claims</span>
          <span className="font-bold text-[var(--cs-text-secondary)] tabular-nums">{meta.total_count}</span>
        </div>
