...
export function HomePrivacyDignityIntelligenceCard() {
  ...
  const __emptyState = d.privacy_rating === "inadequate" && (d.privacy_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      privacy_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-amber-600 border-2")} aria-label="Cara Privacy &amp; Dignity Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Lock className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-[--cs-warning]")} />
