...
export function HomeSexualHealthRseEducationIntelligenceCard() {
  const { data, isLoading } = useHomeSexualHealthRseEducationIntelligence();
  ...
  const __emptyState = d.rse_rating === "inadequate" && (d.rse_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      rse_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.rse_rating as SexualHealthRseRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.rse_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartPulse className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
        ...
        {d.rse_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.rse_delivery_rate >= 90 ? "bg-green-50" : d.rse_delivery_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.rse_delivery_rate >= 90 ? "text-green-600" : d.rse_delivery_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.rse_delivery_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.health_screening_rate >= 90 ? "bg-green-50" : d.health_screening_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.health_screening_rate >= 90 ? "text-green-600" : d.health_screening_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.health_screening_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.consent_education_rate >= 90 ? "bg-green-50" : d.consent_education_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.consent_education_rate >= 90 ? "text-green-600" : d.consent_education_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.consent_education_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.safeguarding_awareness_rate >= 90 ? "bg-green-50" : d.safeguarding_awareness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safeguarding_awareness_rate >= 90 ? "text-green-600" : d.safeguarding_awareness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.safeguarding_awareness_rate}%</p>
