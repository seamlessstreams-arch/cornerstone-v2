...
const RATING_STYLES: Record<StatutoryVisitRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeStatutoryVisitComplianceIntelligenceCard() {
  const { data, isLoading } = useHomeStatutoryVisitComplianceIntelligence();
  ...
  const __emptyState = d.visit_rating === "inadequate" && (d.visit_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      visit_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.visit_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.visit_rating === "inadequate";
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-sky-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ClipboardCheck className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
            <span className="text-slate-900 font-bold">Statutory Visiting</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.visit_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.visit_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {d.visit_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.statutory_visit_completion_rate >= 85 ? "bg-green-50" : d.statutory_visit_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.statutory_visit_completion_rate >= 85 ? "text-green-600" : d.statutory_visit_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.statutory_visit_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_seen_alone_rate >= 85 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_seen_alone_rate >= 85 ? "text-green-600" : "text-amber-600")}>{d.children_seen_alone_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.social_worker_contact_rate >= 80 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.social_worker_contact_rate >= 80 ? "text-green-600" : "text-amber-600")}>{d.social_worker_contact_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_without_recent_visit === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_without_recent_visit === 0 ? "text-green-600" : "text-red-600")}>{d.children_without_recent_visit}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reg22_compliance_rate >= 80 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reg22_compliance_rate >= 80 ? "text-green-600" : "text-amber-600")}>{d.reg22_compliance_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Visiting Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
