...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, MessageSquare } from "lucide-react";
...
export function HomeChildVoiceParticipationIntelligenceCard() {
  const { data, isLoading } = useHomeChildVoiceParticipationIntelligence();
  ...
  const __emptyState = d.voice_rating === "inadequate" && (d.voice_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.voice_rating as ChildVoiceRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.voice_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MessageSquare className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Child Voice & Participation</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.voice_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.voice_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.voice_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.meeting_attendance_rate >= 90 ? "bg-green-50" : d.meeting_attendance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.meeting_attendance_rate >= 90 ? "text-green-600" : d.meeting_attendance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.meeting_attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.consultation_rate >= 90 ? "bg-green-50" : d.consultation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.consultation_rate >= 90 ? "text-green-600" : d.consultation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.consultation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.feedback_action_rate >= 90 ? "bg-green-50" : d.feedback_action_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.feedback_action_rate >= 90 ? "text-green-600" : d.feedback_action_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.feedback_action_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.council_engagement_rate >= 90 ? "bg-green-50" : d.council_engagement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.council_engagement_rate >= 90 ? "text-green-600" : d.council_engagement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.council_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.feeling_heard_rate >= 90 ? "bg-green-50" : d.feeling_heard_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.feeling_heard_rate >= 90 ? "text-green-600" : d.feeling_heard_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.feeling_heard_rate}%</p>
