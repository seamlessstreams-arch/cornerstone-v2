...
import type { SiblingContactRating } from "@/lib/engines/home-sibling-contact-relationships-intelligence-engine";

const RATING_STYLES: Record<SiblingContactRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSiblingContactRelationshipsIntelligenceCard() {
  const { data, isLoading } = useHomeSiblingContactRelationshipsIntelligence();
  ...
  const __emptyState = d.sibling_rating === "inadequate" && (d.sibling_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      sibling_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.sibling_rating as SiblingContactRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.sibling_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-purple-600 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Sibling Contact & Relationships</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.sibling_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.sibling_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.sibling_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.placement_consideration_rate >= 90 ? "bg-green-50" : d.placement_consideration_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.placement_consideration_rate >= 90 ? "text-green-600" : d.placement_consideration_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.placement_consideration_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Placement</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.contact_facilitation_rate >= 90 ? "bg-green-50" : d.contact_facilitation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.contact_facilitation_rate >= 90 ? "text-green-600" : d.contact_facilitation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.contact_facilitation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.relationship_quality_rate >= 90 ? "bg-green-50" : d.relationship_quality_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.relationship_quality_rate >= 90 ? "text-green-600" : d.relationship_quality_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.relationship_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.event_participation_rate >= 90 ? "bg-green-50" : d.event_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.event_participation_rate >= 90 ? "text-green-600" : d.event_participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.event_participation_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Events</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.child_wishes_rate >= 90 ? "bg-green-50" : d.child_wishes_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_wishes_rate >= 90 ? "text-green-600" : d.child_wishes_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_wishes_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Wishes</p>
            </div>
