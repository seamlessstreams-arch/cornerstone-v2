...
export function HomeHandoverCommunicationQualityIntelligenceCard() {
  const { data, isLoading } = useHomeHandoverCommunicationQualityIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.handover_rating as HandoverRating] ?? RATING_STYLES.insufficient_data;
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ArrowRightLeft className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
        ...
        {d.handover_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.handover_quality_rate >= 90 ? "bg-green-50" : d.handover_quality_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.handover_quality_rate >= 90 ? "text-green-600" : d.handover_quality_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.handover_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.communication_log_rate >= 90 ? "bg-green-50" : d.communication_log_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.communication_log_rate >= 90 ? "text-green-600" : d.communication_log_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.communication_log_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.critical_info_rate >= 90 ? "bg-green-50" : d.critical_info_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.critical_info_rate >= 90 ? "text-green-600" : d.critical_info_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.critical_info_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.handover_timeliness_rate >= 90 ? "bg-green-50" : d.handover_timeliness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.handover_timeliness_rate >= 90 ? "text-green-600" : d.handover_timeliness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.handover_timeliness_rate}%</p>
