...
export function HomeDailyRoutineCareContinuityIntelligenceCard() {
  ...
  const ratingStyle = RATING_STYLES[d.routine_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.routine_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-amber-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Clock className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-amber-600")} />
            <span className="text-slate-900 font-bold">Daily Routine & Continuity</span>
      ...
      <CardContent className="space-y-4">
        {d.routine_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.routine_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_routines > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_routines > 0 ? "text-green-600" : "text-amber-600")}>{d.children_with_routines}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.duty_log_completion_rate >= 90 ? "bg-green-50" : d.duty_log_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.duty_log_completion_rate >= 90 ? "text-green-600" : d.duty_log_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.duty_log_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.shift_note_quality_rate >= 85 ? "bg-green-50" : d.shift_note_quality_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.shift_note_quality_rate >= 85 ? "text-green-600" : d.shift_note_quality_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.shift_note_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.cleaning_standard_rate >= 90 ? "bg-green-50" : d.cleaning_standard_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cleaning_standard_rate >= 90 ? "text-green-600" : d.cleaning_standard_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cleaning_standard_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sleep_in_response_rate >= 90 ? "bg-green-50" : d.sleep_in_response_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sleep_in_response_rate >= 90 ? "text-green-600" : d.sleep_in_response_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.sleep_in_response_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Care Continuity Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
