...
export function HomeFoodStorageRefrigerationSafetyIntelligenceCard() {
  const { data, isLoading } = useHomeFoodStorageRefrigerationSafetyIntelligence();
  ...
  const __emptyState = d.food_storage_rating === "inadequate" && (d.food_storage_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      food_storage_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.food_storage_rating as FoodStorageRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.food_storage_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Refrigerator className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
        ...
        {d.food_storage_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.temperature_logging_rate >= 90 ? "bg-green-50" : d.temperature_logging_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.temperature_logging_rate >= 90 ? "text-green-600" : d.temperature_logging_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.temperature_logging_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.storage_compliance_rate >= 90 ? "bg-green-50" : d.storage_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.storage_compliance_rate >= 90 ? "text-green-600" : d.storage_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.storage_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.date_checking_rate >= 90 ? "bg-green-50" : d.date_checking_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.date_checking_rate >= 90 ? "text-green-600" : d.date_checking_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.date_checking_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hygiene_rating_rate >= 90 ? "bg-green-50" : d.hygiene_rating_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hygiene_rating_rate >= 90 ? "text-green-600" : d.hygiene_rating_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hygiene_rating_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.cross_contamination_rate >= 90 ? "bg-green-50" : d.cross_contamination_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cross_contamination_rate >= 90 ? "text-green-600" : d.cross_contamination_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cross_contamination_rate}%</p>
