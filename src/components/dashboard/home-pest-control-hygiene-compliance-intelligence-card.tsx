...
const RATING_STYLES: Record<PestControlRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePestControlHygieneComplianceIntelligenceCard() {
  const { data, isLoading } = useHomePestControlHygieneComplianceIntelligence();
  ...
  const __emptyState = d.pest_control_rating === "inadequate" && (d.pest_control_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      pest_control_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.pest_control_rating as PestControlRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.pest_control_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-amber-600 border-2")} role="region" aria-label="Cara Pest Control &amp; Hygiene Compliance Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Bug className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-amber-600")} />
            <span className="text-slate-900 font-bold">Pest Control & Hygiene Compliance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.pest_control_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.pest_control_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.pest_control_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.treatment_effectiveness_rate >= 90 ? "bg-green-50" : d.treatment_effectiveness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.treatment_effectiveness_rate >= 90 ? "text-green-600" : d.treatment_effectiveness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.treatment_effectiveness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.kitchen_hygiene_rate >= 90 ? "bg-green-50" : d.kitchen_hygiene_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.kitchen_hygiene_rate >= 90 ? "text-green-600" : d.kitchen_hygiene_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.kitchen_hygiene_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.cleanliness_rate >= 90 ? "bg-green-50" : d.cleanliness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cleanliness_rate >= 90 ? "text-green-600" : d.cleanliness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cleanliness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.product_safety_rate >= 90 ? "bg-green-50" : d.product_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.product_safety_rate >= 90 ? "text-green-600" : d.product_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.product_safety_rate}%</p>
