...
export function HomeEthnicHairSkincareIntelligenceCard() {
  ...
  const __emptyState = d.haircare_rating === "inadequate" && (d.haircare_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      haircare_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Scissors className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-fuchsia-600")} />
