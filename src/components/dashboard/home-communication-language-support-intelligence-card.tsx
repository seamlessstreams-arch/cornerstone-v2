...
const RATING_STYLES: Record<CommunicationSupportRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeCommunicationLanguageSupportIntelligenceCard() {
  const { data, isLoading } = useHomeCommunicationLanguageSupportIntelligence();
  ...
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.communication_rating as CommunicationSupportRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.communication_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MessageCircle className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Communication & Language Support</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.communication_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.communication_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.communication_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.assessment_coverage_rate >= 90 ? "bg-green-50" : d.assessment_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.assessment_coverage_rate >= 90 ? "text-green-600" : d.assessment_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.assessment_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.therapy_engagement_rate >= 90 ? "bg-green-50" : d.therapy_engagement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.therapy_engagement_rate >= 90 ? "text-green-600" : d.therapy_engagement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.therapy_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.aid_provision_rate >= 90 ? "bg-green-50" : d.aid_provision_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.aid_provision_rate >= 90 ? "text-green-600" : d.aid_provision_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.aid_provision_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.inclusive_practice_rate >= 90 ? "bg-green-50" : d.inclusive_practice_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.inclusive_practice_rate >= 90 ? "text-green-600" : d.inclusive_practice_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.inclusive_practice_rate}%</p>
