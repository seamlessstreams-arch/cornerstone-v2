...
const RATING_STYLES: Record<SaferRecruitmentRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSaferRecruitmentVettingIntelligenceCard() {
  const { data, isLoading } = useHomeSaferRecruitmentVettingIntelligence();
  ...
  const isAlert = d.recruitment_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldCheck className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
            <span className="text-slate-900 font-bold">Safer Recruitment</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.recruitment_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.recruitment_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.recruitment_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_candidates > 0 ? "bg-green-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_candidates > 0 ? "text-green-600" : "text-slate-600")}>{d.total_candidates}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.dbs_clearance_rate >= 95 ? "bg-green-50" : d.dbs_clearance_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dbs_clearance_rate >= 95 ? "text-green-600" : d.dbs_clearance_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.dbs_clearance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reference_completion_rate >= 90 ? "bg-green-50" : d.reference_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reference_completion_rate >= 90 ? "text-green-600" : d.reference_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.reference_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.history_verification_rate >= 90 ? "bg-green-50" : d.history_verification_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.history_verification_rate >= 90 ? "text-green-600" : d.history_verification_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.history_verification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.interview_compliance_rate >= 90 ? "bg-green-50" : d.interview_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.interview_compliance_rate >= 90 ? "text-green-600" : d.interview_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.interview_compliance_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> ARIA Recruitment Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
