...
import type { ConsentRightsRating } from "@/lib/engines/home-consent-rights-literacy-intelligence-engine";

const RATING_STYLES: Record<ConsentRightsRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeConsentRightsLiteracyIntelligenceCard() {
  const { data, isLoading } = useHomeConsentRightsLiteracyIntelligence();
  ...
  const __emptyState = d.consent_rights_rating === "inadequate" && (d.consent_rights_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.consent_rights_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.consent_rights_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Scale className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
            <span className="text-slate-900 font-bold">Consent & Rights</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.consent_rights_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.consent_rights_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.consent_rights_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.consent_rights_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.active_consents > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_consents > 0 ? "text-green-600" : "text-amber-600")}>{d.active_consents}</p>
              <p className="text-[9px] text-muted-foreground">Active</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.expired_consents === 0 ? "bg-green-50" : d.expired_consents <= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.expired_consents === 0 ? "text-green-600" : d.expired_consents <= 2 ? "text-amber-600" : "text-red-600")}>{d.expired_consents}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.rights_knowledge_rate >= 85 ? "bg-green-50" : d.rights_knowledge_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.rights_knowledge_rate >= 85 ? "text-green-600" : d.rights_knowledge_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.rights_knowledge_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_rights_assessed > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_rights_assessed > 0 ? "text-green-600" : "text-amber-600")}>{d.children_rights_assessed}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.pr_documentation_rate >= 85 ? "bg-green-50" : d.pr_documentation_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.pr_documentation_rate >= 85 ? "text-green-600" : d.pr_documentation_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.pr_documentation_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Rights Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
