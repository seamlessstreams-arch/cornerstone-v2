...
import { useHomeTransportVehicleSafetyIntelligence } from "@/hooks/use-home-transport-vehicle-safety-intelligence";
...
export function HomeTransportVehicleSafetyIntelligenceCard() {
  const { data, isLoading } = useHomeTransportVehicleSafetyIntelligence();
  ...
  const __emptyState = d.transport_rating === "inadequate" && (d.transport_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.transport_rating as TransportSafetyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.transport_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Car className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
            <span className="text-slate-900 font-bold">Transport & Vehicle Safety</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.transport_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.transport_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.transport_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.vehicle_check_compliance_rate >= 90 ? "bg-green-50" : d.vehicle_check_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.vehicle_check_compliance_rate >= 90 ? "text-green-600" : d.vehicle_check_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.vehicle_check_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.pre_use_check_completion_rate >= 90 ? "bg-green-50" : d.pre_use_check_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.pre_use_check_completion_rate >= 90 ? "text-green-600" : d.pre_use_check_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.pre_use_check_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.driver_qualification_currency_rate >= 90 ? "bg-green-50" : d.driver_qualification_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.driver_qualification_currency_rate >= 90 ? "text-green-600" : d.driver_qualification_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.driver_qualification_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.seatbelt_compliance_rate >= 90 ? "bg-green-50" : d.seatbelt_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.seatbelt_compliance_rate >= 90 ? "text-green-600" : d.seatbelt_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.seatbelt_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.mot_service_currency_rate >= 90 ? "bg-green-50" : d.mot_service_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mot_service_currency_rate >= 90 ? "text-green-600" : d.mot_service_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.mot_service_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.defect_resolution_rate >= 90 ? "bg-green-50" : d.defect_resolution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.defect_resolution_rate >= 90 ? "text-green-600" : d.defect_resolution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.defect_resolution_rate}%</p>
