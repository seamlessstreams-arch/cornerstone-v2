...
export function HomeBirthdaySpecialOccasionCelebrationIntelligenceCard() {
  const { data, isLoading } = useHomeBirthdaySpecialOccasionCelebrationIntelligence();
  ...
  const __emptyState = d.celebration_rating === "inadequate" && (d.celebration_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      celebration_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.celebration_rating as BirthdayCelebrationRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.celebration_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Cake className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Birthday & Special Occasion Celebrations</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.celebration_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.celebration_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.celebration_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.birthday_planning_rate >= 90 ? "bg-green-50" : d.birthday_planning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.birthday_planning_rate >= 90 ? "text-green-600" : d.birthday_planning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.birthday_planning_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Planning</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.celebration_execution_rate >= 90 ? "bg-green-50" : d.celebration_execution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.celebration_execution_rate >= 90 ? "text-green-600" : d.celebration_execution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.celebration_execution_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Execution</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.gift_provision_rate >= 90 ? "bg-green-50" : d.gift_provision_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.gift_provision_rate >= 90 ? "text-green-600" : d.gift_provision_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.gift_provision_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Gifts</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.memory_making_rate >= 90 ? "bg-green-50" : d.memory_making_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.memory_making_rate >= 90 ? "text-green-600" : d.memory_making_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.memory_making_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_choice_rate >= 90 ? "bg-green-50" : d.child_choice_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_choice_rate >= 90 ? "text-green-600" : d.child_choice_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_choice_rate}%</p>
