...
export function HomeAllergyManagementFoodSafetyIntelligenceCard() {
  const { data, isLoading } = useHomeAllergyManagementFoodSafetyIntelligence();
  ...
  const __emptyState = d.allergy_rating === "inadequate" && (d.allergy_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      allergy_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.allergy_rating as AllergyManagementRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.allergy_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <AlertTriangle className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">Allergy Management & Food Safety</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.allergy_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.allergy_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.allergy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.allergy_plan_rate >= 90 ? "bg-green-50" : d.allergy_plan_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.allergy_plan_rate >= 90 ? "text-green-600" : d.allergy_plan_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.allergy_plan_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.allergen_awareness_rate >= 90 ? "bg-green-50" : d.allergen_awareness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.allergen_awareness_rate >= 90 ? "text-green-600" : d.allergen_awareness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.allergen_awareness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.epipen_check_rate >= 90 ? "bg-green-50" : d.epipen_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.epipen_check_rate >= 90 ? "text-green-600" : d.epipen_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.epipen_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.food_labelling_rate >= 90 ? "bg-green-50" : d.food_labelling_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.food_labelling_rate >= 90 ? "text-green-600" : d.food_labelling_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.food_labelling_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.emergency_response_rate >= 90 ? "bg-green-50" : d.emergency_response_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.emergency_response_rate >= 90 ? "text-green-600" : d.emergency_response_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.emergency_response_rate}%</p>
