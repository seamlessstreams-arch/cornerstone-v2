      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BellRing className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-rose-600")} />
        ...
        {d.responsiveness_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.read_rate >= 90 ? "bg-green-50" : d.read_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.read_rate >= 90 ? "text-[--cs-success]" : d.read_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.read_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.urgent_read_rate >= 100 ? "bg-green-50" : d.urgent_read_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.urgent_read_rate >= 100 ? "text-[--cs-success]" : d.urgent_read_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.urgent_read_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_response_hours <= 2 ? "bg-green-50" : d.average_response_hours <= 6 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_response_hours <= 2 ? "text-[--cs-success]" : d.average_response_hours <= 6 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.average_response_hours}h</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.unread_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.unread_count === 0 ? "text-[--cs-success]" : "text-[--cs-risk]")}>{d.unread_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.urgent_unread_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.urgent_unread_count === 0 ? "text-[--cs-success]" : "text-[--cs-risk]")}>{d.urgent_unread_count}</p>
