...
import { useHomeRegulatoryComplianceComposite } from "@/hooks/use-home-regulatory-compliance-composite";
import type { RegulatoryComplianceRating } from "@/lib/engines/home-regulatory-compliance-composite-engine";
...
const DOMAIN_LABELS: Record<string, string> = {
  reg44_visits: "Reg 44", reg45_46_evidence: "Reg 45/46", policy_compliance: "Policy",
  data_governance: "Data Gov", quality_assurance: "QA", notifiable_events: "Notif. Events",
  document_governance: "Documents",
};

export function HomeRegulatoryComplianceCompositeCard() {
  const { data, isLoading } = useHomeRegulatoryComplianceComposite();
  ...
  const d = data?.data;
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.compliance_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.compliance_rating === "inadequate";
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-violet-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Scale className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
      ...
      <CardContent className="space-y-4">
        {d.compliance_rating !== "insufficient_data" && d.domain_scores.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-1.5">
            {d.domain_scores.map(ds => (
              <div key={ds.name} className={cn("text-center rounded-lg p-1.5", ds.compliant ? "bg-green-50" : "bg-red-50")}>
                <p className={cn("text-sm font-bold tabular-nums", ds.compliant ? "text-green-600" : "text-red-600")}>{ds.score}/{ds.max}</p>
                <p className="text-[9px] text-muted-foreground leading-tight">{DOMAIN_LABELS[ds.name] ?? ds.name}</p>
              </div>
            ))}
          </div>
        )}
