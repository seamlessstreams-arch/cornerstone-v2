...
const RATING_STYLES: Record<NoiseSoundRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeNoiseSoundManagementIntelligenceCard() {
  const { data, isLoading } = useHomeNoiseSoundManagementIntelligence();
  ...
  const __emptyState = d.noise_rating === "inadequate" && (d.noise_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      noise_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.noise_rating as NoiseSoundRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.noise_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Volume2 className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
            <span className="text-slate-900 font-bold">Noise & Sound Management</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.noise_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.noise_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.noise_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.noise_monitoring_rate >= 90 ? "bg-green-50" : d.noise_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.noise_monitoring_rate >= 90 ? "text-green-600" : d.noise_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.noise_monitoring_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.quiet_hours_compliance_rate >= 90 ? "bg-green-50" : d.quiet_hours_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.quiet_hours_compliance_rate >= 90 ? "text-green-600" : d.quiet_hours_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.quiet_hours_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sensory_environment_rate >= 90 ? "bg-green-50" : d.sensory_environment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sensory_environment_rate >= 90 ? "text-green-600" : d.sensory_environment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.sensory_environment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sound_insulation_rate >= 90 ? "bg-green-50" : d.sound_insulation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sound_insulation_rate >= 90 ? "text-green-600" : d.sound_insulation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.sound_insulation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_comfort_rate >= 90 ? "bg-green-50" : d.child_comfort_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_comfort_rate >= 90 ? "text-green-600" : d.child_comfort_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_comfort_rate}%</p>
