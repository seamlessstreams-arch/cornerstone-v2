...
import type { QocIntelligenceRating } from "@/lib/engines/home-quality-of-care-review-intelligence-engine";

const RATING_STYLES: Record<QocIntelligenceRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeQualityOfCareReviewIntelligenceCard() {
  const { data, isLoading } = useHomeQualityOfCareReviewIntelligence();
  ...
  const __emptyState = d.qoc_rating === "inadequate" && (d.qoc_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.qoc_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.qoc_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Quality of Care Reviews</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.qoc_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.qoc_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.qoc_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.qoc_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_reviews > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_reviews > 0 ? "text-green-600" : "text-amber-600")}>{d.total_reviews}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.good_or_outstanding_rate >= 90 ? "bg-green-50" : d.good_or_outstanding_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.good_or_outstanding_rate >= 90 ? "text-green-600" : d.good_or_outstanding_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.good_or_outstanding_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.action_completion_rate >= 90 ? "bg-green-50" : d.action_completion_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.action_completion_rate >= 90 ? "text-green-600" : d.action_completion_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.action_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_feedback_rate >= 90 ? "bg-green-50" : d.children_feedback_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_feedback_rate >= 90 ? "text-green-600" : d.children_feedback_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.children_feedback_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.domain_quality_rate >= 90 ? "bg-green-50" : d.domain_quality_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.domain_quality_rate >= 90 ? "text-green-600" : d.domain_quality_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.domain_quality_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Quality Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
