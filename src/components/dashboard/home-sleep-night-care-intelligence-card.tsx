...
export function HomeSleepNightCareIntelligenceCard() {
  const { data, isLoading } = useHomeSleepNightCareIntelligence();
  ...
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.sleep_rating as SleepNightCareRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.sleep_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-pink-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Moon className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Sleep & Night Care</span>
      ...
      <CardContent className="space-y-4">
        {d.sleep_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_logs > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_logs > 0 ? "text-green-600" : "text-amber-600")}>{d.total_logs}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.check_compliance_rate >= 95 ? "bg-green-50" : d.check_compliance_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.check_compliance_rate >= 95 ? "text-green-600" : d.check_compliance_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.check_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.quiet_night_rate >= 70 ? "bg-green-50" : d.quiet_night_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.quiet_night_rate >= 70 ? "text-green-600" : d.quiet_night_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.quiet_night_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.significant_disturbance_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.significant_disturbance_count === 0 ? "text-green-600" : "text-red-600")}>{d.significant_disturbance_count}</p>
              <p className="text-[9px] text-muted-foreground">Severe</p>
            </div>
          </div>
        )}
