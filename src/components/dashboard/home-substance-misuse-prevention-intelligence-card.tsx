...
export function HomeSubstanceMisusePreventionIntelligenceCard() {
  const { data, isLoading } = useHomeSubstanceMisusePreventionIntelligence();
  ...
  const __emptyState = d.substance_rating === "inadequate" && (d.substance_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      substance_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.substance_rating as SubstanceMisuseRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.substance_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-orange-600 border-2")} aria-label="Cara Substance Misuse Prevention Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Substance Misuse Prevention</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.substance_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.substance_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.substance_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.education_coverage_rate >= 90 ? "bg-green-50" : d.education_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.education_coverage_rate >= 90 ? "text-green-600" : d.education_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.education_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.referral_compliance_rate >= 90 ? "bg-green-50" : d.referral_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.referral_compliance_rate >= 90 ? "text-green-600" : d.referral_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.referral_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.harm_reduction_rate >= 90 ? "bg-green-50" : d.harm_reduction_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.harm_reduction_rate >= 90 ? "text-green-600" : d.harm_reduction_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.harm_reduction_rate}%</p>
