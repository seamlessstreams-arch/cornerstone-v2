...
import type { OutcomeStarRating } from "@/lib/engines/home-outcome-star-needs-intelligence-engine";
...
export function HomeOutcomeStarNeedsIntelligenceCard() {
  const { data, isLoading } = useHomeOutcomeStarNeedsIntelligence();
  ...
  const __emptyState = d.outcome_rating === "inadequate" && (d.outcome_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.outcome_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.outcome_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-yellow-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-yellow-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Outcome Star & Needs</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.outcome_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.outcome_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.outcome_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.outcome_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_assessed > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_assessed > 0 ? "text-green-600" : "text-amber-600")}>{d.children_assessed}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_outcome_score >= 7 ? "bg-green-50" : d.average_outcome_score >= 5 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_outcome_score >= 7 ? "text-green-600" : d.average_outcome_score >= 5 ? "text-amber-600" : "text-red-600")}>{d.average_outcome_score}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_improving > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_improving > 0 ? "text-green-600" : "text-amber-600")}>{d.children_improving}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.needs_addressed_rate >= 85 ? "bg-green-50" : d.needs_addressed_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.needs_addressed_rate >= 85 ? "text-green-600" : d.needs_addressed_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.needs_addressed_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.kpi_met_rate >= 80 ? "bg-green-50" : d.kpi_met_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.kpi_met_rate >= 80 ? "text-green-600" : d.kpi_met_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.kpi_met_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Outcomes Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
