...
const RATING_STYLES: Record<RestorativePracticeRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeRestorativePracticeConflictResolutionIntelligenceCard() {
  const { data, isLoading } = useHomeRestorativePracticeConflictResolutionIntelligence();
  ...
  const __emptyState = d.restorative_rating === "inadequate" && (d.restorative_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      restorative_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.restorative_rating as RestorativePracticeRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.restorative_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-green-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Handshake className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-green-600")} />
            <span className="text-slate-900 font-bold">Restorative Practice & Conflict Resolution</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.restorative_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.restorative_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.restorative_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.conference_completion_rate >= 90 ? "bg-green-50" : d.conference_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.conference_completion_rate >= 90 ? "text-green-600" : d.conference_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.conference_completion_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Conf.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.conflict_resolution_rate >= 90 ? "bg-green-50" : d.conflict_resolution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.conflict_resolution_rate >= 90 ? "text-green-600" : d.conflict_resolution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.conflict_resolution_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Resolv.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.relationship_repair_rate >= 90 ? "bg-green-50" : d.relationship_repair_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.relationship_repair_rate >= 90 ? "text-green-600" : d.relationship_repair_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.relationship_repair_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.mediation_quality_rate >= 90 ? "bg-green-50" : d.mediation_quality_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mediation_quality_rate >= 90 ? "text-green-600" : d.mediation_quality_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.mediation_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_voice_rate >= 90 ? "bg-green-50" : d.child_voice_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_voice_rate >= 90 ? "text-green-600" : d.child_voice_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_voice_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.satisfaction_rate >= 90 ? "bg-green-50" : d.satisfaction_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.satisfaction_rate >= 90 ? "text-green-600" : d.satisfaction_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.satisfaction_rate}%</p>
