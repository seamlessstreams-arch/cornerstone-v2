...
import type { FriendshipSocialRating } from "@/lib/engines/home-friendship-social-network-intelligence-engine";

const RATING_STYLES: Record<FriendshipSocialRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeFriendshipSocialNetworkIntelligenceCard() {
  const { data, isLoading } = useHomeFriendshipSocialNetworkIntelligence();
  ...
  const __emptyState = d.friendship_rating === "inadequate" && (d.friendship_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      friendship_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.friendship_rating as FriendshipSocialRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.friendship_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Friendship & Social Network</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.friendship_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.friendship_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.friendship_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.friendship_mapping_rate >= 90 ? "bg-green-50" : d.friendship_mapping_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.friendship_mapping_rate >= 90 ? "text-green-600" : d.friendship_mapping_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.friendship_mapping_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Mapped</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.social_network_rate >= 90 ? "bg-green-50" : d.social_network_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.social_network_rate >= 90 ? "text-green-600" : d.social_network_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.social_network_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Networks</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.peer_support_rate >= 90 ? "bg-green-50" : d.peer_support_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.peer_support_rate >= 90 ? "text-green-600" : d.peer_support_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.peer_support_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Peer Sup.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.isolation_prevention_rate >= 90 ? "bg-green-50" : d.isolation_prevention_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.isolation_prevention_rate >= 90 ? "text-green-600" : d.isolation_prevention_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.isolation_prevention_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Isolation</p>
            </div>
