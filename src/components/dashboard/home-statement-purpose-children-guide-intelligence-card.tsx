...
export function HomeStatementPurposeChildrenGuideIntelligenceCard() {
  const { data, isLoading } = useHomeStatementPurposeChildrenGuideIntelligence();
  ...
  const __emptyState = d.statement_rating === "inadequate" && (d.statement_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      statement_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.statement_rating as StatementPurposeRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.statement_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BookMarked className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Statement of Purpose & Children's Guide</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.statement_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.statement_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.statement_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.statement_currency_rate >= 90 ? "bg-green-50" : d.statement_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.statement_currency_rate >= 90 ? "text-green-600" : d.statement_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.statement_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.guide_accessibility_rate >= 90 ? "bg-green-50" : d.guide_accessibility_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.guide_accessibility_rate >= 90 ? "text-green-600" : d.guide_accessibility_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.guide_accessibility_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.review_cycle_rate >= 90 ? "bg-green-50" : d.review_cycle_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_cycle_rate >= 90 ? "text-green-600" : d.review_cycle_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.review_cycle_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.young_person_involvement_rate >= 90 ? "bg-green-50" : d.young_person_involvement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.young_person_involvement_rate >= 90 ? "text-green-600" : d.young_person_involvement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.young_person_involvement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.ofsted_submission_rate >= 90 ? "bg-green-50" : d.ofsted_submission_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.ofsted_submission_rate >= 90 ? "text-green-600" : d.ofsted_submission_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.ofsted_submission_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.stakeholder_awareness_rate >= 90 ? "bg-green-50" : d.stakeholder_awareness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.stakeholder_awareness_rate >= 90 ? "text-green-600" : d.stakeholder_awareness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.stakeholder_awareness_rate}%</p>
