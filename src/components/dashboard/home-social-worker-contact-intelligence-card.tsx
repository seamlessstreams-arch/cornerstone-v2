...
export function HomeSocialWorkerContactIntelligenceCard() {
  const { data, isLoading } = useHomeSocialWorkerContactIntelligence();
  ...
  const __emptyState = d.contact_rating === "inadequate" && (d.contact_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.contact_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.contact_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-teal-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Phone className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Social Worker Contact</span>
      ...
      <CardContent className="space-y-4">
        {d.contact_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.contact_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.home_initiated_rate >= 50 ? "bg-green-50" : d.home_initiated_rate >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.home_initiated_rate >= 50 ? "text-[--cs-success]" : d.home_initiated_rate >= 30 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.home_initiated_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Proactive</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.child_awareness_rate >= 80 ? "bg-green-50" : d.child_awareness_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_awareness_rate >= 80 ? "text-[--cs-success]" : d.child_awareness_rate >= 50 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.child_awareness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.decision_documentation_rate >= 60 ? "bg-green-50" : d.decision_documentation_rate >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.decision_documentation_rate >= 60 ? "text-[--cs-success]" : d.decision_documentation_rate >= 30 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.decision_documentation_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Social Worker Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
