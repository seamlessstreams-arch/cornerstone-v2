...
import { useHomeAriaContentQualityIntelligence } from "@/hooks/use-home-aria-content-quality-intelligence";
import type { AriaContentRating } from "@/lib/engines/home-aria-content-quality-intelligence-engine";

const RATING_STYLES: Record<AriaContentRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeAriaContentQualityIntelligenceCard() {
  const { data, isLoading } = useHomeAriaContentQualityIntelligence();
  ...
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.content_rating as AriaContentRating] ?? RATING_STYLES.insufficient_data;
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">ARIA Content Quality</span>
