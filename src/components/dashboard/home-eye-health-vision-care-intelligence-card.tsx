...
import { useHomeEyeHealthVisionCareIntelligence } from "@/hooks/use-home-eye-health-vision-care-intelligence";
import type { EyeHealthRating } from "@/lib/engines/home-eye-health-vision-care-intelligence-engine";

const RATING_STYLES: Record<EyeHealthRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeEyeHealthVisionCareIntelligenceCard() {
  const { data, isLoading } = useHomeEyeHealthVisionCareIntelligence();
  ...
  const __emptyState = d.eye_health_rating === "inadequate" && (d.eye_health_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      eye_health_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.eye_health_rating as EyeHealthRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.eye_health_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Eye className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
        ...
        {d.eye_health_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.eye_test_compliance_rate >= 90 ? "bg-green-50" : d.eye_test_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.eye_test_compliance_rate >= 90 ? "text-green-600" : d.eye_test_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.eye_test_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.prescription_management_rate >= 90 ? "bg-green-50" : d.prescription_management_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.prescription_management_rate >= 90 ? "text-green-600" : d.prescription_management_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.prescription_management_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.optician_referral_rate >= 90 ? "bg-green-50" : d.optician_referral_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.optician_referral_rate >= 90 ? "text-green-600" : d.optician_referral_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.optician_referral_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.visual_aid_rate >= 90 ? "bg-green-50" : d.visual_aid_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.visual_aid_rate >= 90 ? "text-green-600" : d.visual_aid_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.visual_aid_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.follow_up_rate >= 90 ? "bg-green-50" : d.follow_up_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.follow_up_rate >= 90 ? "text-green-600" : d.follow_up_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.follow_up_rate}%</p>
