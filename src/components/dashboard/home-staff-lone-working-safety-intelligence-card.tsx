...
export function HomeStaffLoneWorkingSafetyIntelligenceCard() {
  ...
  const __emptyState = d.lone_working_rating === "inadequate" && (d.lone_working_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <UserCheck className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-[--cs-risk]")} />
            <span className="text-slate-900 font-bold">Staff Lone Working Safety</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.lone_working_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.lone_working_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.lone_working_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.risk_assessment_rate >= 90 ? "bg-green-50" : d.risk_assessment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.risk_assessment_rate >= 90 ? "text-[--cs-success]" : d.risk_assessment_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.risk_assessment_rate}%</p><p className="text-[9px] text-muted-foreground">Risk Asst.</p></div>
            <div className={cn("text-center rounded-lg p-1.5", d.check_in_compliance_rate >= 90 ? "bg-green-50" : d.check_in_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.check_in_compliance_rate >= 90 ? "text-[--cs-success]" : d.check_in_compliance_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.check_in_compliance_rate}%</p><p className="text-[9px] text-muted-foreground">Check-in</p></div>
            <div className={cn("text-center rounded-lg p-1.5", d.safety_protocol_rate >= 90 ? "bg-green-50" : d.safety_protocol_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.safety_protocol_rate >= 90 ? "text-[--cs-success]" : d.safety_protocol_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.safety_protocol_rate}%</p><p className="text-[9px] text-muted-foreground">Protocol</p></div>
            <div className={cn("text-center rounded-lg p-1.5", d.communication_device_rate >= 90 ? "bg-green-50" : d.communication_device_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.communication_device_rate >= 90 ? "text-[--cs-success]" : d.communication_device_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.communication_device_rate}%</p><p className="text-[9px] text-muted-foreground">Comms</p></div>
            <div className={cn("text-center rounded-lg p-1.5", d.incident_reporting_rate >= 90 ? "bg-green-50" : d.incident_reporting_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.incident_reporting_rate >= 90 ? "text-[--cs-success]" : d.incident_reporting_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.incident_reporting_rate}%</p><p className="text-[9px] text-muted-foreground">Incidents</p></div>
            <div className={cn("text-center rounded-lg p-1.5", d.staff_confidence_rate >= 90 ? "bg-green-50" : d.staff_confidence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}><p className={cn("text-sm font-bold tabular-nums", d.staff_confidence_rate >= 90 ? "text-[--cs-success]" : d.staff_confidence_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.staff_confidence_rate}%</p><p className="text-[9px] text-muted-foreground">Confid.</p></div>
          </div>
        )}
