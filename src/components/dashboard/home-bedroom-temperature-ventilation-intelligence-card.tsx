...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Thermometer } from "lucide-react";
...
const RATING_STYLES: Record<BedroomTempRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeBedroomTemperatureVentilationIntelligenceCard() {
  const { data, isLoading } = useHomeBedroomTemperatureVentilationIntelligence();
  ...
  const __emptyState = d.temperature_rating === "inadequate" && (d.temperature_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      temperature_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.temperature_rating as BedroomTempRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.temperature_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Thermometer className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
        ...
        {d.temperature_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.temperature_monitoring_rate >= 90 ? "bg-green-50" : d.temperature_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.temperature_monitoring_rate >= 90 ? "text-green-600" : d.temperature_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.temperature_monitoring_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.heating_check_rate >= 90 ? "bg-green-50" : d.heating_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.heating_check_rate >= 90 ? "text-green-600" : d.heating_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.heating_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.window_compliance_rate >= 90 ? "bg-green-50" : d.window_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.window_compliance_rate >= 90 ? "text-green-600" : d.window_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.window_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.action_response_rate >= 90 ? "bg-green-50" : d.action_response_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.action_response_rate >= 90 ? "text-green-600" : d.action_response_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.action_response_rate}%</p>
