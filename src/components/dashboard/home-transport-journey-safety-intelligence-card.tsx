...
export function HomeTransportJourneySafetyIntelligenceCard() {
  const { data, isLoading } = useHomeTransportJourneySafetyIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.transport_rating] ?? RATING_STYLES.insufficient_data;
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-teal-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Car className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Transport & Journey Safety</span>
      ...
      <CardContent className="space-y-4">
        {d.transport_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.transport_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_journeys > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_journeys > 0 ? "text-green-600" : "text-amber-600")}>{d.total_journeys}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.driver_compliance_rate >= 90 ? "bg-green-50" : d.driver_compliance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.driver_compliance_rate >= 90 ? "text-green-600" : d.driver_compliance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.driver_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.vehicle_check_rate >= 90 ? "bg-green-50" : d.vehicle_check_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.vehicle_check_rate >= 90 ? "text-green-600" : d.vehicle_check_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.vehicle_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.incident_rate === 0 ? "bg-green-50" : d.incident_rate <= 10 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.incident_rate === 0 ? "text-green-600" : d.incident_rate <= 10 ? "text-amber-600" : "text-red-600")}>{d.incident_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.defect_free_rate >= 90 ? "bg-green-50" : d.defect_free_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.defect_free_rate >= 90 ? "text-green-600" : d.defect_free_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.defect_free_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Transport Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
