...
export function HomeBelongingPersonalPropertyIntelligenceCard() {
  const { data, isLoading } = useHomeBelongingPersonalPropertyIntelligence();
  ...
  const __emptyState = d.belonging_rating === "inadequate" && (d.belonging_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      belonging_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.belonging_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.belonging_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Package className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Belongings & Property</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.belonging_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.belonging_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.belonging_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.belonging_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_inventory > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_inventory > 0 ? "text-[--cs-success]" : "text-[--cs-warning]")}>{d.children_with_inventory}</p>
              <p className="text-[9px] text-muted-foreground">Inventories</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.clothing_choice_rate >= 85 ? "bg-green-50" : d.clothing_choice_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.clothing_choice_rate >= 85 ? "text-[--cs-success]" : d.clothing_choice_rate >= 60 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.clothing_choice_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Clothing Choice</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.hair_cultural_rate >= 85 ? "bg-green-50" : d.hair_cultural_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hair_cultural_rate >= 85 ? "text-[--cs-success]" : d.hair_cultural_rate >= 60 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.hair_cultural_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Cultural Hair</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.gift_personalisation_rate >= 85 ? "bg-green-50" : d.gift_personalisation_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.gift_personalisation_rate >= 85 ? "text-[--cs-success]" : d.gift_personalisation_rate >= 60 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.gift_personalisation_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Gift Quality</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.property_loss_rate <= 0.5 ? "bg-green-50" : d.property_loss_rate <= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.property_loss_rate <= 0.5 ? "text-[--cs-success]" : d.property_loss_rate <= 2 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.property_loss_rate}</p>
              <p className="text-[9px] text-muted-foreground">Loss/Child</p>
            </div>
          </div>
        )}
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Belongings Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
