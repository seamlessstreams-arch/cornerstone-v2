...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Cross } from "lucide-react";
...
const RATING_STYLES: Record<FirstAidKitRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeFirstAidKitMedicalSuppliesIntelligenceCard() {
  const { data, isLoading } = useHomeFirstAidKitMedicalSuppliesIntelligence();
  ...
  const __emptyState = d.first_aid_rating === "inadequate" && (d.first_aid_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      first_aid_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.first_aid_rating as FirstAidKitRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.first_aid_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Cross className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-red-600")} />
        ...
        {d.first_aid_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.kit_check_rate >= 90 ? "bg-green-50" : d.kit_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.kit_check_rate >= 90 ? "text-green-600" : d.kit_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.kit_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.stock_adequacy_rate >= 90 ? "bg-green-50" : d.stock_adequacy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.stock_adequacy_rate >= 90 ? "text-green-600" : d.stock_adequacy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.stock_adequacy_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.expiry_monitoring_rate >= 90 ? "bg-green-50" : d.expiry_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.expiry_monitoring_rate >= 90 ? "text-green-600" : d.expiry_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.expiry_monitoring_rate}%</p>
