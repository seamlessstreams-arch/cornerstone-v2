...
import { useHomeTaskActionCompletionIntelligence } from "@/hooks/use-home-task-action-completion-intelligence";
import type { TaskActionRating } from "@/lib/engines/home-task-action-completion-intelligence-engine";

const RATING_STYLES: Record<TaskActionRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeTaskActionCompletionIntelligenceCard() {
  const { data, isLoading } = useHomeTaskActionCompletionIntelligence();
  ...
  const __emptyState = d.task_rating === "inadequate" && (d.task_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.task_rating as TaskActionRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.task_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-emerald-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ListChecks className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Task Completion</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.task_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.task_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.task_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_tasks > 0 ? "bg-blue-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_tasks > 0 ? "text-blue-600" : "text-slate-600")}>{d.total_tasks}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.overdue_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.overdue_count === 0 ? "text-green-600" : "text-red-600")}>{d.overdue_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.on_time_rate >= 90 ? "bg-green-50" : d.on_time_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.on_time_rate >= 90 ? "text-green-600" : d.on_time_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.on_time_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.incident_follow_through >= 90 ? "bg-green-50" : d.incident_follow_through >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.incident_follow_through >= 90 ? "text-green-600" : d.incident_follow_through >= 70 ? "text-amber-600" : "text-red-600")}>{d.incident_follow_through}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.urgent_completion_rate >= 95 ? "bg-green-50" : d.urgent_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.urgent_completion_rate >= 95 ? "text-green-600" : d.urgent_completion_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.urgent_completion_rate}%</p>
