...
const RATING_STYLES: Record<PocketMoneyEquityRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePocketMoneyDistributionEquityIntelligenceCard() {
  const { data, isLoading } = useHomePocketMoneyDistributionEquityIntelligence();
  ...
  const __emptyState = d.equity_rating === "inadequate" && (d.equity_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      equity_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.equity_rating as PocketMoneyEquityRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.equity_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Coins className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Pocket Money Distribution Equity</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.equity_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.equity_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.equity_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.equitable_distribution_rate >= 90 ? "bg-green-50" : d.equitable_distribution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.equitable_distribution_rate >= 90 ? "text-green-600" : d.equitable_distribution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.equitable_distribution_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.timely_payment_rate >= 90 ? "bg-green-50" : d.timely_payment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.timely_payment_rate >= 90 ? "text-green-600" : d.timely_payment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.timely_payment_rate}%</p>
