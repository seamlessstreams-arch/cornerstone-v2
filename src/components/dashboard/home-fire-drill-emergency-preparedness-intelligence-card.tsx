...
import type { FireDrillRating } from "@/lib/engines/home-fire-drill-emergency-preparedness-intelligence-engine";

const RATING_STYLES: Record<FireDrillRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeFireDrillEmergencyPreparednessIntelligenceCard() {
  const { data, isLoading } = useHomeFireDrillEmergencyPreparednessIntelligence();
  ...
  const __emptyState = d.drill_rating === "inadequate" && (d.drill_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.drill_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.drill_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-red-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/30")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Flame className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-red-500")} />
            <span className="text-slate-900 font-bold">Fire Drill & Emergency Preparedness</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.drill_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.drill_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.drill_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.drill_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_drills > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_drills > 0 ? "text-green-600" : "text-amber-600")}>{d.total_drills}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.satisfactory_rate >= 90 ? "bg-green-50" : d.satisfactory_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.satisfactory_rate >= 90 ? "text-green-600" : d.satisfactory_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.satisfactory_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.all_present_rate >= 90 ? "bg-green-50" : d.all_present_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.all_present_rate >= 90 ? "text-green-600" : d.all_present_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.all_present_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_evacuation_time > 0 && d.average_evacuation_time <= 120 ? "bg-green-50" : d.average_evacuation_time <= 180 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_evacuation_time > 0 && d.average_evacuation_time <= 120 ? "text-green-600" : d.average_evacuation_time <= 180 ? "text-amber-600" : "text-red-600")}>{d.average_evacuation_time}s</p>
              <p className="text-[9px] text-muted-foreground">Avg Evac</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.drill_type_variety >= 4 ? "bg-green-50" : d.drill_type_variety >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.drill_type_variety >= 4 ? "text-green-600" : d.drill_type_variety >= 2 ? "text-amber-600" : "text-red-600")}>{d.drill_type_variety}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.failed_rate === 0 ? "bg-green-50" : d.failed_rate <= 10 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.failed_rate === 0 ? "text-green-600" : d.failed_rate <= 10 ? "text-amber-600" : "text-red-600")}>{d.failed_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Emergency Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
