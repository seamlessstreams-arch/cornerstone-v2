...
const RATING_STYLES: Record<PlacementStabilityDepthRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePlacementStabilityDepthIntelligenceCard() {
  const { data, isLoading } = useHomePlacementStabilityDepthIntelligence();
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-200")}>
      ...
      <CardContent className="space-y-4">
        {d.depth_rating === "insufficient_data" && <IntelligenceCardEmpty />}
