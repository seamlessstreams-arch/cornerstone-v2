...
import type { KeyWorkerRating } from "@/lib/engines/home-key-worker-relationship-quality-intelligence-engine";
...
export function HomeKeyWorkerRelationshipQualityIntelligenceCard() {
  const { data, isLoading } = useHomeKeyWorkerRelationshipQualityIntelligence();
  ...
  const __emptyState = d.key_worker_rating === "inadequate" && (d.key_worker_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      key_worker_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.key_worker_rating as KeyWorkerRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.key_worker_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <UserCheck className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
            <span className="text-slate-900 font-bold">Key Worker Relationship Quality</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.key_worker_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.key_worker_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.key_worker_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.allocation_coverage_rate >= 90 ? "bg-green-50" : d.allocation_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.allocation_coverage_rate >= 90 ? "text-green-600" : d.allocation_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.allocation_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.session_regularity_rate >= 90 ? "bg-green-50" : d.session_regularity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.session_regularity_rate >= 90 ? "text-green-600" : d.session_regularity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.session_regularity_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.continuity_rate >= 90 ? "bg-green-50" : d.continuity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.continuity_rate >= 90 ? "text-green-600" : d.continuity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.continuity_rate}%</p>
