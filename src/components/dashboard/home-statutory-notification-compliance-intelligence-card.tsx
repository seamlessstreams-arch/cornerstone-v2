...
export function HomeStatutoryNotificationComplianceIntelligenceCard() {
  ...
  const __emptyState = d.notification_rating === "inadequate" && (d.notification_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-neutral-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Bell className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-neutral-600")} />
        ...
        {d.notification_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_notifications > 0 ? "bg-blue-50" : "bg-green-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_notifications > 0 ? "text-blue-600" : "text-[--cs-success]")}>{d.total_notifications}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.missed_notifications === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.missed_notifications === 0 ? "text-[--cs-success]" : "text-[--cs-risk]")}>{d.missed_notifications}</p>
