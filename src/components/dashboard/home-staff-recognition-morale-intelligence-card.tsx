...
import type { StaffRecognitionRating } from "@/lib/engines/home-staff-recognition-morale-intelligence-engine";

const RATING_STYLES: Record<StaffRecognitionRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeStaffRecognitionMoraleIntelligenceCard() {
  const { data, isLoading } = useHomeStaffRecognitionMoraleIntelligence();
  ...
  const __emptyState = d.recognition_rating === "inadequate" && (d.recognition_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.recognition_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.recognition_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-pink-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Award className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Staff Recognition & Morale</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.recognition_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.recognition_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.recognition_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.recognition_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_recognitions > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_recognitions > 0 ? "text-green-600" : "text-amber-600")}>{d.total_recognitions}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_recognised_rate >= 80 ? "bg-green-50" : d.staff_recognised_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_recognised_rate >= 80 ? "text-green-600" : d.staff_recognised_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.staff_recognised_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_involvement_rate >= 30 ? "bg-green-50" : d.child_involvement_rate >= 15 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_involvement_rate >= 30 ? "text-green-600" : d.child_involvement_rate >= 15 ? "text-amber-600" : "text-red-600")}>{d.child_involvement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.public_celebration_rate >= 50 ? "bg-green-50" : d.public_celebration_rate >= 25 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.public_celebration_rate >= 50 ? "text-green-600" : d.public_celebration_rate >= 25 ? "text-amber-600" : "text-red-600")}>{d.public_celebration_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.recognition_type_variety >= 5 ? "bg-green-50" : d.recognition_type_variety >= 3 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.recognition_type_variety >= 5 ? "text-green-600" : d.recognition_type_variety >= 3 ? "text-amber-600" : "text-red-600")}>{d.recognition_type_variety}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Morale Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
