...
export function HomeCaseFileAuditQualityIntelligenceCard() {
  const { data, isLoading } = useHomeCaseFileAuditQualityIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.audit_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.audit_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <FolderSearch className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
            <span className="text-slate-900 font-bold">Case File Audit</span>
      ...
      <CardContent className="space-y-4">
        {d.audit_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.audit_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_audited > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_audited > 0 ? "text-green-600" : "text-amber-600")}>{d.children_audited}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_audit_score >= 85 ? "bg-green-50" : d.average_audit_score >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_audit_score >= 85 ? "text-green-600" : d.average_audit_score >= 70 ? "text-amber-600" : "text-red-600")}>{d.average_audit_score}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.green_rag_rate >= 80 ? "bg-green-50" : d.green_rag_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.green_rag_rate >= 80 ? "text-green-600" : d.green_rag_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.green_rag_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.policy_currency_rate >= 90 ? "bg-green-50" : d.policy_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.policy_currency_rate >= 90 ? "text-green-600" : d.policy_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.policy_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.ofsted_readiness_rate >= 90 ? "bg-green-50" : d.ofsted_readiness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.ofsted_readiness_rate >= 90 ? "text-green-600" : d.ofsted_readiness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.ofsted_readiness_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Audit Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
