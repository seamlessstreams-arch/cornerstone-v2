...
const DOMAIN_LABELS: Record<string, string> = {
  referral_management: "Referrals", exploitation_screening: "Exploitation", missing_episodes: "Missing",
  restrictive_practice: "Restraint", incident_management: "Incidents", disclosure_response: "Disclosures",
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-orange-200 border-2")}>
      ...
      <CardContent className="space-y-4">
        {d.safeguarding_rating !== "insufficient_data" && d.domain_scores.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-1.5">
            {d.domain_scores.map(ds => (
              <div key={ds.name} className={cn("text-center rounded-lg p-1.5", ds.at_risk ? "bg-red-50" : "bg-green-50")}>
                <p className={cn("text-sm font-bold tabular-nums", ds.at_risk ? "text-red-600" : "text-green-600")}>{ds.score}/{ds.max}</p>
