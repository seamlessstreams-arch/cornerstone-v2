...
export function HomeHealthWellbeingOversightIntelligenceCard() {
  const { data, isLoading } = useHomeHealthWellbeingOversightIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.wellbeing_rating as HealthWellbeingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.wellbeing_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-green-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-green-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Activity className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-green-600")} />
            <span className="text-slate-900 font-bold">Health & Wellbeing Oversight</span>
        ...
        {d.wellbeing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.health_assessment_compliance_rate >= 90 ? "bg-green-50" : d.health_assessment_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.health_assessment_compliance_rate >= 90 ? "text-green-600" : d.health_assessment_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.health_assessment_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.dental_check_rate >= 90 ? "bg-green-50" : d.dental_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dental_check_rate >= 90 ? "text-green-600" : d.dental_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dental_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.health_passport_currency_rate >= 90 ? "bg-green-50" : d.health_passport_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.health_passport_currency_rate >= 90 ? "text-green-600" : d.health_passport_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.health_passport_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.consent_form_rate >= 90 ? "bg-green-50" : d.consent_form_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.consent_form_rate >= 90 ? "text-green-600" : d.consent_form_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.consent_form_rate}%</p>
