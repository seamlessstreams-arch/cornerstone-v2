...
export function HomeParentPartnershipEngagementIntelligenceCard() {
  const { data, isLoading } = useHomeParentPartnershipEngagementIntelligence();
  ...
  const __emptyState = d.partnership_rating === "inadequate" && (d.partnership_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.partnership_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.partnership_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-green-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-green-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Heart className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-[--cs-success]")} />
            <span className="text-slate-900 font-bold">Parent & Family Partnership</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.partnership_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.partnership_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {d.partnership_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_contacts > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_contacts > 0 ? "text-[--cs-success]" : "text-[--cs-warning]")}>{d.total_contacts}</p>
              <p className="text-[9px] text-muted-foreground">Contacts</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.positive_engagement_rate >= 70 ? "bg-green-50" : d.positive_engagement_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.positive_engagement_rate >= 70 ? "text-[--cs-success]" : d.positive_engagement_rate >= 40 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.positive_engagement_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Positive</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_contact_rate >= 90 ? "bg-green-50" : d.children_with_contact_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_contact_rate >= 90 ? "text-[--cs-success]" : d.children_with_contact_rate >= 60 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.children_with_contact_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Coverage</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.sw_informed_rate >= 80 ? "bg-green-50" : d.sw_informed_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sw_informed_rate >= 80 ? "text-[--cs-success]" : d.sw_informed_rate >= 50 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.sw_informed_rate}%</p>
              <p className="text-[9px] text-muted-foreground">SW Told</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.contact_type_variety >= 4 ? "bg-green-50" : d.contact_type_variety >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.contact_type_variety >= 4 ? "text-[--cs-success]" : d.contact_type_variety >= 2 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.contact_type_variety}</p>
              <p className="text-[9px] text-muted-foreground">Methods</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.relationship_variety >= 3 ? "bg-green-50" : d.relationship_variety >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.relationship_variety >= 3 ? "text-[--cs-success]" : d.relationship_variety >= 2 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.relationship_variety}</p>
              <p className="text-[9px] text-muted-foreground">Family Types</p>
            </div>
          </div>
        )}
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Family Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
