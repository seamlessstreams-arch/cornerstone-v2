...
export function HomeCareEventQualityIntelligenceCard() {
  ...
  const __emptyState = d.event_rating === "inadequate" && (d.event_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-yellow-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <FileText className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-yellow-600")} />
        ...
        {d.event_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_events > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_events > 0 ? "text-[--cs-success]" : "text-[--cs-warning]")}>{d.total_events}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.recording_quality_rate >= 98 ? "bg-green-50" : d.recording_quality_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.recording_quality_rate >= 98 ? "text-[--cs-success]" : d.recording_quality_rate >= 90 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.recording_quality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.verification_rate >= 95 ? "bg-green-50" : d.verification_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.verification_rate >= 95 ? "text-[--cs-success]" : d.verification_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.verification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.routing_completion_rate >= 95 ? "bg-green-50" : d.routing_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.routing_completion_rate >= 95 ? "text-[--cs-success]" : d.routing_completion_rate >= 80 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.routing_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.audit_trail_rate >= 90 ? "bg-green-50" : d.audit_trail_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.audit_trail_rate >= 90 ? "text-[--cs-success]" : d.audit_trail_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.audit_trail_rate}%</p>
