...
export function HomeDbsRenewalStaffVettingIntelligenceCard() {
  const { data, isLoading } = useHomeDbsRenewalStaffVettingIntelligence();
  ...
  const __emptyState = d.vetting_rating === "inadequate" && (d.vetting_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.vetting_rating as DbsVettingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.vetting_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-600 border-2")} aria-label="Cara DBS Renewal &amp; Staff Vetting Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldCheck className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
        ...
        {d.vetting_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.dbs_currency_rate >= 90 ? "bg-green-50" : d.dbs_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dbs_currency_rate >= 90 ? "text-green-600" : d.dbs_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dbs_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.enhanced_dbs_rate >= 90 ? "bg-green-50" : d.enhanced_dbs_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.enhanced_dbs_rate >= 90 ? "text-green-600" : d.enhanced_dbs_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.enhanced_dbs_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.overseas_check_rate >= 90 ? "bg-green-50" : d.overseas_check_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.overseas_check_rate >= 90 ? "text-green-600" : d.overseas_check_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.overseas_check_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.barred_list_rate >= 90 ? "bg-green-50" : d.barred_list_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.barred_list_rate >= 90 ? "text-green-600" : d.barred_list_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.barred_list_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reference_verification_rate >= 90 ? "bg-green-50" : d.reference_verification_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reference_verification_rate >= 90 ? "text-green-600" : d.reference_verification_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.reference_verification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.renewal_timeliness_rate >= 90 ? "bg-green-50" : d.renewal_timeliness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.renewal_timeliness_rate >= 90 ? "text-green-600" : d.renewal_timeliness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.renewal_timeliness_rate}%</p>
