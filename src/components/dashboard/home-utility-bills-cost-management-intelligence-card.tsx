...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Receipt } from "lucide-react";
import { cn } from "@/lib/utils";
import { useHomeUtilityBillsCostManagementIntelligence } from "@/hooks/use-home-utility-bills-cost-management-intelligence";
import type { UtilityBillsRating } from "@/lib/engines/home-utility-bills-cost-management-intelligence-engine";

const RATING_STYLES: Record<UtilityBillsRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeUtilityBillsCostManagementIntelligenceCard() {
  const { data, isLoading } = useHomeUtilityBillsCostManagementIntelligence();
  ...
  const __emptyState = d.utility_rating === "inadequate" && (d.utility_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      utility_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.utility_rating as UtilityBillsRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.utility_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Receipt className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
        ...
        {d.utility_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.cost_monitoring_rate >= 90 ? "bg-green-50" : d.cost_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cost_monitoring_rate >= 90 ? "text-green-600" : d.cost_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cost_monitoring_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.bill_payment_rate >= 90 ? "bg-green-50" : d.bill_payment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.bill_payment_rate >= 90 ? "text-green-600" : d.bill_payment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.bill_payment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.budget_adherence_rate >= 90 ? "bg-green-50" : d.budget_adherence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.budget_adherence_rate >= 90 ? "text-green-600" : d.budget_adherence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.budget_adherence_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sustainability_rate >= 90 ? "bg-green-50" : d.sustainability_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sustainability_rate >= 90 ? "text-green-600" : d.sustainability_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.sustainability_rate}%</p>
