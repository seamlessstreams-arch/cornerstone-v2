...
import type { DiversityInclusionRating } from "@/lib/engines/home-diversity-inclusion-equality-intelligence-engine";

const RATING_STYLES: Record<DiversityInclusionRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeDiversityInclusionEqualityIntelligenceCard() {
  const { data, isLoading } = useHomeDiversityInclusionEqualityIntelligence();
  ...
  const __emptyState = d.diversity_rating === "inadequate" && (d.diversity_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.diversity_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.diversity_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-emerald-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Rainbow className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Diversity & Inclusion</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.diversity_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.diversity_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.diversity_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.diversity_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_cultural_plans > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_cultural_plans > 0 ? "text-green-600" : "text-amber-600")}>{d.children_with_cultural_plans}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.identity_affirmation_rate >= 90 ? "bg-green-50" : d.identity_affirmation_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.identity_affirmation_rate >= 90 ? "text-green-600" : d.identity_affirmation_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.identity_affirmation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.diversity_events_completed >= 4 ? "bg-green-50" : d.diversity_events_completed >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.diversity_events_completed >= 4 ? "text-green-600" : d.diversity_events_completed >= 2 ? "text-amber-600" : "text-red-600")}>{d.diversity_events_completed}</p>
              <p className="text-[9px] text-muted-foreground">Events</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.hate_incidents_total === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hate_incidents_total === 0 ? "text-green-600" : "text-red-600")}>{d.hate_incidents_total}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hate_resolution_rate >= 90 || d.hate_incidents_total === 0 ? "bg-green-50" : d.hate_resolution_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hate_resolution_rate >= 90 || d.hate_incidents_total === 0 ? "text-green-600" : d.hate_resolution_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.hate_resolution_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Diversity Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
