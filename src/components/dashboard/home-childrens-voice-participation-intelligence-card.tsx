...
export function HomeChildrensVoiceParticipationIntelligenceCard() {
  const { data, isLoading } = useHomeChildrensVoiceParticipationIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.voice_rating] ?? RATING_STYLES.insufficient_data;
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-lime-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-lime-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MessageCircle className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-lime-600")} />
            <span className="text-slate-900 font-bold">Children&apos;s Voice</span>
      ...
      <CardContent className="space-y-4">
        ...
        {d.voice_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.meeting_attendance_rate >= 80 ? "bg-green-50" : d.meeting_attendance_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.meeting_attendance_rate >= 80 ? "text-green-600" : d.meeting_attendance_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.meeting_attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.feedback_response_rate >= 90 ? "bg-green-50" : d.feedback_response_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.feedback_response_rate >= 90 ? "text-green-600" : d.feedback_response_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.feedback_response_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.positive_feedback_rate >= 80 ? "bg-green-50" : d.positive_feedback_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.positive_feedback_rate >= 80 ? "text-green-600" : d.positive_feedback_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.positive_feedback_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_friendly_policy_rate >= 80 ? "bg-green-50" : d.child_friendly_policy_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_friendly_policy_rate >= 80 ? "text-green-600" : d.child_friendly_policy_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.child_friendly_policy_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.expert_participation_count >= 3 ? "bg-green-50" : d.expert_participation_count >= 1 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.expert_participation_count >= 3 ? "text-green-600" : d.expert_participation_count >= 1 ? "text-amber-600" : "text-red-600")}>{d.expert_participation_count}</p>
              <p className="text-[9px] text-muted-foreground">Experts</p>
            </div>
          </div>
        )}
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Voice Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
