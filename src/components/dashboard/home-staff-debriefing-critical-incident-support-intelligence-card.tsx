...
export function HomeStaffDebriefingCriticalIncidentSupportIntelligenceCard() {
  const { data, isLoading } = useHomeStaffDebriefingCriticalIncidentSupportIntelligence();
  ...
  const __emptyState = d.debriefing_rating === "inadequate" && (d.debriefing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      debriefing_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.debriefing_rating as StaffDebriefingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.debriefing_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartHandshake className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
        ...
        <p className="text-[10px] text-muted-foreground mt-0.5 italic">Cara Staff Debriefing &amp; Critical Incident Support Intelligence</p>
      </CardHeader>
        ...
        {d.debriefing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.debriefing_completion_rate >= 90 ? "bg-green-50" : d.debriefing_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.debriefing_completion_rate >= 90 ? "text-green-600" : d.debriefing_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.debriefing_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.incident_support_rate >= 90 ? "bg-green-50" : d.incident_support_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.incident_support_rate >= 90 ? "text-green-600" : d.incident_support_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.incident_support_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.wellbeing_followup_rate >= 90 ? "bg-green-50" : d.wellbeing_followup_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.wellbeing_followup_rate >= 90 ? "text-green-600" : d.wellbeing_followup_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.wellbeing_followup_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.learning_extraction_rate >= 90 ? "bg-green-50" : d.learning_extraction_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.learning_extraction_rate >= 90 ? "text-green-600" : d.learning_extraction_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.learning_extraction_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.support_access_rate >= 90 ? "bg-green-50" : d.support_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.support_access_rate >= 90 ? "text-green-600" : d.support_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.support_access_rate}%</p>
