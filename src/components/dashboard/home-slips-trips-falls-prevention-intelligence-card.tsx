...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Footprints } from "lucide-react";
...
const RATING_STYLES: Record<SlipsTripsFallsRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSlipsTripsFallsPreventionIntelligenceCard() {
  const { data, isLoading } = useHomeSlipsTripsFallsPreventionIntelligence();
  ...
  const __emptyState = d.falls_prevention_rating === "inadequate" && (d.falls_prevention_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      falls_prevention_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.falls_prevention_rating as SlipsTripsFallsRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.falls_prevention_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Footprints className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">Slips, Trips & Falls Prevention</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.falls_prevention_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.falls_prevention_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.falls_prevention_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.flooring_condition_rate >= 90 ? "bg-green-50" : d.flooring_condition_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.flooring_condition_rate >= 90 ? "text-green-600" : d.flooring_condition_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.flooring_condition_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.wet_floor_protocol_rate >= 90 ? "bg-green-50" : d.wet_floor_protocol_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.wet_floor_protocol_rate >= 90 ? "text-green-600" : d.wet_floor_protocol_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.wet_floor_protocol_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.stairway_safety_rate >= 90 ? "bg-green-50" : d.stairway_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.stairway_safety_rate >= 90 ? "text-green-600" : d.stairway_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.stairway_safety_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.incident_learning_rate >= 90 ? "bg-green-50" : d.incident_learning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.incident_learning_rate >= 90 ? "text-green-600" : d.incident_learning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.incident_learning_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_awareness_rate >= 90 ? "bg-green-50" : d.staff_awareness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_awareness_rate >= 90 ? "text-green-600" : d.staff_awareness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.staff_awareness_rate}%</p>
