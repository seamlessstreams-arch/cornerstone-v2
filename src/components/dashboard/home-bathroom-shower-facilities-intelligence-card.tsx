...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, ShowerHead } from "lucide-react";
...
const RATING_STYLES: Record<BathroomShowerRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeBathroomShowerFacilitiesIntelligenceCard() {
  const { data, isLoading } = useHomeBathroomShowerFacilitiesIntelligence();
  ...
  const __emptyState = d.bathroom_rating === "inadequate" && (d.bathroom_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      bathroom_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.bathroom_rating as BathroomShowerRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.bathroom_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShowerHead className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
            <span className="text-slate-900 font-bold">Bathroom & Shower Facilities</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.bathroom_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.bathroom_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.bathroom_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.shower_availability_rate >= 90 ? "bg-green-50" : d.shower_availability_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.shower_availability_rate >= 90 ? "text-green-600" : d.shower_availability_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.shower_availability_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hot_water_safety_rate >= 90 ? "bg-green-50" : d.hot_water_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hot_water_safety_rate >= 90 ? "text-green-600" : d.hot_water_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hot_water_safety_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.privacy_rate >= 90 ? "bg-green-50" : d.privacy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.privacy_rate >= 90 ? "text-green-600" : d.privacy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.privacy_rate}%</p>
