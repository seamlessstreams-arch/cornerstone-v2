...
import { useHomeWeeklyPlannerActivityScheduleIntelligence } from "@/hooks/use-home-weekly-planner-activity-schedule-intelligence";
...
export function HomeWeeklyPlannerActivityScheduleIntelligenceCard() {
  const { data, isLoading } = useHomeWeeklyPlannerActivityScheduleIntelligence();
  ...
  const __emptyState = d.planner_rating === "inadequate" && (d.planner_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      planner_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.planner_rating as WeeklyPlannerRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.planner_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <CalendarDays className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
            <span className="text-slate-900 font-bold">Weekly Planner & Activity Schedule</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.planner_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.planner_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.planner_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.schedule_timeliness_rate >= 90 ? "bg-green-50" : d.schedule_timeliness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.schedule_timeliness_rate >= 90 ? "text-green-600" : d.schedule_timeliness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.schedule_timeliness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.activity_variety_rate >= 90 ? "bg-green-50" : d.activity_variety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.activity_variety_rate >= 90 ? "text-green-600" : d.activity_variety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.activity_variety_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_input_rate >= 90 ? "bg-green-50" : d.child_input_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_input_rate >= 90 ? "text-green-600" : d.child_input_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_input_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.communication_rate >= 90 ? "bg-green-50" : d.communication_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.communication_rate >= 90 ? "text-green-600" : d.communication_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.communication_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.adherence_rate >= 90 ? "bg-green-50" : d.adherence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.adherence_rate >= 90 ? "text-green-600" : d.adherence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.adherence_rate}%</p>
