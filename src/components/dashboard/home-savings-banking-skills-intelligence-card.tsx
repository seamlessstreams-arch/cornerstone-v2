...
import { useHomeSavingsBankingSkillsIntelligence } from "@/hooks/use-home-savings-banking-skills-intelligence";
...
export function HomeSavingsBankingSkillsIntelligenceCard() {
  const { data, isLoading } = useHomeSavingsBankingSkillsIntelligence();
  ...
  const __emptyState = d.savings_rating === "inadequate" && (d.savings_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      savings_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.savings_rating as SavingsBankingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.savings_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <PiggyBank className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Savings & Banking Skills</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.savings_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.savings_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.savings_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.savings_account_rate >= 90 ? "bg-green-50" : d.savings_account_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.savings_account_rate >= 90 ? "text-green-600" : d.savings_account_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.savings_account_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.banking_skills_rate >= 90 ? "bg-green-50" : d.banking_skills_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.banking_skills_rate >= 90 ? "text-green-600" : d.banking_skills_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.banking_skills_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.financial_goal_rate >= 90 ? "bg-green-50" : d.financial_goal_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.financial_goal_rate >= 90 ? "text-green-600" : d.financial_goal_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.financial_goal_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.money_confidence_rate >= 90 ? "bg-green-50" : d.money_confidence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.money_confidence_rate >= 90 ? "text-green-600" : d.money_confidence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.money_confidence_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.financial_independence_rate >= 90 ? "bg-green-50" : d.financial_independence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.financial_independence_rate >= 90 ? "text-green-600" : d.financial_independence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.financial_independence_rate}%</p>
