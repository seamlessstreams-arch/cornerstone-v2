...
import { useHomeYoungPersonDailyWellbeingIntelligence } from "@/hooks/use-home-young-person-daily-wellbeing-intelligence";
import type { YPDailyWellbeingRating } from "@/lib/engines/home-young-person-daily-wellbeing-intelligence-engine";

const RATING_STYLES: Record<YPDailyWellbeingRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeYoungPersonDailyWellbeingIntelligenceCard() {
  const { data, isLoading } = useHomeYoungPersonDailyWellbeingIntelligence();
  ...
  const __emptyState = d.wellbeing_rating === "inadequate" && (d.wellbeing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.wellbeing_rating as YPDailyWellbeingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.wellbeing_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-amber-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Smile className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-amber-600")} />
      ...
      <CardContent className="space-y-4">
        {d.wellbeing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.logs_last_30_days > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.logs_last_30_days > 0 ? "text-green-600" : "text-amber-600")}>{d.logs_last_30_days}</p>
              <p className="text-[9px] text-muted-foreground">Logs</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.average_mood_score >= 7 ? "bg-green-50" : d.average_mood_score >= 5 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_mood_score >= 7 ? "text-green-600" : d.average_mood_score >= 5 ? "text-amber-600" : "text-red-600")}>{d.average_mood_score}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.daily_coverage_rate >= 90 ? "bg-green-50" : d.daily_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.daily_coverage_rate >= 90 ? "text-green-600" : d.daily_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.daily_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.high_severity_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.high_severity_count === 0 ? "text-green-600" : "text-red-600")}>{d.high_severity_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_coverage_rate >= 100 ? "bg-green-50" : d.child_coverage_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_coverage_rate >= 100 ? "text-green-600" : d.child_coverage_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.child_coverage_rate}%</p>
