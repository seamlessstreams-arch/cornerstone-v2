...
import { useHomeWeightManagementHealthyEatingIntelligence } from "@/hooks/use-home-weight-management-healthy-eating-intelligence";
...
export function HomeWeightManagementHealthyEatingIntelligenceCard() {
  const { data, isLoading } = useHomeWeightManagementHealthyEatingIntelligence();
  ...
  const __emptyState = d.weight_rating === "inadequate" && (d.weight_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      weight_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.weight_rating as WeightManagementRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.weight_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-green-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Scale className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-green-600")} />
            <span className="text-slate-900 font-bold">Weight Management & Healthy Eating</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.weight_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.weight_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.weight_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.weight_monitoring_rate >= 90 ? "bg-green-50" : d.weight_monitoring_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.weight_monitoring_rate >= 90 ? "text-green-600" : d.weight_monitoring_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.weight_monitoring_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.bmi_tracking_rate >= 90 ? "bg-green-50" : d.bmi_tracking_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.bmi_tracking_rate >= 90 ? "text-green-600" : d.bmi_tracking_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.bmi_tracking_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.healthy_eating_rate >= 90 ? "bg-green-50" : d.healthy_eating_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.healthy_eating_rate >= 90 ? "text-green-600" : d.healthy_eating_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.healthy_eating_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.portion_control_rate >= 90 ? "bg-green-50" : d.portion_control_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.portion_control_rate >= 90 ? "text-green-600" : d.portion_control_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.portion_control_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.body_positivity_rate >= 90 ? "bg-green-50" : d.body_positivity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.body_positivity_rate >= 90 ? "text-green-600" : d.body_positivity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.body_positivity_rate}%</p>
