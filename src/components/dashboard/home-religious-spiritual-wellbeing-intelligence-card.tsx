...
export function HomeReligiousSpiritualWellbeingIntelligenceCard() {
  const { data, isLoading } = useHomeReligiousSpiritualWellbeingIntelligence();
  ...
  const __emptyState = d.spiritual_rating === "inadequate" && (d.spiritual_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      spiritual_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.spiritual_rating as SpiritualWellbeingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.spiritual_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Church className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-fuchsia-600")} />
            <span className="text-slate-900 font-bold">Religious & Spiritual Wellbeing</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.spiritual_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.spiritual_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.spiritual_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.faith_support_coverage_rate >= 90 ? "bg-green-50" : d.faith_support_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.faith_support_coverage_rate >= 90 ? "text-green-600" : d.faith_support_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.faith_support_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.spiritual_development_rate >= 90 ? "bg-green-50" : d.spiritual_development_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.spiritual_development_rate >= 90 ? "text-green-600" : d.spiritual_development_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.spiritual_development_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.dietary_accommodation_rate >= 90 ? "bg-green-50" : d.dietary_accommodation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dietary_accommodation_rate >= 90 ? "text-green-600" : d.dietary_accommodation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dietary_accommodation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.worship_access_rate >= 90 ? "bg-green-50" : d.worship_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.worship_access_rate >= 90 ? "text-green-600" : d.worship_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.worship_access_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.celebration_participation_rate >= 90 ? "bg-green-50" : d.celebration_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.celebration_participation_rate >= 90 ? "text-green-600" : d.celebration_participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.celebration_participation_rate}%</p>
