...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, PartyPopper } from "lucide-react";
...
export function HomeCulturalEventsCelebrationsIntelligenceCard() {
  const { data, isLoading } = useHomeCulturalEventsCelebrationsIntelligence();
  ...
  const __emptyState = d.cultural_rating === "inadequate" && (d.cultural_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      cultural_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.cultural_rating as CulturalEventsRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.cultural_rating === "inadequate";

  return (
    <Card aria-label="Cara Cultural Events &amp; Celebrations Intelligence" className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-fuchsia-600 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <PartyPopper className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-fuchsia-600")} />
        ...
        {d.cultural_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.diversity_celebration_rate >= 90 ? "bg-green-50" : d.diversity_celebration_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.diversity_celebration_rate >= 90 ? "text-green-600" : d.diversity_celebration_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.diversity_celebration_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.heritage_acknowledgement_rate >= 90 ? "bg-green-50" : d.heritage_acknowledgement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.heritage_acknowledgement_rate >= 90 ? "text-green-600" : d.heritage_acknowledgement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.heritage_acknowledgement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.festival_inclusion_rate >= 90 ? "bg-green-50" : d.festival_inclusion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.festival_inclusion_rate >= 90 ? "text-green-600" : d.festival_inclusion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.festival_inclusion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_led_rate >= 60 ? "bg-green-50" : d.child_led_rate >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_led_rate >= 60 ? "text-green-600" : d.child_led_rate >= 30 ? "text-amber-600" : "text-red-600")}>{d.child_led_rate}%</p>
