...
export function HomeSelfHarmSafetyPlanIntelligenceCard() {
  ...
  const __emptyState = d.plan_rating === "inadequate" && (d.plan_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.plan_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.plan_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-emerald-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldAlert className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Self-Harm Safety Plans</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.plan_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.plan_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.plan_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.plan_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.co_production_rate >= 80 ? "bg-green-50" : d.co_production_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.co_production_rate >= 80 ? "text-green-600" : d.co_production_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.co_production_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Co-Produced</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.warning_sign_coverage_rate >= 80 ? "bg-green-50" : d.warning_sign_coverage_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.warning_sign_coverage_rate >= 80 ? "text-green-600" : d.warning_sign_coverage_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.warning_sign_coverage_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Warnings</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.coping_strategy_rate >= 75 ? "bg-green-50" : d.coping_strategy_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.coping_strategy_rate >= 75 ? "text-green-600" : d.coping_strategy_rate >= 40 ? "text-amber-600" : "text-red-600")}>{d.coping_strategy_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Coping</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.contact_network_rate >= 80 ? "bg-green-50" : d.contact_network_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.contact_network_rate >= 80 ? "text-green-600" : d.contact_network_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.contact_network_rate}%</p>
