      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <MapPin className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
        ...
        {d.placement_journey_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.pre_admission.all_complete_rate >= 90 ? "text-green-600" : d.pre_admission.all_complete_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.pre_admission.total > 0 ? `${d.pre_admission.all_complete_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Pre-Admission</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.welcome_packs.personalised_rate >= 80 ? "text-green-600" : d.welcome_packs.personalised_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.welcome_packs.total > 0 ? `${d.welcome_packs.personalised_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Welcome Packs</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.objectives.on_track_rate >= 70 ? "text-green-600" : d.objectives.on_track_rate >= 40 ? "text-blue-600" : "text-red-600")}>{d.objectives.total > 0 ? `${d.objectives.on_track_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">On Track</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.return_interviews.within_24h_rate >= 90 ? "text-green-600" : d.return_interviews.within_24h_rate >= 60 ? "text-blue-600" : "text-red-600")}>{d.return_interviews.total > 0 ? `${d.return_interviews.within_24h_rate}%` : "—"}</p>
