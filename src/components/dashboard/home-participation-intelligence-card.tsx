...
const RATING_STYLES: Record<ParticipationRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeParticipationIntelligenceCard() {
  const { data, isLoading } = useHomeParticipationIntelligence();
  ...
  const __emptyState = d.participation_rating === "inadequate" && (d.participation_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-200")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            {d.participation_rating !== "insufficient_data" && (
              <span className="text-xs font-bold tabular-nums text-slate-600">{d.participation_score}%</span>
            )}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.participation_rating === "insufficient_data" && <IntelligenceCardEmpty />}
