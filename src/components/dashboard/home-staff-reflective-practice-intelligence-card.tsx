...
export function HomeStaffReflectivePracticeIntelligenceCard() {
  const { data, isLoading } = useHomeStaffReflectivePracticeIntelligence();
  ...
  const __emptyState = d.reflective_rating === "inadequate" && (d.reflective_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.reflective_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.reflective_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-lime-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-lime-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BookOpen className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-lime-600")} />
            <span className="text-slate-900 font-bold">Reflective Practice</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.reflective_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.reflective_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.reflective_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.reflective_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.staff_reflecting > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_reflecting > 0 ? "text-green-600" : "text-amber-600")}>{d.staff_reflecting}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reflection_sharing_rate >= 80 ? "bg-green-50" : d.reflection_sharing_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reflection_sharing_rate >= 80 ? "text-green-600" : d.reflection_sharing_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.reflection_sharing_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.shadowing_completion_rate >= 85 ? "bg-green-50" : d.shadowing_completion_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.shadowing_completion_rate >= 85 ? "text-green-600" : d.shadowing_completion_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.shadowing_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.meeting_attendance_rate >= 80 ? "bg-green-50" : d.meeting_attendance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.meeting_attendance_rate >= 80 ? "text-green-600" : d.meeting_attendance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.meeting_attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.themes_resolved_rate >= 80 ? "bg-green-50" : d.themes_resolved_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.themes_resolved_rate >= 80 ? "text-green-600" : d.themes_resolved_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.themes_resolved_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Practice Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
