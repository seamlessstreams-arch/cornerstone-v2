...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Flower } from "lucide-react";
...
import type { GardenOutdoorRating } from "@/lib/engines/home-garden-outdoor-space-maintenance-intelligence-engine";

const RATING_STYLES: Record<GardenOutdoorRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeGardenOutdoorSpaceMaintenanceIntelligenceCard() {
  ...
  const __emptyState = d.garden_rating === "inadequate" && (d.garden_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      garden_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.garden_rating as GardenOutdoorRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.garden_rating === "inadequate";
