...
import type { LifeStoryIdentityRating } from "@/lib/engines/home-life-story-identity-intelligence-engine";

const RATING_STYLES: Record<LifeStoryIdentityRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeLifeStoryIdentityIntelligenceCard() {
  const { data, isLoading } = useHomeLifeStoryIdentityIntelligence();
  ...
  const __emptyState = d.life_story_rating === "inadequate" && (d.life_story_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      life_story_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.life_story_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.life_story_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BookHeart className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-rose-500")} />
            <span className="text-slate-900">Life Story & Identity</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.life_story_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.life_story_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.life_story_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.life_story_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.life_stories.child_voice_rate >= 80 ? "text-[--cs-success]" : d.life_stories.child_voice_rate >= 50 ? "text-blue-600" : "text-[--cs-risk]")}>{d.life_stories.total > 0 ? `${d.life_stories.child_voice_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Story Voice</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.passports.child_authored_rate >= 80 ? "text-[--cs-success]" : d.passports.child_authored_rate >= 50 ? "text-blue-600" : "text-[--cs-risk]")}>{d.passports.total > 0 ? `${d.passports.child_authored_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Passports</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.friendships.high_isolation_count === 0 ? "text-[--cs-success]" : d.friendships.high_isolation_count <= 1 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.friendships.high_isolation_count}</p>
              <p className="text-[10px] text-muted-foreground">High Isolation</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.aspirations.child_chosen_rate >= 80 ? "text-[--cs-success]" : d.aspirations.child_chosen_rate >= 50 ? "text-blue-600" : "text-[--cs-risk]")}>{d.aspirations.total > 0 ? `${d.aspirations.child_chosen_rate}%` : "—"}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Identity Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
