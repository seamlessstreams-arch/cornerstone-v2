      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BookHeart className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-rose-500")} />
        ...
        {d.life_story_rating !== "insufficient_data" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.life_stories.child_voice_rate >= 80 ? "text-green-600" : d.life_stories.child_voice_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.life_stories.total > 0 ? `${d.life_stories.child_voice_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Story Voice</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.passports.child_authored_rate >= 80 ? "text-green-600" : d.passports.child_authored_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.passports.total > 0 ? `${d.passports.child_authored_rate}%` : "—"}</p>
              <p className="text-[10px] text-muted-foreground">Passports</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.friendships.high_isolation_count === 0 ? "text-green-600" : d.friendships.high_isolation_count <= 1 ? "text-amber-600" : "text-red-600")}>{d.friendships.high_isolation_count}</p>
              <p className="text-[10px] text-muted-foreground">High Isolation</p>
            </div>
            <div className="text-center rounded-lg bg-slate-50 p-2">
              <p className={cn("text-lg font-bold tabular-nums", d.aspirations.child_chosen_rate >= 80 ? "text-green-600" : d.aspirations.child_chosen_rate >= 50 ? "text-blue-600" : "text-red-600")}>{d.aspirations.total > 0 ? `${d.aspirations.child_chosen_rate}%` : "—"}</p>
