...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Laptop } from "lucide-react";
import { cn } from "@/lib/utils";
import { useHomeTechnologyDigitalInclusionIntelligence } from "@/hooks/use-home-technology-digital-inclusion-intelligence";
import type { DigitalInclusionRating } from "@/lib/engines/home-technology-digital-inclusion-intelligence-engine";

const RATING_STYLES: Record<DigitalInclusionRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeTechnologyDigitalInclusionIntelligenceCard() {
  const { data, isLoading } = useHomeTechnologyDigitalInclusionIntelligence();
  ...
  const __emptyState = d.digital_inclusion_rating === "inadequate" && (d.digital_inclusion_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      digital_inclusion_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.digital_inclusion_rating as DigitalInclusionRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.digital_inclusion_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-indigo-600 border-2")} aria-label="Cara Technology &amp; Digital Inclusion Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Laptop className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
            <span className="text-slate-900 font-bold">Technology & Digital Inclusion</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.digital_inclusion_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.digital_inclusion_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.digital_inclusion_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.device_access_rate >= 90 ? "bg-green-50" : d.device_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.device_access_rate >= 90 ? "text-green-600" : d.device_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.device_access_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Devices</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.digital_skills_rate >= 90 ? "bg-green-50" : d.digital_skills_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.digital_skills_rate >= 90 ? "text-green-600" : d.digital_skills_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.digital_skills_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.assistive_technology_rate >= 90 ? "bg-green-50" : d.assistive_technology_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.assistive_technology_rate >= 90 ? "text-green-600" : d.assistive_technology_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.assistive_technology_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Assistive</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.internet_safety_rate >= 90 ? "bg-green-50" : d.internet_safety_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.internet_safety_rate >= 90 ? "text-green-600" : d.internet_safety_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.internet_safety_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Safety</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.technology_learning_rate >= 90 ? "bg-green-50" : d.technology_learning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.technology_learning_rate >= 90 ? "text-green-600" : d.technology_learning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.technology_learning_rate}%</p>
