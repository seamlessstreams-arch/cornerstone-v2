...
// ══════════════════════════════════════════════════════════════════════════════
// CARA — PARENTAL RESPONSIBILITY CARD
...
import {
  AlertTriangle, Brain, ChevronRight, Loader2, UserCheck,
} from "lucide-react";
      ...
      <CardHeader className="pb-3 bg-slate-50/50">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900">Parental Responsibility</span>
          </CardTitle>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-4 gap-2">
          ...
          <div className={cn("text-center rounded-lg p-2", (d?.compliance?.plans_overdue_review ?? 0) > 0 ? "bg-amber-50" : "bg-green-50")}>
            <p className={cn("text-lg font-bold tabular-nums", (d?.compliance?.plans_overdue_review ?? 0) > 0 ? "text-[--cs-warning]" : "text-[--cs-success]")}>{d?.compliance?.plans_overdue_review ?? 0}</p>
        ...
        {insights.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-xs font-semibold flex items-center gap-1 text-purple-700">
              ...
              Cara Parental Responsibility Intelligence
            </p>
