...
export function HomeRewardsIncentivesManagementIntelligenceCard() {
  const { data, isLoading } = useHomeRewardsIncentivesManagementIntelligence();
  ...
  const __emptyState = d.rewards_rating === "inadequate" && (d.rewards_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      rewards_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.rewards_rating as RewardsIncentivesRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.rewards_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-yellow-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Trophy className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-yellow-600")} />
        ...
        {d.rewards_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.reward_fairness_rate >= 90 ? "bg-green-50" : d.reward_fairness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reward_fairness_rate >= 90 ? "text-green-600" : d.reward_fairness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.reward_fairness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.reinforcement_consistency_rate >= 90 ? "bg-green-50" : d.reinforcement_consistency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.reinforcement_consistency_rate >= 90 ? "text-green-600" : d.reinforcement_consistency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.reinforcement_consistency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.programme_effectiveness_rate >= 90 ? "bg-green-50" : d.programme_effectiveness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.programme_effectiveness_rate >= 90 ? "text-green-600" : d.programme_effectiveness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.programme_effectiveness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.equity_rate >= 90 ? "bg-green-50" : d.equity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.equity_rate >= 90 ? "text-green-600" : d.equity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.equity_rate}%</p>
