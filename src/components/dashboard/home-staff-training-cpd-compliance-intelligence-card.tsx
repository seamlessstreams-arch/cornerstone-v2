...
const RATING_STYLES: Record<TrainingComplianceRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeStaffTrainingCpdComplianceIntelligenceCard() {
  const { data, isLoading } = useHomeStaffTrainingCpdComplianceIntelligence();
  ...
  const __emptyState = d.training_rating === "inadequate" && (d.training_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.training_rating as TrainingComplianceRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.training_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <GraduationCap className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Staff Training & CPD Compliance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.training_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.training_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.training_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.mandatory_training_compliance_rate >= 90 ? "bg-green-50" : d.mandatory_training_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mandatory_training_compliance_rate >= 90 ? "text-green-600" : d.mandatory_training_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.mandatory_training_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.cpd_completion_rate >= 90 ? "bg-green-50" : d.cpd_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cpd_completion_rate >= 90 ? "text-green-600" : d.cpd_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cpd_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.training_needs_coverage_rate >= 90 ? "bg-green-50" : d.training_needs_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.training_needs_coverage_rate >= 90 ? "text-green-600" : d.training_needs_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.training_needs_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.qualification_currency_rate >= 90 ? "bg-green-50" : d.qualification_currency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.qualification_currency_rate >= 90 ? "text-green-600" : d.qualification_currency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.qualification_currency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.development_plan_coverage_rate >= 90 ? "bg-green-50" : d.development_plan_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.development_plan_coverage_rate >= 90 ? "text-green-600" : d.development_plan_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.development_plan_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.training_effectiveness_rate >= 90 ? "bg-green-50" : d.training_effectiveness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.training_effectiveness_rate >= 90 ? "text-green-600" : d.training_effectiveness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.training_effectiveness_rate}%</p>
