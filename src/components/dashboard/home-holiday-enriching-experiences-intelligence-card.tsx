...
import type { HolidayExperiencesRating } from "@/lib/engines/home-holiday-enriching-experiences-intelligence-engine";

const RATING_STYLES: Record<HolidayExperiencesRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeHolidayEnrichingExperiencesIntelligenceCard() {
  const { data, isLoading } = useHomeHolidayEnrichingExperiencesIntelligence();
  ...
  const __emptyState = d.experiences_rating === "inadequate" && (d.experiences_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.experiences_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.experiences_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-neutral-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-neutral-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Palmtree className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-neutral-600")} />
            <span className="text-slate-900 font-bold">Holidays & Enriching Experiences</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.experiences_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.experiences_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.experiences_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.experiences_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_holidays > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_holidays > 0 ? "text-green-600" : "text-amber-600")}>{d.total_holidays}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_choice_rate >= 90 ? "bg-green-50" : d.child_choice_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_choice_rate >= 90 ? "text-green-600" : d.child_choice_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_choice_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.photos_documented_rate >= 90 ? "bg-green-50" : d.photos_documented_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.photos_documented_rate >= 90 ? "text-green-600" : d.photos_documented_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.photos_documented_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.positive_anniversary_rate >= 90 ? "bg-green-50" : d.positive_anniversary_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.positive_anniversary_rate >= 90 ? "text-green-600" : d.positive_anniversary_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.positive_anniversary_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Experiences Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
