...
export function HomeRestraintPhysicalInterventionIntelligenceCard() {
  const { data, isLoading } = useHomeRestraintPhysicalInterventionIntelligence();
  ...
  const __emptyState = d.restraint_rating === "inadequate" && (d.restraint_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.restraint_rating as RestraintPhysicalInterventionRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.restraint_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-rose-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HandMetal className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-rose-600")} />
            <span className="text-slate-900 font-bold">Physical Intervention</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.restraint_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.restraint_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.restraint_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_restraints === 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_restraints === 0 ? "text-green-600" : "text-amber-600")}>{d.total_restraints}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.de_escalation_rate >= 90 ? "bg-green-50" : d.de_escalation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.de_escalation_rate >= 90 ? "text-green-600" : d.de_escalation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.de_escalation_rate}%</p>
              <p className="text-[9px] text-muted-foreground">De-Esc</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.team_teach_compliance_rate >= 95 ? "bg-green-50" : d.team_teach_compliance_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.team_teach_compliance_rate >= 95 ? "text-green-600" : d.team_teach_compliance_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.team_teach_compliance_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Trained</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.child_debrief_rate >= 85 ? "bg-green-50" : d.child_debrief_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_debrief_rate >= 85 ? "text-green-600" : d.child_debrief_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_debrief_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Debriefed</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.review_completion_rate >= 90 ? "bg-green-50" : d.review_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_completion_rate >= 90 ? "text-green-600" : d.review_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.review_completion_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Reviewed</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.body_map_rate >= 90 ? "bg-green-50" : d.body_map_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.body_map_rate >= 90 ? "text-green-600" : d.body_map_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.body_map_rate}%</p>
