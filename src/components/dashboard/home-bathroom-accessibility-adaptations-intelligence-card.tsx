...
const RATING_STYLES: Record<BathroomAccessibilityRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeBathroomAccessibilityAdaptationsIntelligenceCard() {
  const { data, isLoading } = useHomeBathroomAccessibilityAdaptationsIntelligence();
  ...
  const __emptyState = d.bath_access_rating === "inadequate" && (d.bath_access_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      bath_access_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.bath_access_rating as BathroomAccessibilityRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.bath_access_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-blue-600 border-2")} aria-label="Cara Bathroom Accessibility &amp; Adaptations Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Accessibility className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
            <span className="text-slate-900 font-bold">Bathroom Accessibility</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.bath_access_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.bath_access_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.bath_access_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.adaptation_adequacy_rate >= 90 ? "bg-green-50" : d.adaptation_adequacy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.adaptation_adequacy_rate >= 90 ? "text-green-600" : d.adaptation_adequacy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.adaptation_adequacy_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.grab_rail_rate >= 90 ? "bg-green-50" : d.grab_rail_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.grab_rail_rate >= 90 ? "text-green-600" : d.grab_rail_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.grab_rail_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.non_slip_rate >= 90 ? "bg-green-50" : d.non_slip_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.non_slip_rate >= 90 ? "text-green-600" : d.non_slip_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.non_slip_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.wheelchair_access_rate >= 90 ? "bg-green-50" : d.wheelchair_access_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.wheelchair_access_rate >= 90 ? "text-green-600" : d.wheelchair_access_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.wheelchair_access_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_modification_rate >= 90 ? "bg-green-50" : d.child_modification_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_modification_rate >= 90 ? "text-green-600" : d.child_modification_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_modification_rate}%</p>
