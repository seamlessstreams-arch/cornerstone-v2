...
export function HomeInfectionPreventionControlIntelligenceCard() {
  const { data, isLoading } = useHomeInfectionPreventionControlIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.infection_rating as InfectionPreventionRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.infection_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-lime-600 border-2")} aria-label="Cara Infection Prevention &amp; Control Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-lime-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldPlus className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-lime-600")} />
        ...
        {d.infection_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.hygiene_audit_compliance_rate >= 90 ? "bg-green-50" : d.hygiene_audit_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hygiene_audit_compliance_rate >= 90 ? "text-green-600" : d.hygiene_audit_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hygiene_audit_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.outbreak_management_rate >= 90 ? "bg-green-50" : d.outbreak_management_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.outbreak_management_rate >= 90 ? "text-green-600" : d.outbreak_management_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.outbreak_management_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.hand_hygiene_rate >= 90 ? "bg-green-50" : d.hand_hygiene_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hand_hygiene_rate >= 90 ? "text-green-600" : d.hand_hygiene_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hand_hygiene_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.cleaning_compliance_rate >= 90 ? "bg-green-50" : d.cleaning_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.cleaning_compliance_rate >= 90 ? "text-green-600" : d.cleaning_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.cleaning_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.immunisation_coverage_rate >= 90 ? "bg-green-50" : d.immunisation_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.immunisation_coverage_rate >= 90 ? "text-green-600" : d.immunisation_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.immunisation_coverage_rate}%</p>
