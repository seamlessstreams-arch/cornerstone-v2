...
export function HomeBoilerHeatingSystemServicingIntelligenceCard() {
  const { data, isLoading } = useHomeBoilerHeatingSystemServicingIntelligence();
  ...
  const __emptyState = d.boiler_rating === "inadequate" && (d.boiler_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      boiler_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.boiler_rating as BoilerHeatingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.boiler_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-orange-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Flame className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-orange-600")} />
            <span className="text-slate-900 font-bold">Boiler & Heating System Servicing</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.boiler_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.boiler_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.boiler_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.boiler_servicing_rate >= 90 ? "bg-green-50" : d.boiler_servicing_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.boiler_servicing_rate >= 90 ? "text-green-600" : d.boiler_servicing_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.boiler_servicing_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.radiator_maintenance_rate >= 90 ? "bg-green-50" : d.radiator_maintenance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.radiator_maintenance_rate >= 90 ? "text-green-600" : d.radiator_maintenance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.radiator_maintenance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.thermostat_calibration_rate >= 90 ? "bg-green-50" : d.thermostat_calibration_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.thermostat_calibration_rate >= 90 ? "text-green-600" : d.thermostat_calibration_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.thermostat_calibration_rate}%</p>
