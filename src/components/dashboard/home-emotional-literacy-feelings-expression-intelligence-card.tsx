...
export function HomeEmotionalLiteracyFeelingsExpressionIntelligenceCard() {
  const { data, isLoading } = useHomeEmotionalLiteracyFeelingsExpressionIntelligence();
  ...
  const __emptyState = d.emotional_literacy_rating === "inadequate" && (d.emotional_literacy_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      emotional_literacy_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.emotional_literacy_rating as EmotionalLiteracyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.emotional_literacy_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <SmilePlus className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-rose-600")} />
        ...
        {d.emotional_literacy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.emotion_identification_rate >= 90 ? "bg-green-50" : d.emotion_identification_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.emotion_identification_rate >= 90 ? "text-green-600" : d.emotion_identification_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.emotion_identification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.vocabulary_breadth_rate >= 90 ? "bg-green-50" : d.vocabulary_breadth_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.vocabulary_breadth_rate >= 90 ? "text-green-600" : d.vocabulary_breadth_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.vocabulary_breadth_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.expression_tool_rate >= 90 ? "bg-green-50" : d.expression_tool_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.expression_tool_rate >= 90 ? "text-green-600" : d.expression_tool_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.expression_tool_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.journal_engagement_rate >= 90 ? "bg-green-50" : d.journal_engagement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.journal_engagement_rate >= 90 ? "text-green-600" : d.journal_engagement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.journal_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_attunement_rate >= 90 ? "bg-green-50" : d.staff_attunement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_attunement_rate >= 90 ? "text-green-600" : d.staff_attunement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.staff_attunement_rate}%</p>
