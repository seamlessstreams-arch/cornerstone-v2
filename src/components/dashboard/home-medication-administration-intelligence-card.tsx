...
export function HomeMedicationAdministrationIntelligenceCard() {
  ...
  const __emptyState = d.medication_rating === "inadequate" && (d.medication_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.medication_rating as MedicationAdministrationRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.medication_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-purple-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Pill className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Medication Administration</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.medication_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.medication_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.medication_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_administrations > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_administrations > 0 ? "text-green-600" : "text-amber-600")}>{d.total_administrations}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.administration_rate >= 98 ? "bg-green-50" : d.administration_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.administration_rate >= 98 ? "text-green-600" : d.administration_rate >= 90 ? "text-amber-600" : "text-red-600")}>{d.administration_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.on_time_rate >= 95 ? "bg-green-50" : d.on_time_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.on_time_rate >= 95 ? "text-green-600" : d.on_time_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.on_time_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.witness_rate >= 95 ? "bg-green-50" : d.witness_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.witness_rate >= 95 ? "text-green-600" : d.witness_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.witness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.refusal_rate <= 5 ? "bg-green-50" : d.refusal_rate <= 15 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.refusal_rate <= 5 ? "text-green-600" : d.refusal_rate <= 15 ? "text-amber-600" : "text-red-600")}>{d.refusal_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_on_medication > 0 ? "bg-blue-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_on_medication > 0 ? "text-blue-600" : "text-slate-600")}>{d.children_on_medication}</p>
              <p className="text-[9px] text-muted-foreground">On Meds</p>
            </div>
          </div>
        )}
