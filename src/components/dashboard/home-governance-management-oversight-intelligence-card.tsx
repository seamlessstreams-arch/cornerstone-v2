      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Building2 className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-cyan-600")} />
        ...
        {d.governance_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_walkrounds >= 4 ? "bg-green-50" : d.total_walkrounds >= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_walkrounds >= 4 ? "text-[--cs-success]" : d.total_walkrounds >= 2 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.total_walkrounds}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.total_governance_meetings >= 3 ? "bg-green-50" : d.total_governance_meetings >= 1 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_governance_meetings >= 3 ? "text-[--cs-success]" : d.total_governance_meetings >= 1 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.total_governance_meetings}</p>
