...
export function HomeChildWellbeingCompositeCard() {
  const { data, isLoading } = useHomeChildWellbeingComposite();
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Heart className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Child Wellbeing</span>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <div className="text-center rounded-lg bg-green-50 p-2">
                <p className="text-lg font-bold tabular-nums text-green-600">{d.children_flourishing}</p>
                <p className="text-[10px] text-muted-foreground">Flourishing</p>
              </div>
              <div className="text-center rounded-lg bg-blue-50 p-2">
                <p className="text-lg font-bold tabular-nums text-blue-600">{d.children_stable}</p>
                <p className="text-[10px] text-muted-foreground">Stable</p>
              </div>
              <div className="text-center rounded-lg bg-amber-50 p-2">
                <p className="text-lg font-bold tabular-nums text-amber-600">{d.children_struggling}</p>
                <p className="text-[10px] text-muted-foreground">Struggling</p>
              </div>
              <div className="text-center rounded-lg bg-red-50 p-2">
                <p className="text-lg font-bold tabular-nums text-red-600">{d.children_crisis}</p>
                <p className="text-[10px] text-muted-foreground">Crisis</p>
              </div>
            </div>
          </>
        )}
