...
export function HomeTherapeuticWellbeingImpactIntelligenceCard() {
  const { data, isLoading } = useHomeTherapeuticWellbeingImpactIntelligence();
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-fuchsia-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartPulse className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-fuchsia-600")} />
      ...
      <CardContent className="space-y-4">
        {d.wellbeing_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.wellbeing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_therapeutic_plans > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_therapeutic_plans > 0 ? "text-green-600" : "text-amber-600")}>{d.children_with_therapeutic_plans}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_wellbeing_score >= 7 ? "bg-green-50" : d.average_wellbeing_score >= 5 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_wellbeing_score >= 7 ? "text-green-600" : d.average_wellbeing_score >= 5 ? "text-amber-600" : "text-red-600")}>{d.average_wellbeing_score}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.improving_trend_rate >= 70 ? "bg-green-50" : d.improving_trend_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.improving_trend_rate >= 70 ? "text-green-600" : d.improving_trend_rate >= 40 ? "text-amber-600" : "text-red-600")}>{d.improving_trend_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.self_soothing_coverage_rate >= 80 ? "bg-green-50" : d.self_soothing_coverage_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.self_soothing_coverage_rate >= 80 ? "text-green-600" : d.self_soothing_coverage_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.self_soothing_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.grief_support_rate >= 90 ? "bg-green-50" : d.grief_support_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.grief_support_rate >= 90 ? "text-green-600" : d.grief_support_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.grief_support_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Wellbeing Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
