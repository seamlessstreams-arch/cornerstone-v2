...
export function HomeInfectionControlHealthSafetyIntelligenceCard() {
  ...
  const __emptyState = d.infection_rating === "inadequate" && (d.infection_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.infection_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.infection_rating === "inadequate";
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-emerald-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldPlus className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
            <span className="text-slate-900 font-bold">Infection & Health Safety</span>
      ...
      <CardContent className="space-y-4">
        {d.infection_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.infection_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.active_infections === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_infections === 0 ? "text-green-600" : "text-red-600")}>{d.active_infections}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.mar_accuracy_rate >= 95 ? "bg-green-50" : d.mar_accuracy_rate >= 90 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.mar_accuracy_rate >= 95 ? "text-green-600" : d.mar_accuracy_rate >= 90 ? "text-amber-600" : "text-red-600")}>{d.mar_accuracy_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.med_training_rate >= 90 ? "bg-green-50" : d.med_training_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.med_training_rate >= 90 ? "text-green-600" : d.med_training_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.med_training_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.first_aid_coverage >= 50 ? "bg-green-50" : d.first_aid_coverage >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.first_aid_coverage >= 50 ? "text-green-600" : d.first_aid_coverage >= 30 ? "text-amber-600" : "text-red-600")}>{d.first_aid_coverage}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.infection_resolution_rate >= 90 ? "bg-green-50" : d.infection_resolution_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.infection_resolution_rate >= 90 ? "text-green-600" : d.infection_resolution_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.infection_resolution_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Health Safety Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
