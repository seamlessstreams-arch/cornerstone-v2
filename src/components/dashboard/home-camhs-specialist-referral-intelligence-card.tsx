...
export function HomeCamhsSpecialistReferralIntelligenceCard() {
  const { data, isLoading } = useHomeCamhsSpecialistReferralIntelligence();
  ...
  const __emptyState = d.camhs_rating === "inadequate" && (d.camhs_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      camhs_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.camhs_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.camhs_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-teal-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HeartPulse className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">CAMHS & Specialist Referrals</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.camhs_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.camhs_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.camhs_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.camhs_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.active_referrals > 0 ? "bg-green-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_referrals > 0 ? "text-green-600" : "text-slate-600")}>{d.active_referrals}</p>
              <p className="text-[9px] text-muted-foreground">Active</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.children_waiting === 0 ? "bg-green-50" : d.children_waiting <= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_waiting === 0 ? "text-green-600" : d.children_waiting <= 2 ? "text-amber-600" : "text-red-600")}>{d.children_waiting}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.appointment_attendance_rate >= 85 ? "bg-green-50" : d.appointment_attendance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.appointment_attendance_rate >= 85 ? "text-green-600" : d.appointment_attendance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.appointment_attendance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.emergency_response_rate >= 100 ? "bg-green-50" : d.emergency_response_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.emergency_response_rate >= 100 ? "text-green-600" : d.emergency_response_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.emergency_response_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.specialist_coverage_rate >= 80 ? "bg-green-50" : d.specialist_coverage_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.specialist_coverage_rate >= 80 ? "text-green-600" : d.specialist_coverage_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.specialist_coverage_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara CAMHS Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
