...
export function HomeAgencyStaffManagementIntelligenceCard() {
  const { data, isLoading } = useHomeAgencyStaffManagementIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.agency_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.agency_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-zinc-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-zinc-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-zinc-600")} />
            <span className="text-slate-900 font-bold">Agency Staff Management</span>
      ...
      <CardContent className="space-y-4">
        {d.agency_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.agency_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_agency_shifts > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_agency_shifts > 0 ? "text-green-600" : "text-amber-600")}>{d.total_agency_shifts}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.vetting_compliance_rate >= 90 ? "bg-green-50" : d.vetting_compliance_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.vetting_compliance_rate >= 90 ? "text-green-600" : d.vetting_compliance_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.vetting_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.induction_completion_rate >= 90 ? "bg-green-50" : d.induction_completion_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.induction_completion_rate >= 90 ? "text-green-600" : d.induction_completion_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.induction_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.safeguarding_briefing_rate >= 90 ? "bg-green-50" : d.safeguarding_briefing_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safeguarding_briefing_rate >= 90 ? "text-green-600" : d.safeguarding_briefing_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.safeguarding_briefing_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.concerns_flagged === 0 ? "bg-green-50" : d.concerns_flagged <= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.concerns_flagged === 0 ? "text-green-600" : d.concerns_flagged <= 2 ? "text-amber-600" : "text-red-600")}>{d.concerns_flagged}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Agency Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
