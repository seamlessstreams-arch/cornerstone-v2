...
export function HomeCommunityIntegrationVolunteeringIntelligenceCard() {
  const { data, isLoading } = useHomeCommunityIntegrationVolunteeringIntelligence();
  ...
  const __emptyState = d.community_rating === "inadequate" && (d.community_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      community_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.community_rating as CommunityIntegrationRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.community_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-emerald-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <HandHelping className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-emerald-600")} />
        ...
        {d.community_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.community_participation_rate >= 90 ? "bg-green-50" : d.community_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.community_participation_rate >= 90 ? "text-green-600" : d.community_participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.community_participation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.volunteering_rate >= 90 ? "bg-green-50" : d.volunteering_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.volunteering_rate >= 90 ? "text-green-600" : d.volunteering_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.volunteering_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.social_inclusion_rate >= 90 ? "bg-green-50" : d.social_inclusion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.social_inclusion_rate >= 90 ? "text-green-600" : d.social_inclusion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.social_inclusion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.neighbourhood_relation_rate >= 90 ? "bg-green-50" : d.neighbourhood_relation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.neighbourhood_relation_rate >= 90 ? "text-green-600" : d.neighbourhood_relation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.neighbourhood_relation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.local_service_rate >= 90 ? "bg-green-50" : d.local_service_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.local_service_rate >= 90 ? "text-green-600" : d.local_service_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.local_service_rate}%</p>
