      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Shield className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
        ...
        {d.safeguarding_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.active_risks === 0 ? "bg-green-50" : d.active_risks <= 2 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_risks === 0 ? "text-green-600" : d.active_risks <= 2 ? "text-amber-600" : "text-red-600")}>{d.active_risks}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.protective_actions_rate >= 90 ? "bg-green-50" : d.protective_actions_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.protective_actions_rate >= 90 ? "text-green-600" : d.protective_actions_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.protective_actions_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.resolution_rate >= 60 ? "bg-green-50" : d.resolution_rate >= 30 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.resolution_rate >= 60 ? "text-green-600" : d.resolution_rate >= 30 ? "text-amber-600" : "text-red-600")}>{d.resolution_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.review_overdue_count === 0 ? "bg-green-50" : d.review_overdue_count <= 1 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_overdue_count === 0 ? "text-green-600" : d.review_overdue_count <= 1 ? "text-amber-600" : "text-red-600")}>{d.review_overdue_count}</p>
