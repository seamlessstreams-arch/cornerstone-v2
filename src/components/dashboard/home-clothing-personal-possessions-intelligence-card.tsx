...
export function HomeClothingPersonalPossessionsIntelligenceCard() {
  const { data, isLoading } = useHomeClothingPersonalPossessionsIntelligence();
  ...
  const __emptyState = d.clothing_rating === "inadequate" && (d.clothing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      clothing_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.clothing_rating as ClothingPossessionsRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.clothing_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Shirt className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Clothing & Personal Possessions</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.clothing_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.clothing_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.clothing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.allowance_utilisation_rate >= 90 ? "bg-green-50" : d.allowance_utilisation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.allowance_utilisation_rate >= 90 ? "text-green-600" : d.allowance_utilisation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.allowance_utilisation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.wardrobe_adequacy_rate >= 90 ? "bg-green-50" : d.wardrobe_adequacy_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.wardrobe_adequacy_rate >= 90 ? "text-green-600" : d.wardrobe_adequacy_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.wardrobe_adequacy_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.inventory_completeness_rate >= 90 ? "bg-green-50" : d.inventory_completeness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.inventory_completeness_rate >= 90 ? "text-green-600" : d.inventory_completeness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.inventory_completeness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.request_fulfilment_rate >= 90 ? "bg-green-50" : d.request_fulfilment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.request_fulfilment_rate >= 90 ? "text-green-600" : d.request_fulfilment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.request_fulfilment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.possession_safeguarding_rate >= 90 ? "bg-green-50" : d.possession_safeguarding_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.possession_safeguarding_rate >= 90 ? "text-green-600" : d.possession_safeguarding_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.possession_safeguarding_rate}%</p>
