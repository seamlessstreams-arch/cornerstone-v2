...
export function HomePlacementStabilityPermanenceIntelligenceCard() {
  const { data, isLoading } = useHomePlacementStabilityPermanenceIntelligence();
  ...
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.stability_rating as PlacementStabilityRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.stability_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Home className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
            <span className="text-slate-900 font-bold">Placement Stability & Permanence</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.stability_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.stability_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.stability_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.placement_stability_rate >= 90 ? "bg-green-50" : d.placement_stability_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.placement_stability_rate >= 90 ? "text-green-600" : d.placement_stability_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.placement_stability_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.stability_meeting_rate >= 90 ? "bg-green-50" : d.stability_meeting_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.stability_meeting_rate >= 90 ? "text-green-600" : d.stability_meeting_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.stability_meeting_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.disruption_prevention_rate >= 90 ? "bg-green-50" : d.disruption_prevention_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.disruption_prevention_rate >= 90 ? "text-green-600" : d.disruption_prevention_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.disruption_prevention_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Prevent.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.planned_ending_rate >= 90 ? "bg-green-50" : d.planned_ending_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.planned_ending_rate >= 90 ? "text-green-600" : d.planned_ending_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.planned_ending_rate}%</p>
