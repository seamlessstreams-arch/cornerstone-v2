...
import type { LadoAllegationRating } from "@/lib/engines/home-lado-allegation-management-intelligence-engine";

const RATING_STYLES: Record<LadoAllegationRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeLadoAllegationManagementIntelligenceCard() {
  const { data, isLoading } = useHomeLadoAllegationManagementIntelligence();
  ...
  const d = data?.data;
  if (!d) return null;
  const ratingStyle = RATING_STYLES[d.lado_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.lado_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-amber-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ShieldAlert className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-amber-600")} />
            <span className="text-slate-900 font-bold">LADO & Allegations</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.lado_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.lado_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.lado_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_referrals === 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_referrals === 0 ? "text-green-600" : "text-amber-600")}>{d.total_referrals}</p>
              <p className="text-[9px] text-muted-foreground">Referrals</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.open_referrals === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.open_referrals === 0 ? "text-green-600" : "text-red-600")}>{d.open_referrals}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.ofsted_notification_rate >= 95 ? "bg-green-50" : d.ofsted_notification_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.ofsted_notification_rate >= 95 ? "text-green-600" : d.ofsted_notification_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.ofsted_notification_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_days_to_close <= 30 ? "bg-green-50" : d.average_days_to_close <= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_days_to_close <= 30 ? "text-green-600" : d.average_days_to_close <= 60 ? "text-amber-600" : "text-red-600")}>{d.average_days_to_close}d</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> ARIA LADO Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
