...
export function HomeMenstruationPubertySupportIntelligenceCard() {
  const { data, isLoading } = useHomeMenstruationPubertySupportIntelligence();
  ...
  const __emptyState = d.puberty_rating === "inadequate" && (d.puberty_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      puberty_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.puberty_rating as MenstruationPubertyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.puberty_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="text-slate-900 font-bold">Menstruation & Puberty Support</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.puberty_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.puberty_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.puberty_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.puberty_education_rate >= 90 ? "bg-green-50" : d.puberty_education_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.puberty_education_rate >= 90 ? "text-green-600" : d.puberty_education_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.puberty_education_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.menstruation_support_rate >= 90 ? "bg-green-50" : d.menstruation_support_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.menstruation_support_rate >= 90 ? "text-green-600" : d.menstruation_support_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.menstruation_support_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.dignity_care_rate >= 90 ? "bg-green-50" : d.dignity_care_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dignity_care_rate >= 90 ? "text-green-600" : d.dignity_care_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dignity_care_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.body_confidence_rate >= 90 ? "bg-green-50" : d.body_confidence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.body_confidence_rate >= 90 ? "text-green-600" : d.body_confidence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.body_confidence_rate}%</p>
