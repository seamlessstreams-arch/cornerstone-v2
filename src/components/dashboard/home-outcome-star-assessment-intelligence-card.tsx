...
export function HomeOutcomeStarAssessmentIntelligenceCard() {
  const { data, isLoading } = useHomeOutcomeStarAssessmentIntelligence();
  ...
  const __emptyState = d.star_rating === "inadequate" && (d.star_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.star_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.star_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-violet-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Star className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
      ...
      <CardContent className="space-y-4">
        {d.star_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.star_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_assessed_rate >= 80 ? "bg-green-50" : d.children_assessed_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_assessed_rate >= 80 ? "text-green-600" : d.children_assessed_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.children_assessed_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.repeat_assessment_rate >= 70 ? "bg-green-50" : d.repeat_assessment_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.repeat_assessment_rate >= 70 ? "text-green-600" : d.repeat_assessment_rate >= 40 ? "text-amber-600" : "text-red-600")}>{d.repeat_assessment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.improvement_rate >= 70 ? "bg-green-50" : d.improvement_rate >= 40 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.improvement_rate >= 70 ? "text-green-600" : d.improvement_rate >= 40 ? "text-amber-600" : "text-red-600")}>{d.improvement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.action_plan_rate >= 80 ? "bg-green-50" : d.action_plan_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.action_plan_rate >= 80 ? "text-green-600" : d.action_plan_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.action_plan_rate}%</p>
