...
import type { SiblingContactRating } from "@/lib/engines/home-sibling-contact-protocol-intelligence-engine";
...
export function HomeSiblingContactProtocolIntelligenceCard() {
  const { data, isLoading } = useHomeSiblingContactProtocolIntelligence();
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Sibling Contact Protocol</span>
      ...
      <CardContent className="space-y-4">
        ...
        {d.contact_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_protocols > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_protocols > 0 ? "text-green-600" : "text-amber-600")}>{d.total_protocols}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_protocol_rate >= 80 ? "bg-green-50" : d.children_with_protocol_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_protocol_rate >= 80 ? "text-green-600" : d.children_with_protocol_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.children_with_protocol_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.regular_contact_rate >= 80 ? "bg-green-50" : d.regular_contact_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.regular_contact_rate >= 80 ? "text-green-600" : d.regular_contact_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.regular_contact_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.agreed_plan_rate >= 90 ? "bg-green-50" : d.agreed_plan_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.agreed_plan_rate >= 90 ? "text-green-600" : d.agreed_plan_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.agreed_plan_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.celebration_plan_rate >= 80 ? "bg-green-50" : d.celebration_plan_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.celebration_plan_rate >= 80 ? "text-green-600" : d.celebration_plan_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.celebration_plan_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.review_current_rate >= 80 ? "bg-green-50" : d.review_current_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_current_rate >= 80 ? "text-green-600" : d.review_current_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.review_current_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Sibling Contact Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
