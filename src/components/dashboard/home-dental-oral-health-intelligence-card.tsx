...
export function HomeDentalOralHealthIntelligenceCard() {
  const { data, isLoading } = useHomeDentalOralHealthIntelligence();
  ...
  const __emptyState = d.dental_rating === "inadequate" && (d.dental_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      dental_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.dental_rating as DentalOralHealthRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.dental_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-cyan-600 border-2")} aria-label="Cara Dental &amp; Oral Health Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Smile className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
        ...
        {d.dental_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.checkup_compliance_rate >= 90 ? "bg-green-50" : d.checkup_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.checkup_compliance_rate >= 90 ? "text-green-600" : d.checkup_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.checkup_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.oral_hygiene_rate >= 90 ? "bg-green-50" : d.oral_hygiene_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.oral_hygiene_rate >= 90 ? "text-green-600" : d.oral_hygiene_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.oral_hygiene_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.treatment_completion_rate >= 90 ? "bg-green-50" : d.treatment_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.treatment_completion_rate >= 90 ? "text-green-600" : d.treatment_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.treatment_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.orthodontic_compliance_rate >= 90 ? "bg-green-50" : d.orthodontic_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.orthodontic_compliance_rate >= 90 ? "text-green-600" : d.orthodontic_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.orthodontic_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.anxiety_support_rate >= 90 ? "bg-green-50" : d.anxiety_support_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.anxiety_support_rate >= 90 ? "text-green-600" : d.anxiety_support_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.anxiety_support_rate}%</p>
