...
export function HomeSharpsDisposalHazardousWasteIntelligenceCard() {
  ...
  const __emptyState = d.sharps_rating === "inadequate" && (d.sharps_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      sharps_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-red-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Syringe className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-[--cs-risk]")} />
