...
export function HomeStaffRotaAdequateStaffingIntelligenceCard() {
  const { data, isLoading } = useHomeStaffRotaAdequateStaffingIntelligence();
  ...
  const __emptyState = d.staffing_rating === "inadequate" && (d.staffing_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      staffing_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.staffing_rating as StaffRotaRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.staffing_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ClipboardList className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Staff Rota & Adequate Staffing</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.staffing_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.staffing_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.staffing_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.shift_coverage_rate >= 90 ? "bg-green-50" : d.shift_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.shift_coverage_rate >= 90 ? "text-green-600" : d.shift_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.shift_coverage_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Coverage</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.ratio_compliance_rate >= 90 ? "bg-green-50" : d.ratio_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.ratio_compliance_rate >= 90 ? "text-green-600" : d.ratio_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.ratio_compliance_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Ratios</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.overtime_rate <= 20 ? "bg-green-50" : d.overtime_rate <= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.overtime_rate <= 20 ? "text-green-600" : d.overtime_rate <= 50 ? "text-amber-600" : "text-red-600")}>{d.overtime_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Overtime</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.agency_usage_rate <= 20 ? "bg-green-50" : d.agency_usage_rate <= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.agency_usage_rate <= 20 ? "text-green-600" : d.agency_usage_rate <= 50 ? "text-amber-600" : "text-red-600")}>{d.agency_usage_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Agency</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.rota_planning_rate >= 90 ? "bg-green-50" : d.rota_planning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.rota_planning_rate >= 90 ? "text-green-600" : d.rota_planning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.rota_planning_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Rota</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.staff_satisfaction_rate >= 90 ? "bg-green-50" : d.staff_satisfaction_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_satisfaction_rate >= 90 ? "text-green-600" : d.staff_satisfaction_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.staff_satisfaction_rate}%</p>
