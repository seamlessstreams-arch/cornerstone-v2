...
import type { ProfessionalNetworkRating } from "@/lib/engines/home-professional-network-intelligence-engine";

const RATING_STYLES: Record<ProfessionalNetworkRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeProfessionalNetworkIntelligenceCard() {
  const { data, isLoading } = useHomeProfessionalNetworkIntelligence();
  ...
  const __emptyState = d.network_rating === "inadequate" && (d.network_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.network_rating as ProfessionalNetworkRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.network_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-indigo-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-indigo-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Network className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-indigo-600")} />
            <span className="text-slate-900 font-bold">Professional Network</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.network_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.network_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.network_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_contacts > 0 ? "bg-blue-50" : "bg-slate-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_contacts > 0 ? "text-blue-600" : "text-slate-600")}>{d.total_contacts}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.meeting_completion_rate >= 90 ? "bg-green-50" : d.meeting_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.meeting_completion_rate >= 90 ? "text-green-600" : d.meeting_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.meeting_completion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.role_diversity >= 4 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.role_diversity >= 4 ? "text-green-600" : "text-amber-600")}>{d.role_diversity}</p>
