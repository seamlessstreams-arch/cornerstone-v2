...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, ScrollText } from "lucide-react";
...
import type { PolicyReviewRating } from "@/lib/engines/home-policy-review-cycle-compliance-intelligence-engine";

const RATING_STYLES: Record<PolicyReviewRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomePolicyReviewCycleComplianceIntelligenceCard() {
  const { data, isLoading } = useHomePolicyReviewCycleComplianceIntelligence();
  ...
  const ratingStyle = RATING_STYLES[d.policy_rating as PolicyReviewRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.policy_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-teal-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ScrollText className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-teal-600")} />
            <span className="text-slate-900 font-bold">Policy Review Cycle Compliance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.policy_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.policy_score}%</span>}
          </CardTitle>
        </div>
      ...
      <CardContent className="space-y-4">
        {d.policy_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.review_schedule_rate >= 90 ? "bg-green-50" : d.review_schedule_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.review_schedule_rate >= 90 ? "text-green-600" : d.review_schedule_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.review_schedule_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Reviews</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.version_control_rate >= 90 ? "bg-green-50" : d.version_control_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.version_control_rate >= 90 ? "text-green-600" : d.version_control_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.version_control_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_acknowledgement_rate >= 90 ? "bg-green-50" : d.staff_acknowledgement_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_acknowledgement_rate >= 90 ? "text-green-600" : d.staff_acknowledgement_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.staff_acknowledgement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.regulatory_alignment_rate >= 90 ? "bg-green-50" : d.regulatory_alignment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.regulatory_alignment_rate >= 90 ? "text-green-600" : d.regulatory_alignment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.regulatory_alignment_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Aligned</p>
            </div>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.update_timeliness_rate >= 90 ? "bg-green-50" : d.update_timeliness_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.update_timeliness_rate >= 90 ? "text-green-600" : d.update_timeliness_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.update_timeliness_rate}%</p>
