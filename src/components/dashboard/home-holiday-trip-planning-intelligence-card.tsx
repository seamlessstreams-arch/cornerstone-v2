...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Plane } from "lucide-react";
import { cn } from "@/lib/utils";
import { useHomeHolidayTripPlanningIntelligence } from "@/hooks/use-home-holiday-trip-planning-intelligence";
import type { HolidayTripRating } from "@/lib/engines/home-holiday-trip-planning-intelligence-engine";

const RATING_STYLES: Record<HolidayTripRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeHolidayTripPlanningIntelligenceCard() {
  const { data, isLoading } = useHomeHolidayTripPlanningIntelligence();
  ...
  const __emptyState = d.holiday_rating === "inadequate" && (d.holiday_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      holiday_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.holiday_rating as HolidayTripRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.holiday_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-sky-600 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-sky-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Plane className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-sky-600")} />
            <span className="text-slate-900 font-bold">Holiday & Trip Planning</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.holiday_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.holiday_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.holiday_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.holiday_planning_rate >= 90 ? "bg-green-50" : d.holiday_planning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.holiday_planning_rate >= 90 ? "text-green-600" : d.holiday_planning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.holiday_planning_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.risk_assessment_rate >= 90 ? "bg-green-50" : d.risk_assessment_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.risk_assessment_rate >= 90 ? "text-green-600" : d.risk_assessment_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.risk_assessment_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Risk Asst.</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.consent_compliance_rate >= 90 ? "bg-green-50" : d.consent_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.consent_compliance_rate >= 90 ? "text-green-600" : d.consent_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.consent_compliance_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Consent</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.experience_quality_rate >= 90 ? "bg-green-50" : d.experience_quality_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.experience_quality_rate >= 90 ? "text-green-600" : d.experience_quality_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.experience_quality_rate}%</p>
