...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Key } from "lucide-react";
...
export function HomeKeyholdingAccessControlIntelligenceCard() {
  const { data, isLoading } = useHomeKeyholdingAccessControlIntelligence();
  ...
  const __emptyState = d.keyholding_rating === "inadequate" && (d.keyholding_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      keyholding_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.keyholding_rating as KeyholdingRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.keyholding_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-stone-600 border-2")} role="region" aria-label="Cara Keyholding & Access Control Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-stone-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Key className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-stone-600")} />
            <span className="text-slate-900 font-bold">Keyholding & Access Control</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.keyholding_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.keyholding_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.keyholding_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.key_register_rate >= 90 ? "bg-green-50" : d.key_register_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.key_register_rate >= 90 ? "text-green-600" : d.key_register_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.key_register_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.access_control_rate >= 90 ? "bg-green-50" : d.access_control_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.access_control_rate >= 90 ? "text-green-600" : d.access_control_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.access_control_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.key_tracking_rate >= 90 ? "bg-green-50" : d.key_tracking_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.key_tracking_rate >= 90 ? "text-green-600" : d.key_tracking_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.key_tracking_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.security_audit_rate >= 90 ? "bg-green-50" : d.security_audit_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.security_audit_rate >= 90 ? "text-green-600" : d.security_audit_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.security_audit_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_safe_rate >= 90 ? "bg-green-50" : d.child_safe_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_safe_rate >= 90 ? "text-green-600" : d.child_safe_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_safe_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_compliance_rate >= 90 ? "bg-green-50" : d.staff_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_compliance_rate >= 90 ? "text-green-600" : d.staff_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.staff_compliance_rate}%</p>
