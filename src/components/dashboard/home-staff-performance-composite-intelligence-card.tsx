...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, BarChart3 } from "lucide-react";
import { cn } from "@/lib/utils";
import { useHomeStaffPerformanceCompositeIntelligence } from "@/hooks/use-home-staff-performance-composite-intelligence";
import type { StaffPerformanceRating } from "@/lib/engines/home-staff-performance-composite-intelligence-engine";
...
export function HomeStaffPerformanceCompositeIntelligenceCard() {
  const { data, isLoading } = useHomeStaffPerformanceCompositeIntelligence();
  ...
  const __emptyState = d.performance_rating === "inadequate" && (d.performance_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.performance_rating as StaffPerformanceRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.performance_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-slate-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-slate-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <BarChart3 className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-slate-600")} />
            <span className="text-slate-900 font-bold">Staff Performance</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.performance_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.performance_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.performance_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.supervision_completion_rate >= 95 ? "bg-green-50" : d.supervision_completion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.supervision_completion_rate >= 95 ? "text-green-600" : d.supervision_completion_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.supervision_completion_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Supervision</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.safeguarding_discussion_rate >= 95 ? "bg-green-50" : d.safeguarding_discussion_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safeguarding_discussion_rate >= 95 ? "text-green-600" : d.safeguarding_discussion_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.safeguarding_discussion_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.expired_mandatory_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.expired_mandatory_count === 0 ? "text-green-600" : "text-red-600")}>{d.expired_mandatory_count}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.action_completion_rate >= 90 ? "bg-green-50" : d.action_completion_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.action_completion_rate >= 90 ? "text-green-600" : d.action_completion_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.action_completion_rate}%</p>
