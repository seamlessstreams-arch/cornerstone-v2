...
import { useHomeSelfEvaluationImprovementIntelligence } from "@/hooks/use-home-self-evaluation-improvement-intelligence";
import type { SelfEvaluationRating } from "@/lib/engines/home-self-evaluation-improvement-intelligence-engine";

const RATING_STYLES: Record<SelfEvaluationRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSelfEvaluationImprovementIntelligenceCard() {
  const { data, isLoading } = useHomeSelfEvaluationImprovementIntelligence();
  ...
  const __emptyState = d.evaluation_rating === "inadequate" && (d.evaluation_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.evaluation_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.evaluation_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-rose-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-rose-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <ClipboardList className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-rose-600")} />
            <span className="text-slate-900 font-bold">Self-Evaluation & Improvement</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.evaluation_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.evaluation_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.evaluation_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.evaluation_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_areas > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_areas > 0 ? "text-green-600" : "text-amber-600")}>{d.total_areas}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.good_or_outstanding_rate >= 80 ? "bg-green-50" : d.good_or_outstanding_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.good_or_outstanding_rate >= 80 ? "text-green-600" : d.good_or_outstanding_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.good_or_outstanding_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.evidence_coverage_rate >= 90 ? "bg-green-50" : d.evidence_coverage_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.evidence_coverage_rate >= 90 ? "text-green-600" : d.evidence_coverage_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.evidence_coverage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_strengths_per_area >= 5 ? "bg-green-50" : d.average_strengths_per_area >= 3 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_strengths_per_area >= 5 ? "text-green-600" : d.average_strengths_per_area >= 3 ? "text-amber-600" : "text-red-600")}>{d.average_strengths_per_area}</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Evaluation Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
