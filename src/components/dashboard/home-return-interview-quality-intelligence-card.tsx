...
export function HomeReturnInterviewQualityIntelligenceCard() {
  const { data, isLoading } = useHomeReturnInterviewQualityIntelligence();
  ...
  const __emptyState = d.interview_rating === "inadequate" && (d.interview_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.interview_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.interview_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-fuchsia-300 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <UserSearch className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-fuchsia-600")} />
            <span className="text-slate-900 font-bold">Return Home Interviews</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.interview_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.interview_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.interview_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.interview_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_interviews > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_interviews > 0 ? "text-green-600" : "text-amber-600")}>{d.total_interviews}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.independence_rate >= 80 ? "bg-green-50" : d.independence_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.independence_rate >= 80 ? "text-green-600" : d.independence_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.independence_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.action_completion_rate >= 85 ? "bg-green-50" : d.action_completion_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.action_completion_rate >= 85 ? "text-green-600" : d.action_completion_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.action_completion_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Return Interview Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
