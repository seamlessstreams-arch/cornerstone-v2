...
const RATING_STYLES: Record<AttachmentProfileRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeAttachmentProfileIntelligenceCard() {
  const { data, isLoading } = useHomeAttachmentProfileIntelligence();
  ...
  const __emptyState = d.profile_rating === "inadequate" && (d.profile_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.profile_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.profile_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-cyan-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Anchor className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
            <span className="text-slate-900 font-bold">Attachment Profiles</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.profile_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.profile_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.profile_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.profile_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_profiles > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_profiles > 0 ? "text-green-600" : "text-amber-600")}>{d.total_profiles}</p>
              <p className="text-[9px] text-muted-foreground">Profiles</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_profile_rate >= 80 ? "bg-green-50" : d.children_with_profile_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_profile_rate >= 80 ? "text-green-600" : d.children_with_profile_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.children_with_profile_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Coverage</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.active_profile_rate >= 85 ? "bg-green-50" : d.active_profile_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_profile_rate >= 85 ? "text-green-600" : d.active_profile_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.active_profile_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Active</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.behaviour_analysis_rate >= 75 ? "bg-green-50" : d.behaviour_analysis_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.behaviour_analysis_rate >= 75 ? "text-green-600" : d.behaviour_analysis_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.behaviour_analysis_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.staff_guidance_rate >= 80 ? "bg-green-50" : d.staff_guidance_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.staff_guidance_rate >= 80 ? "text-green-600" : d.staff_guidance_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.staff_guidance_rate}%</p>
