...
import { useHomeContinencePersonalHygieneSupportIntelligence } from "@/hooks/use-home-continence-personal-hygiene-support-intelligence";
import type { ContinenceHygieneRating } from "@/lib/engines/home-continence-personal-hygiene-support-intelligence-engine";

const RATING_STYLES: Record<ContinenceHygieneRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeContinencePersonalHygieneSupportIntelligenceCard() {
  const { data, isLoading } = useHomeContinencePersonalHygieneSupportIntelligence();
  ...
  const __emptyState = d.hygiene_rating === "inadequate" && (d.hygiene_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      hygiene_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.hygiene_rating as ContinenceHygieneRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.hygiene_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Sparkles className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
            <span className="text-slate-900 font-bold">Continence & Personal Hygiene Support</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.hygiene_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.hygiene_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.hygiene_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.continence_plan_rate >= 90 ? "bg-green-50" : d.continence_plan_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.continence_plan_rate >= 90 ? "text-green-600" : d.continence_plan_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.continence_plan_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Plans</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.hygiene_routine_rate >= 90 ? "bg-green-50" : d.hygiene_routine_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hygiene_routine_rate >= 90 ? "text-green-600" : d.hygiene_routine_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.hygiene_routine_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Routines</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.dignity_compliance_rate >= 90 ? "bg-green-50" : d.dignity_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.dignity_compliance_rate >= 90 ? "text-green-600" : d.dignity_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.dignity_compliance_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.age_appropriate_rate >= 90 ? "bg-green-50" : d.age_appropriate_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.age_appropriate_rate >= 90 ? "text-green-600" : d.age_appropriate_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.age_appropriate_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Guidance</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.product_provision_rate >= 90 ? "bg-green-50" : d.product_provision_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.product_provision_rate >= 90 ? "text-green-600" : d.product_provision_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.product_provision_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_independence_rate >= 90 ? "bg-green-50" : d.child_independence_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_independence_rate >= 90 ? "text-green-600" : d.child_independence_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.child_independence_rate}%</p>
