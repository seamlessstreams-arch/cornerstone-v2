...
export function HomeCaraContentQualityIntelligenceCard() {
  ...
  const __emptyState = d.content_rating === "inadequate" && (d.content_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-fuchsia-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <PenTool className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-fuchsia-600")} />
        ...
        {d.content_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.approval_rate >= 90 ? "bg-green-50" : d.approval_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.approval_rate >= 90 ? "text-[--cs-success]" : d.approval_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.approval_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_quality_score >= 80 ? "bg-green-50" : d.average_quality_score >= 65 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_quality_score >= 80 ? "text-[--cs-success]" : d.average_quality_score >= 65 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.average_quality_score}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.average_evidence_confidence >= 75 ? "bg-green-50" : d.average_evidence_confidence >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.average_evidence_confidence >= 75 ? "text-[--cs-success]" : d.average_evidence_confidence >= 60 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.average_evidence_confidence}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.framework_usage_rate >= 80 ? "bg-green-50" : d.framework_usage_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.framework_usage_rate >= 80 ? "text-[--cs-success]" : d.framework_usage_rate >= 50 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.framework_usage_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.artifact_type_diversity >= 4 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.artifact_type_diversity >= 4 ? "text-[--cs-success]" : "text-[--cs-warning]")}>{d.artifact_type_diversity}</p>
