...
import { Loader2, AlertCircle, AlertTriangle, Sparkles, Brain, Palette } from "lucide-react";
import { cn } from "@/lib/utils";
import { useHomeHobbiesInterestsDevelopmentIntelligence } from "@/hooks/use-home-hobbies-interests-development-intelligence";
import type { HobbiesInterestsRating } from "@/lib/engines/home-hobbies-interests-development-intelligence-engine";

const RATING_STYLES: Record<HobbiesInterestsRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeHobbiesInterestsDevelopmentIntelligenceCard() {
  const { data, isLoading } = useHomeHobbiesInterestsDevelopmentIntelligence();
  ...
  const __emptyState = d.hobbies_rating === "inadequate" && (d.hobbies_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      hobbies_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.hobbies_rating as HobbiesInterestsRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.hobbies_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-pink-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Palette className={cn("h-4 w-4", isAlert ? "text-[--cs-risk]" : "text-pink-600")} />
            <span className="text-slate-900 font-bold">Hobbies & Interests Development</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.hobbies_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.hobbies_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.hobbies_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.hobby_participation_rate >= 90 ? "bg-green-50" : d.hobby_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.hobby_participation_rate >= 90 ? "text-[--cs-success]" : d.hobby_participation_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.hobby_participation_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Hobbies</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.interest_exploration_rate >= 90 ? "bg-green-50" : d.interest_exploration_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.interest_exploration_rate >= 90 ? "text-[--cs-success]" : d.interest_exploration_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.interest_exploration_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Explore</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.talent_development_rate >= 90 ? "bg-green-50" : d.talent_development_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.talent_development_rate >= 90 ? "text-[--cs-success]" : d.talent_development_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.talent_development_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Talent</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.creative_expression_rate >= 90 ? "bg-green-50" : d.creative_expression_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.creative_expression_rate >= 90 ? "text-[--cs-success]" : d.creative_expression_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.creative_expression_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Creative</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.child_led_rate >= 90 ? "bg-green-50" : d.child_led_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_led_rate >= 90 ? "text-[--cs-success]" : d.child_led_rate >= 70 ? "text-[--cs-warning]" : "text-[--cs-risk]")}>{d.child_led_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_satisfaction_rate >= 90 ? "bg-green-50" : d.child_satisfaction_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              ...
              <p className="text-[9px] text-muted-foreground">Satisfact.</p>
            </div>
          </div>
        )}
