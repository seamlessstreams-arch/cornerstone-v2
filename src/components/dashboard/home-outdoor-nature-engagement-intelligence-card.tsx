...
const RATING_STYLES: Record<OutdoorNatureRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeOutdoorNatureEngagementIntelligenceCard() {
  const { data, isLoading } = useHomeOutdoorNatureEngagementIntelligence();
  ...
  const __emptyState = d.outdoor_rating === "inadequate" && (d.outdoor_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      outdoor_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.outdoor_rating as OutdoorNatureRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.outdoor_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-green-600 border-2")} aria-label="Cara Outdoor & Nature Engagement Intelligence">
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-green-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <TreePine className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-green-600")} />
            <span className="text-slate-900 font-bold">Outdoor & Nature Engagement</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.outdoor_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.outdoor_score}%</span>}
          </CardTitle>
        </div>
        ...
        {d.outdoor_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.outdoor_frequency_rate >= 90 ? "bg-green-50" : d.outdoor_frequency_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.outdoor_frequency_rate >= 90 ? "text-green-600" : d.outdoor_frequency_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.outdoor_frequency_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.nature_learning_rate >= 90 ? "bg-green-50" : d.nature_learning_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.nature_learning_rate >= 90 ? "text-green-600" : d.nature_learning_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.nature_learning_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.garden_participation_rate >= 90 ? "bg-green-50" : d.garden_participation_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.garden_participation_rate >= 90 ? "text-green-600" : d.garden_participation_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.garden_participation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.exploration_diversity_rate >= 90 ? "bg-green-50" : d.exploration_diversity_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.exploration_diversity_rate >= 90 ? "text-green-600" : d.exploration_diversity_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.exploration_diversity_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.safety_compliance_rate >= 90 ? "bg-green-50" : d.safety_compliance_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.safety_compliance_rate >= 90 ? "text-green-600" : d.safety_compliance_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.safety_compliance_rate}%</p>
