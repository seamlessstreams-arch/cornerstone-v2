...
import { useHomeStakeholderEngagementFeedbackIntelligence } from "@/hooks/use-home-stakeholder-engagement-feedback-intelligence";
import type { StakeholderEngagementRating } from "@/lib/engines/home-stakeholder-engagement-feedback-intelligence-engine";

const RATING_STYLES: Record<StakeholderEngagementRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeStakeholderEngagementFeedbackIntelligenceCard() {
  const { data, isLoading } = useHomeStakeholderEngagementFeedbackIntelligence();
  ...
  const __emptyState = d.stakeholder_rating === "inadequate" && (d.stakeholder_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.stakeholder_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.stakeholder_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-violet-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Users className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
            <span className="text-slate-900 font-bold">Stakeholder Engagement</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.stakeholder_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.stakeholder_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.stakeholder_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.stakeholder_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_feedback_items > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_feedback_items > 0 ? "text-green-600" : "text-amber-600")}>{d.total_feedback_items}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.positive_sentiment_rate >= 80 ? "bg-green-50" : d.positive_sentiment_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.positive_sentiment_rate >= 80 ? "text-green-600" : d.positive_sentiment_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.positive_sentiment_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.response_rate >= 90 ? "bg-green-50" : d.response_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.response_rate >= 90 ? "text-green-600" : d.response_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.response_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.parent_engagement_rate >= 80 ? "bg-green-50" : d.parent_engagement_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.parent_engagement_rate >= 80 ? "text-green-600" : d.parent_engagement_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.parent_engagement_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.community_sentiment_rate >= 80 ? "bg-green-50" : d.community_sentiment_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.community_sentiment_rate >= 80 ? "text-green-600" : d.community_sentiment_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.community_sentiment_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Stakeholder Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
