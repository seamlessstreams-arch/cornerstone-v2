...
import type { FormulationRating } from "@/lib/engines/home-multidisciplinary-formulation-intelligence-engine";

const RATING_STYLES: Record<FormulationRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeMultidisciplinaryFormulationIntelligenceCard() {
  const { data, isLoading } = useHomeMultidisciplinaryFormulationIntelligence();
  ...
  const __emptyState = d.formulation_rating === "inadequate" && (d.formulation_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.formulation_rating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.formulation_rating === "inadequate";
      ...
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-violet-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Shapes className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-violet-600")} />
            <span className="text-slate-900 font-bold">Multi-Disciplinary Formulation</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.formulation_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.formulation_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.formulation_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.formulation_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_formulations > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_formulations > 0 ? "text-green-600" : "text-amber-600")}>{d.total_formulations}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_with_formulation_rate >= 80 ? "bg-green-50" : d.children_with_formulation_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_with_formulation_rate >= 80 ? "text-green-600" : d.children_with_formulation_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.children_with_formulation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.four_p_completeness_rate >= 85 ? "bg-green-50" : d.four_p_completeness_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.four_p_completeness_rate >= 85 ? "text-green-600" : d.four_p_completeness_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.four_p_completeness_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_contribution_rate >= 90 ? "bg-green-50" : d.child_contribution_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_contribution_rate >= 90 ? "text-green-600" : d.child_contribution_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.child_contribution_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.intervention_planning_rate >= 90 ? "bg-green-50" : d.intervention_planning_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.intervention_planning_rate >= 90 ? "text-green-600" : d.intervention_planning_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.intervention_planning_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.multi_agency_rate >= 80 ? "bg-green-50" : d.multi_agency_rate >= 50 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.multi_agency_rate >= 80 ? "text-green-600" : d.multi_agency_rate >= 50 ? "text-amber-600" : "text-red-600")}>{d.multi_agency_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Formulation Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
