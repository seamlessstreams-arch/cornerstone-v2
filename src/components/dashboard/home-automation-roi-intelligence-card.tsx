...
import type { AutomationROIRating } from "@/lib/engines/home-automation-roi-intelligence-engine";

const RATING_STYLES: Record<AutomationROIRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeAutomationROIIntelligenceCard() {
  const { data, isLoading } = useHomeAutomationROIIntelligence();
  ...
  const __emptyState = d.automation_rating === "inadequate" && (d.automation_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.automation_rating as AutomationROIRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.automation_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-cyan-500 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-cyan-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Zap className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-cyan-600")} />
            <span className="text-slate-900 font-bold">Automation ROI</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.automation_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.automation_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.automation_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.total_time_saved > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.total_time_saved > 0 ? "text-green-600" : "text-amber-600")}>{d.total_time_saved}m</p>
              <p className="text-[9px] text-muted-foreground">Saved</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.route_success_rate >= 90 ? "bg-green-50" : d.route_success_rate >= 70 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.route_success_rate >= 90 ? "text-green-600" : d.route_success_rate >= 70 ? "text-amber-600" : "text-red-600")}>{d.route_success_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Routes</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.automation_coverage >= 80 ? "bg-green-50" : d.automation_coverage >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.automation_coverage >= 80 ? "text-green-600" : d.automation_coverage >= 60 ? "text-amber-600" : "text-red-600")}>{d.automation_coverage}%</p>
              <p className="text-[9px] text-muted-foreground">Coverage</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.error_rate <= 5 ? "bg-green-50" : d.error_rate <= 15 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.error_rate <= 5 ? "text-green-600" : d.error_rate <= 15 ? "text-amber-600" : "text-red-600")}>{d.error_rate}%</p>
              <p className="text-[9px] text-muted-foreground">Errors</p>
            </div>
            <div className={cn("text-center rounded-lg p-1.5", d.route_type_diversity >= 3 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.route_type_diversity >= 3 ? "text-green-600" : "text-amber-600")}>{d.route_type_diversity}</p>
