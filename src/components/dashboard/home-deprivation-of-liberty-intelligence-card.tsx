...
import type { DeprivationOfLibertyRating } from "@/lib/engines/home-deprivation-of-liberty-intelligence-engine";

const RATING_STYLES: Record<DeprivationOfLibertyRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeDeprivationOfLibertyIntelligenceCard() {
  const { data, isLoading } = useHomeDeprivationOfLibertyIntelligence();
  ...
  const __emptyState = d.dol_rating === "inadequate" && (d.dol_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
      dol_rating: "insufficient_data",
      concerns: [],
      recommendations: [],
      insights: [],
      headline:
  ...
  const ratingStyle = RATING_STYLES[d.dol_rating as DeprivationOfLibertyRating] ?? RATING_STYLES.insufficient_data;
  const isAlert = d.dol_rating === "inadequate";

  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-blue-400 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-blue-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Lock className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-blue-600")} />
            <span className="text-slate-900 font-bold">Liberty & Rights</span>
            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border", ratingStyle.bg, ratingStyle.text, ratingStyle.border)}>{ratingStyle.label}</span>
            {d.dol_rating !== "insufficient_data" && <span className="text-xs font-bold tabular-nums text-slate-600">{d.dol_score}%</span>}
          </CardTitle>
        </div>
        <p className="text-xs text-muted-foreground mt-1">{d.headline}</p>
      </CardHeader>
      <CardContent className="space-y-4">
        {d.dol_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.active_restrictions === 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.active_restrictions === 0 ? "text-green-600" : "text-amber-600")}>{d.active_restrictions}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.proportionality_rate >= 98 ? "bg-green-50" : d.proportionality_rate >= 85 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.proportionality_rate >= 98 ? "text-green-600" : d.proportionality_rate >= 85 ? "text-amber-600" : "text-red-600")}>{d.proportionality_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.child_consultation_rate >= 95 ? "bg-green-50" : d.child_consultation_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.child_consultation_rate >= 95 ? "text-green-600" : d.child_consultation_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.child_consultation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sw_consultation_rate >= 95 ? "bg-green-50" : d.sw_consultation_rate >= 80 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sw_consultation_rate >= 95 ? "text-green-600" : d.sw_consultation_rate >= 80 ? "text-amber-600" : "text-red-600")}>{d.sw_consultation_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.overdue_review_count === 0 ? "bg-green-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.overdue_review_count === 0 ? "text-green-600" : "text-red-600")}>{d.overdue_review_count}</p>
