...
import type { SensoryTherapeuticRating } from "@/lib/engines/home-sensory-therapeutic-environment-intelligence-engine";

const RATING_STYLES: Record<SensoryTherapeuticRating, { bg: string; text: string; border: string; label: string }> = {
...
export function HomeSensoryTherapeuticEnvironmentIntelligenceCard() {
  const { data, isLoading } = useHomeSensoryTherapeuticEnvironmentIntelligence();
  ...
  const __emptyState = d.sensory_rating === "inadequate" && (d.sensory_score ?? 0) <= 15;
  if (__emptyState) {
    d = {
      ...d,
  ...
  const ratingStyle = RATING_STYLES[d.sensory_rating] ?? RATING_STYLES.insufficient_data;
  ...
  return (
    <Card className={cn("overflow-hidden", isAlert ? "border-red-400 border-2" : "border-purple-200 border-2")}>
      <CardHeader className={cn("pb-3", isAlert ? "bg-red-50" : "bg-purple-50/50")}>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Flower2 className={cn("h-4 w-4", isAlert ? "text-red-600" : "text-purple-600")} />
            <span className="text-slate-900 font-bold">Sensory & Therapeutic</span>
      ...
      <CardContent className="space-y-4">
        {d.sensory_rating === "insufficient_data" && <IntelligenceCardEmpty />}
        {d.sensory_rating !== "insufficient_data" && (
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
            <div className={cn("text-center rounded-lg p-1.5", d.children_using_sensory_room > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_using_sensory_room > 0 ? "text-green-600" : "text-amber-600")}>{d.children_using_sensory_room}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.sensory_beneficial_rate >= 85 ? "bg-green-50" : d.sensory_beneficial_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.sensory_beneficial_rate >= 85 ? "text-green-600" : d.sensory_beneficial_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.sensory_beneficial_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.equipment_condition_rate >= 85 ? "bg-green-50" : d.equipment_condition_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.equipment_condition_rate >= 85 ? "text-green-600" : d.equipment_condition_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.equipment_condition_rate}%</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.children_physically_active > 0 ? "bg-green-50" : "bg-amber-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.children_physically_active > 0 ? "text-green-600" : "text-amber-600")}>{d.children_physically_active}</p>
            ...
            <div className={cn("text-center rounded-lg p-1.5", d.activity_enjoyment_rate >= 85 ? "bg-green-50" : d.activity_enjoyment_rate >= 60 ? "bg-amber-50" : "bg-red-50")}>
              <p className={cn("text-sm font-bold tabular-nums", d.activity_enjoyment_rate >= 85 ? "text-green-600" : d.activity_enjoyment_rate >= 60 ? "text-amber-600" : "text-red-600")}>{d.activity_enjoyment_rate}%</p>
        ...
        {d.insights.length > 0 && (<div className="space-y-1.5"><p className="text-xs font-semibold flex items-center gap-1 text-purple-700"><Brain className="h-3 w-3" /> Cara Therapeutic Intelligence</p>{d.insights.slice(0, 3).map((insight, i) => (<div key={i} className={cn("rounded border p-2.5 text-xs leading-relaxed", INSIGHT_STYLES[insight.severity] ?? INSIGHT_STYLES.warning)}>{insight.text}</div>))}</div>)}
      </CardContent>
    </Card>
  );
}
