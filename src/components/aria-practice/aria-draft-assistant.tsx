...
import { useAriaPracticeDraft, type DraftInput } from "@/hooks/use-aria-practice";
import type { AriaDraftType } from "@/lib/aria-practice/aria-draft";
import type { PracticeSourceType } from "@/lib/aria-practice/types";

const DRAFT_TYPES: { value: AriaDraftType; label: string }[] = [
...
export interface AriaDraftAssistantProps {
...
export function AriaDraftAssistant(props: AriaDraftAssistantProps) {
  const [draftType, setDraftType] = useState<AriaDraftType>("professional_record");
  const [content, setContent] = useState(props.content ?? "");
  const [sections, setSections] = useState<{ heading: string; body: string }[] | null>(null);
  const [narrative, setNarrative] = useState<string>("");
  const draft = useAriaPracticeDraft();
      ...
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <select
            ...
            value={draftType}
            onChange={(e) => setDraftType(e.target.value as AriaDraftType)}
          >
