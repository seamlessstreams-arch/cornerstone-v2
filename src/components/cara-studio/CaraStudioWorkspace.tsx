...
interface CaraStudioWorkspaceProps {
  childId?: string;
  childName?: string;
...
export function CaraStudioWorkspace({
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/cara-studio/approve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ...
                        : "border-[var(--cs-border)] bg-white hover:border-[var(--cs-cara-gold-soft)]",
                    )}
                  >
                    <Icon className={cn("h-3.5 w-3.5 shrink-0", isSelected ? "text-[var(--cs-cara-gold)]" : "text-[var(--cs-text-muted)]")} />
        ...
        <div className="rounded-xl border border-[var(--cs-border)] bg-white p-5 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-[var(--cs-border)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--cs-cara-gold)] resize-none"
            />
          </div>
