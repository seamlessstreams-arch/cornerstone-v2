# Cara — Universal Layer

The universal Cara layer sits beneath every Cara-driven feature in
Cara. Domain engines (Management Oversight, Voice of the Child, HR
...
queued for the next phases, and how to set Cara up.
...
- **`src/lib/aria/aria-permissions.ts`** — RBAC matrix mapping the spec's
  ...
  team leader, RSW, HR/admin, auditor, viewer) to the spec's Cara
...
- **`src/lib/aria/aria-service.ts`** — orchestrator. `invokeAriaCommand`
  authenticates, checks Cara permission, builds a redacted context
...
- Cara entry points in every dashboard / form / module (Phase 8 of the
...
Every Cara draft persisted by the universal layer follows the same flow:
...
- `aria_audit_events` — every event in an Cara request / output's life
...
Cara is told explicitly:

- Output is always labelled "Cara suggested draft".
...
Coverage percentage = modules with at least one dedicated Cara command ÷ 27.
...
Cara supports professional judgement. Cara does not replace the
...
Cara-generated record must be reviewed by a human before it is used.
