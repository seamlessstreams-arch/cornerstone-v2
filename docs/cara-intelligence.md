# Cara Intelligence — Cara's Operational AI Layer
...
**Core Principle:** Cara suggests. Humans decide. Cara evidences.
...
1. Cara must not make final decisions
2. Cara must not diagnose children
3. Cara must not invent evidence
4. Cara must identify missing evidence
5. Cara must write in UK professional language
6. Cara must not blame children or staff
7. Cara must not create unsupported safeguarding conclusions
8. Cara must escalate high-risk themes
9. Cara must maintain child-centred, trauma-informed wording
10. Cara must keep reports factual, balanced and evidence-linked
...
|------------|-------------|
| `aria:view` | View Cara reports and dashboard |
...
| Route | Purpose |
|-------|---------|
| `/cara` | Redirects to `/cara/dashboard` |
| `/cara/dashboard` | 8-stat grid, recent reports, quick actions |
| `/cara/reports` | Reports list with type/status/search filters |
| `/cara/reports/new` | Report generation form |
| `/cara/reports/[reportId]` | Two-column review: sections + evidence/challenge/actions |
| `/cara/reg45` | Regulation 45 Evidence Bank with filters |
...
When a report is locked, it can be filed into the Cara filing cabinet via `POST /api/aria/reports/[id]/file`. Filing paths follow the pattern `young-people/{child_id}/aria-reports/{report_type}/{date}`.
...
No new environment variables required. Cara Reports uses the existing AI provider configuration:
...
## Cara Universal Command Layer

The universal command layer (built on top of the report system above) provides Cara across every module in Cara via a single entry point.
...
src/
  ...
  components/aria/
    ...
    aria-global-fab.tsx               # Floating action button — Cara on every page
    ...
    aria-task-creator.tsx             # Parse Cara task suggestions, confirm/edit/create
  ...
  app/api/aria/
    ...
    create-tasks/route.ts            # Bulk task creation from Cara output
```
...
| Table | Purpose |
|-------|---------|
| `aria_requests` | Every Cara command invocation with context |
...
| `aria_context_links` | Source records included in Cara context |
| `aria_task_links` | Links Cara outputs to created tasks |
...
| `aria.create_tasks` | Create tasks from Cara output |
| `aria.commit_to_records` | Write back to records |
| `aria.approve_outputs` | Approve Cara drafts |
| `aria.reject_outputs` | Reject Cara drafts |
| `aria.view_audit_logs` | View Cara audit trail |
| `aria.admin_config` | Cara configuration |
...
`aria-context-builder.ts` fetches operational records from 16+ source tables to enrich Cara commands with real data:
...
### Task Creation from Cara

Cara commands with `canCreateTasks: true` produce structured task suggestions. The `AriaTaskCreator` component parses these and lets the manager:
...
- Tasks are created via `/api/v1/tasks` with `auto_generated: true` and linked back to the Cara output via `aria_task_links`

### Global Access

`AriaGlobalFab` provides a floating Cara button on every platform page. It:
